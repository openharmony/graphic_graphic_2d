...
class OHFilterBlurWithDirectionTest : public RSGraphicTest {
public:
...
GRAPHIC_TEST(OHFilterBlurWithDirectionTest, EFFECT_TEST, BlurWithDirectionTest1)
{
    int columnCount = 2;
    ...
    for (int i = 0; i < rowCount; i++) {
        ...
        OH_Filter_BlurWithDirection(ohFilter,
            blurWithDirectionParams1[i].radius,
            blurWithDirectionParams1[i].effectDirection,
            blurWithDirectionParams1[i].tileMode);
...
GRAPHIC_TEST(OHFilterBlurWithDirectionTest, EFFECT_TEST, BlurWithDirectionTest2)
{
    int columnCount = 2;
    int rowCount = static_cast<int>(OHFilterTestDataGroupParamsType::COUNT);
    auto sizeX = screenWidth / columnCount;
    ...
    for (int i = 0; i < rowCount; i++) {
        ...
        OH_Filter_BlurWithDirection(ohFilter,
            blurWithDirectionParams2[i].radius,
            blurWithDirectionParams2[i].effectDirection,
            blurWithDirectionParams2[i].tileMode);
        OH_Filter_GetEffectPixelMap(ohFilter, &pixelMapNative);

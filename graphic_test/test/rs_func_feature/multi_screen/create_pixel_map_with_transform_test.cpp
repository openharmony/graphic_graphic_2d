...
public:
    ...
    void OnBufferAvailable() override
    {
        ...
        std::shared_ptr<Media::PixelMap> pixelMap =
            RSInterfaces::GetInstance().CreatePixelMapFromSurfaceId(producerSurface_->GetUniqueId(), rect, true);
        if (pixelMap == nullptr) {
...
class RSCreatePixelMapWithTransformTest : public RSGraphicTest {
public:
...
/*
 * @tc.name: RSCreatePixelMapWithTransformTest001
 * @tc.desc: attach two display nodes to screen node, capture displayNode0
 * @tc.type: FUNC
 */
GRAPHIC_N_TEST(RSCreatePixelMapWithTransformTest, CONTENT_DISPLAY_TEST, RSCreatePixelMapWithTransformTest001)
{
    uint32_t width = 640;
    uint32_t height = 640;
    ...
    LOGI("RSCreatePixelMapWithTransformTest001 screenId[%{public}" PRIu64 "], nodeId[%{public}" PRIu64 "]",

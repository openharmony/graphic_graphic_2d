...
// Radius values
const std::vector<Vector2f> radiiXY = {
    Vector2f{0.0f, 0.0f},
    Vector2f{0.5f, 0.5f},
    Vector2f{1.0f, 1.0f}
};
...
class NGShaderLightCaveTest : public RSGraphicTest {
public:
...
private:
        ...
        std::shared_ptr<RSNGLightCave>& lightCave)
    {
        ...
        testNode->SetBackgroundNGShader(lightCave);
        GetRootNode()->AddChild(testNode);
        RegisterNode(testNode);
    }
};

GRAPHIC_TEST(NGShaderLightCaveTest, EFFECT_TEST, Set_Light_Cave_Progress_Boundary_Test)
{
...
GRAPHIC_TEST(NGShaderLightCaveTest, EFFECT_TEST, Set_Light_Cave_Position_Radius_Combination_Test)
{

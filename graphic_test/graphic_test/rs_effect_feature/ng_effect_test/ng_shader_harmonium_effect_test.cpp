...
public:
    ...
    std::shared_ptr<RSEffectNode> SetUpEffectNode()
    {
        ...
        std::shared_ptr<Rosen::RSFilter> backFilter = Rosen::RSFilter::CreateMaterialFilter(10., 1, 1, 0, BLUR_COLOR_MODE::DEFAULT, true);
...
{
    auto effectNode = SetUpEffectNode();
    SetUpHarmoniumEffect<Vector4f>(effectNode, [](std::shared_ptr<RSNGHarmoniumEffect>& harmoniumEffect, Vector4f tintcolor)
        {harmoniumEffect->Setter<HarmoniumEffectTintColorTag>(tintcolor);}, tintColorVec);
}
...
{
    auto effectNode = SetUpEffectNode();
    SetUpHarmoniumEffect<float>(effectNode, [](std::shared_ptr<RSNGHarmoniumEffect>& harmoniumEffect, float lightUpDegree)
        {harmoniumEffect->Setter<HarmoniumEffectLightUpDegreeTag>(lightUpDegree);}, lightUpDegreeVec);
}
...
{
    auto effectNode = SetUpEffectNode();
    SetUpHarmoniumEffect<Vector3f>(effectNode, [](std::shared_ptr<RSNGHarmoniumEffect>& harmoniumEffect, Vector3f posRgb)
        {harmoniumEffect->Setter<HarmoniumEffectPosRGBTag>(posRgb);}, posRgbVec);
}

GRAPHIC_TEST(NGShaderHarmoniumEffectTest, EFFECT_TEST, Set_NG_Shader_Harmonium_Effect_Negrgb_Test)
{
    auto effectNode = SetUpEffectNode();
    SetUpHarmoniumEffect<Vector3f>(effectNode, [](std::shared_ptr<RSNGHarmoniumEffect>& harmoniumEffect, Vector3f negRgb)
        {harmoniumEffect->Setter<HarmoniumEffectNegRGBTag>(negRgb);}, negRgbVec);
}

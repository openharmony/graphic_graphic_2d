...
class BackgroundTest07 : public RSGraphicTest {
private:
...
GRAPHIC_TEST(BackgroundTest07, CONTENT_DISPLAY_TEST, Alpha_Color_Blend_Mode)
{
    int columnCount = 6;
    int rowCount = 5;
...
GRAPHIC_TEST(BackgroundTest07, CONTENT_DISPLAY_TEST, Alpha_Color_Blend_Apply_Type)
{
    int columnCount = 1;
    int rowCount = 3;
    auto sizeX = screenWidth / columnCount;
    ...
    for (auto i = static_cast<int>(RSColorBlendApplyType::FAST); i <= static_cast<int>(RSColorBlendApplyType::MAX);
        ...
        auto testNodeBackGround =
            SetUpNodeBgImage("/data/local/tmp/Images/backGroundImage.jpg", { x, y, sizeX - 10, sizeY - 10 });
        testNodeBackGround->SetAlpha(0.8f);
        testNodeBackGround->SetBorderStyle(0, 0, 0, 0);
        testNodeBackGround->SetBorderWidth(5, 5, 5, 5);
        testNodeBackGround->SetBorderColor(Vector4<Color>(RgbPalette::Green()));
...
GRAPHIC_TEST(BackgroundTest07, CONTENT_DISPLAY_TEST, Alpha_Color_Blend_Mode_With_Alpha)
{
    int columnCount = 1;
    int rowCount = 3;
    auto sizeX = screenWidth / columnCount;

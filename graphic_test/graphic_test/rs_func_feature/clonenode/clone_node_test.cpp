...
private:
    ...
    RSSurfaceNode::SharedPtr CreateCloneNodeWithImageCanvas(const Vector4f& bounds, uint32_t colorValue)
    {
        RSSurfaceNodeConfig clonedSurfaceNodeConfig;
        clonedSurfaceNodeConfig.isSync = true;
        clonedSurfaceNodeConfig.SurfaceNodeName = "cloneSurfaceNode";
        ...
        if (!clonedSurfaceNode) {
            LOGE("CreateCloneNodeWithImageCanvas failed");
            return nullptr;
        }
        clonedSurfaceNode->SetBounds(bounds);
        clonedSurfaceNode->SetFrame(bounds);
        clonedSurfaceNode->SetBackgroundColor(bounds);
    ...
    RSSurfaceNode::SharedPtr CreatSurfaceNode(const Vector4f& bounds, std::string& name)
    {
...
{
    auto clonedSurfaceNode = CreateCloneNodeWithImageCanvas({0, 0, 500, 500}, 0xFFFF0000);
    if (!clonedSurfaceNode) {
        LOGE("Create CreateCloneNodeWithImageCanvas failed");
        return;
    }
    ...
    auto containerSurfaceNode = CreatSurfaceNode({0, 0, 500, 500}, containerSurfaceNodeName);
    ...
    auto cloneSurfaceNode = CreatSurfaceNode({0, 0, 500, 500}, containerSurfaceNodeName);
    if (!cloneSurfaceNode) {
...
GRAPHIC_TEST(CloneNodeTest, CONTENT_DISPLAY_TEST, CLONE_NODE_MULTIPLE_ONE2ONE_TEST)
{
    ...
    for (int i = 0; i < 3; i++) {
        auto clonedSurfaceNode = CreateCloneNodeWithImageCanvas({0, (width + gap) * i, width, height},
            backgroundColor[i]);
        if (!clonedSurfaceNode) {
            LOGE("Create CreateCloneNodeWithImageCanvas failed");
            return;
        ...
        auto containerSurfaceNode = CreatSurfaceNode({0, (width + gap) * i, width, height},
        ...
        auto cloneSurfaceNode = CreatSurfaceNode({0, 0, width, height + (height + gap) * i},
            containerSurfaceNodeName);
        if (!cloneSurfaceNode) {
            LOGE("Create cloneSurfaceNode failed");
            return;
...
GRAPHIC_TEST(CloneNodeTest, CONTENT_DISPLAY_TEST, CLONE_NODE_ONE2N_TEST)
{
    ...
    for (int y = 0; y < 2; y++) {
        for (int i = 0; i < 2; i++) {
            auto clonedSurfaceNode = CreateCloneNodeWithImageCanvas({rowGap * y, lineGap * i, width, height},
            ...
            GetRootNode()->AddChild(clonedSurfaceNode);
            for (int j = 0; j < 3; j ++) {
                auto containerSurfaceNode = CreatSurfaceNode(
                ...
                auto cloneSurfaceNode = CreatSurfaceNode({0, 0, width, height}, containerSurfaceNodeName);
                if (!cloneSurfaceNode) {
                    LOGE("Create cloneSurfaceNode failed");
                    return;
                }
...
GRAPHIC_TEST(CloneNodeTest, CONTENT_DISPLAY_TEST, CLONE_NODE_ONE2N_TEST2)
{
    int width = 200;
    int height = 200;
    int gap = 50;
    ...
    for (int y = 0; y < 2; y++) {
        for (int i = 0; i < 2; i++) {
            ...
            for (int j = 0; j < 3; j ++) {
                ...
                cloneSurfaceNode->SetAlpha(0.5 * (j + 1));
                cloneSurfaceNode->SetClonedNodeInfo(clonedSurfaceNode->GetId(), true, true);
                RegisterNode(cloneSurfaceNode);
                containerSurfaceNode->AddChild(cloneSurfaceNode, 0);   
            }

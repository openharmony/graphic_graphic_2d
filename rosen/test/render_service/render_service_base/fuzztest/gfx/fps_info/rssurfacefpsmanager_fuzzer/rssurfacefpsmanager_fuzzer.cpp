...
std::shared_ptr<RSSurfaceFps> CreateSurfaceFps(const std::string& name)
{
    std::shared_ptr<RSSurfaceFps> surfaceFpsPtr = std::make_shared<RSSurfaceFps>(name);
    uint8_t num = GetData<uint8_t>();
    for (uint8_t i = 0; i < num; i++) {
        uint64_t presentTime = GetData<uint64_t>();
        uint32_t seqNum = GetData<uint32_t>();
        surfaceFpsPtr->RecordPresentTime(presentTime, seqNum);
    }

...
#include "rstaskdispatcher_fuzzer.h"
...
bool DoRegisterTaskDispatchFunc(const uint8_t* data, size_t size)
{
    if (data == nullptr) {
        return false;
    }
...
bool DoPostTask(const uint8_t* data, size_t size)
{
    ...
    RSTask task = []() {};
    RSTaskDispatcher::GetInstance().PostTask(tid, task);
    return true;
}
bool DoHasRegisteredTask(const uint8_t* data, size_t size)
{
    if (data == nullptr) {
        return false;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::Rosen::DoHasRegisteredTask(data, size);
    return 0;
}

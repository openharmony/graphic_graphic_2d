...
/**
 * @tc.name: DrawColorUsingSDFWithDRMNullGeShapeTest
 * @tc.desc: DrawColorUsingSDFWithDRM returns safely when geShape is null
 * @tc.type: FUNC
 * @tc.require: issue25648
 */
HWTEST_F(RSPropertyDrawableUtilsTest, DrawColorUsingSDFWithDRMNullGeShapeTest, testing::ext::TestSize.Level1)
{
    auto utils = std::make_shared<RSPropertyDrawableUtils>();
    ...
    auto filterGEContainer = std::make_shared<Drawing::GEVisualEffectContainer>();
    auto geVisualEffect = std::make_shared<Drawing::GEVisualEffect>("testFilter");
    filterGEContainer->AddToChainedFilter(geVisualEffect);

    bool isDark = true;
    utils->DrawColorUsingSDFWithDRM(&canvas, &rect, isDark, filterGEContainer, "testFilter", "NonExistentShape");
    EXPECT_TRUE(isDark);
    ...
    isDark = true;
    utils->DrawColorUsingSDFWithDRM(&canvas, &rect, isDark, filterGEContainer, "testFilter", "");
    EXPECT_TRUE(isDark);

    isDark = false;
    utils->DrawColorUsingSDFWithDRM(&canvas, &rect, isDark, filterGEContainer, "testFilter", "AnotherShape");
    EXPECT_FALSE(isDark);
}

/**
 * @tc.name: DrawColorUsingSDFWithDRMWithGeShapeTest
 * @tc.desc: DrawColorUsingSDFWithDRM with non-null geShape exercises full path
 * @tc.type: FUNC
 * @tc.require: issue25648
 */
HWTEST_F(RSPropertyDrawableUtilsTest, DrawColorUsingSDFWithDRMWithGeShapeTest, testing::ext::TestSize.Level1)
{
    auto utils = std::make_shared<RSPropertyDrawableUtils>();
    EXPECT_NE(utils, nullptr);

    // Build a frosted glass filter with SDF shape via RSNG framework
    RSProperties properties;
    ...
    auto filterGEContainer = drawingFilter->GetGEContainer();
    EXPECT_NE(filterGEContainer, nullptr);
    auto visualEffect = filterGEContainer->GetGEVisualEffect(Drawing::GE_FILTER_FROSTED_GLASS);
    EXPECT_NE(visualEffect, nullptr);
    auto geShape = visualEffect->GetGEShaderShape(Drawing::GE_FILTER_FROSTED_GLASS_SHAPE);
    ...
    bool isDark = true;
    utils->DrawColorUsingSDFWithDRM(&canvas, &rect, isDark, filterGEContainer,
        Drawing::GE_FILTER_FROSTED_GLASS, Drawing::GE_FILTER_FROSTED_GLASS_SHAPE);
...
HWTEST_F(RSPropertyDrawableUtilsTest, RSFilterSetPixelStretchTest021, testing::ext::TestSize.Level1)
{
    ...
    properties.SetPixelStretch(pixelStretchTest2);

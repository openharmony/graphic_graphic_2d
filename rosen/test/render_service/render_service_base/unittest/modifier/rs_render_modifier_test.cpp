...
HWTEST_F(RSRenderModifierTest, RSForegroundNGFilterRenderModifier001, TestSize.Level1)
{
    ...
    EXPECT_NE(modifier->GetProperty(), nullptr);
    modifier->Update(nullptr, false);
    EXPECT_NE(modifier->GetProperty(), nullptr);

    Parcel parcel;
    EXPECT_TRUE(modifier->Marshalling(parcel));
    EXPECT_EQ(RSRenderModifier::Unmarshalling(parcel), nullptr);
}

/**
 * @tc.name: RSBackgroundNGFilterRenderModifier001
 * @tc.desc: RSBackgroundNGFilterRenderModifier001
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(RSRenderModifierTest, RSBackgroundNGFilterRenderModifier001, TestSize.Level1)
{
    auto prop1 = std::make_shared<RSRenderProperty<std::shared_ptr<RSNGRenderBlurFilter>>>();
    auto modifier = std::make_shared<RSBackgroundNGFilterRenderModifier>(prop1);
    EXPECT_EQ(modifier->GetProperty(), prop1);

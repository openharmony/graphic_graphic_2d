...
/**
 * @tc.name: GetAbilityStateByNodeId001
 * @tc.desc: test results of GetAbilityStateByNodeId
 * @tc.type: FUNC
 * @tc.require: issueIAQL48
 */
HWTEST_F(RSRenderNodeMapTest, GetAbilityStateByNodeId001, TestSize.Level1)
{
    pid_t pid = 0;
    RSRenderNodeMap rsRenderNodeMap;
    bool abilityState = rsRenderNodeMap.GetAbilityStateByPid(pid);
    EXPECT_TRUE(abilityState);
    rsRenderNodeMap.AddBackgroundPid(pid);
    abilityState = rsRenderNodeMap.GetAbilityStateByPid(id);
    EXPECT_FALSE(abilityState);
    rsRenderNodeMap.RemoveBackgroundPid(pid);
    pid_t pid_1 = 1;
    rsRenderNodeMap.RemoveBackgroundPid(pid_1);
    abilityState = rsRenderNodeMap.GetAbilityStateByPid(id);
    EXPECT_TRUE(abilityState);
}

/**
 * @tc.name: IsAllSurfaceNodesWithSamePidOnBackground
 * @tc.desc: test results of IsAllSurfaceNodesWithSamePidOnBackground
 * @tc.type: FUNC
 * @tc.require: issueIAQL48
 */
HWTEST_F(RSRenderNodeMapTest, IsAllSurfaceNodesWithSamePidOnBackground, TestSize.Level1)
{
    pid_t pid = 0;
    RSRenderNodeMap rsRenderNodeMap;
    bool ret = rsRenderNodeMap.IsAllSurfaceNodesWithSamePidOnBackground(pid);
    EXPECT_TRUE(ret);
    NodeId id0 = 0;
    auto rssurfaceRenderNode0 = std::make_shared<RSSurfaceRenderNode>(id0);
    rsRenderNodeMap.surfaceNodeMap_[id0] = rssurfaceRenderNode0;
    NodeId id1 = 1;
    auto rssurfaceRenderNode1 = std::make_shared<RSSurfaceRenderNode>(id1);
    rsRenderNodeMap.surfaceNodeMap_[id1] = rssurfaceRenderNode1;
    ret = rsRenderNodeMap.IsAllSurfaceNodesWithSamePidOnBackground(pid);
    EXPECT_FALSE(ret);
    rssurfaceRenderNode0->abilityState_ = false;
    ret = rsRenderNodeMap.IsAllSurfaceNodesWithSamePidOnBackground(pid);
    EXPECT_FALSE(ret);
    rssurfaceRenderNode1->abilityState_ = false;
    ret = rsRenderNodeMap.IsAllSurfaceNodesWithSamePidOnBackground(pid);
    EXPECT_TRUE(ret);
    rsRenderNodeMap.surfaceNodeMap_[id0] = nullptr;
    ret = rsRenderNodeMap.IsAllSurfaceNodesWithSamePidOnBackground(pid);
    EXPECT_TRUE(ret);
}
} // namespace OHOS::Rosen
...
#include "hardwareEnabledstub_fuzzer.h"
...
void DoSetHardwareEnabled(FuzzedDataProvider& fdp)
{
    ...
    uint8_t selfDrawingType = fdp.ConsumeIntegral<uint8_t>();
    bool dynamicHardwareEnable = fdp.ConsumeBool();
    
    if (!dataP.WriteInterfaceToken(RSIClientToServiceConnection::GetDescriptor())) {
        return;
    }
    if (!dataP.WriteUint64(id) || !dataP.WriteBool(isEnabled) ||
        !dataP.WriteUint8(static_cast<uint8_t>(selfDrawingType)) ||

...
#include "bufferAvailableListenerstub_fuzzer.h"
...
public:
    ...
    ~CustomTestBufferAvailableCallback() override = default;
...
void DoSetBufferAvailableListener(FuzzedDataProvider& fdp)
{
    ...
    sptr<RSIBufferAvailableCallback> callback = new CustomTestBufferAvailableCallback();
    auto nodeId = fdp.ConsumeIntegral<NodeId>();
    ...
    if (!dataP.WriteBool(isFromRenderThread)) {
        return;
    }

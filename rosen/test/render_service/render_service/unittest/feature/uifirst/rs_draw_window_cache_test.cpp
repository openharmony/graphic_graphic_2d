...
/**
 * @tc.name: DrawAndCacheWindowContentOcclusionTest
 * @tc.desc: Test DrawAndCacheWindowContent with occlusion enabled
 * @tc.type: FUNC
 * @tc.require: issues22651
 */
HWTEST_F(RSDrawWindowCacheTest, DrawAndCacheWindowContentOcclusionTest, TestSize.Level1)
{
    RSDrawWindowCache drawWindowCache;
    Drawing::Canvas drawingCanvas;
    RSPaintFilterCanvas canvas(&drawingCanvas);
    ...
    auto drawable = DrawableV2::RSSurfaceRenderNodeDrawable::OnGenerate(surfaceNode);
    ASSERT_NE(drawable, nullptr);
    auto surfaceDrawable = static_cast<DrawableV2::RSSurfaceRenderNodeDrawable*>(drawable);
    ASSERT_NE(surfaceDrawable, nullptr);
    ...
    // Test with ClonedSourceNode = false (should call InsertOpaqueRegion)
    auto surfaceParams = static_cast<RSSurfaceRenderParams*>(surfaceDrawable->renderParams_.get());
    surfaceParams->SetIsCloned(false);

    // Enable occlusion for testing
...
/**
 * @tc.name: DrawCacheOcclusionTest
 * @tc.desc: Test DrawCache with occlusion enabled
 * @tc.type: FUNC
 * @tc.require: issues22651
 */
HWTEST_F(RSDrawWindowCacheTest, DrawCacheOcclusionTest, TestSize.Level1)
{
    RSDrawWindowCache drawWindowCache;
    Drawing::Canvas drawingCanvas;
    RSPaintFilterCanvas canvas(&drawingCanvas);
    ...
    // Test with valid drawable and image
    RSDrawWindowCache::DrawCache(surfaceDrawable, canvas, surfaceParams, image);

    // Test with ClonedSourceNode = true (should skip InsertOpaqueRegion)

...
HWTEST_F(RSRoundCornerDisplayTest, RSRcdRenderManager, TestSize.Level1)
{
    ...
    context->nodeMap.UnregisterRenderNode(id);
    context->nodeMap.RegisterDisplayRenderNode(std::make_shared<RSDisplayRenderNode>(id, config, context));
    rcdManagerInstance.CheckRenderTargetNode(*context);
    context->nodeMap.UnregisterRenderNode(id);
    context->nodeMap.RegisterDisplayRenderNode(std::make_shared<RSDisplayRenderNode>(id + 1, config, context));
    rcdManagerInstance.CheckRenderTargetNode(*context);

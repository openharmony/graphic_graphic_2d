...
#ifdef RS_ENABLE_VK
    static std::unordered_set<uint64_t> CreateImagesFromBufferTest(
        std::shared_ptr<RSRenderEngine> renderEngine, uint32_t imageNums);
#endif
};

#ifdef RS_ENABLE_VK
std::unordered_set<uint64_t> RSBaseRenderEngineUnitTest::CreateImagesFromBufferTest(
    std::shared_ptr<RSRenderEngine> renderEngine, uint32_t imageNums)
{

...
const xmlChar DISABLE_HWC_IN_SCALE_SCNEN[] = "IsDisableInScaleScene";
...
/**
 * @tc.name: TestParseHwcInternal004
 * @tc.desc: Verify the ParseHwcInternal function
 * @tc.type: FUNC
 * @tc.require: #21317
 */
HWTEST_F(HwcParamParseTest, TestParseHwcInternal004, TestSize.Level1)
{
    xmlNode node;
    node.xmlChildrenNode = nullptr;
    node.name = NODE_NAME_SWITCH;
    ...
    xmlAttribute attrVal = CreateXmlAttribute(ATTRIBUTE_VALUE, TV_PLAYER_BUNDLE_NAME_VALUE, nullptr);
    xmlAttribute attrName = CreateXmlAttribute(ATTRIBUTE_NAME, DISABLE_HWC_IN_SCALE_SCNEN, &attrVal);
    node.properties = reinterpret_cast<xmlAttrPtr>(&attrName);

 ...
 * @tc.require:
 */
HWTEST_F(RSScreenManagerTest, OnProcessDisconnectedTest001, TestSize.Level2)
{
    ASSERT_NE(nullptr, screenManager_);
...
/*
 * @tc.name: OnProcessDisconnectedTest002
 * @tc.desc: Test OnProcessDisconnected with screen not in screenMap
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(RSScreenManagerTest, OnProcessDisconnectedTest002, TestSize.Level2)
{
    ASSERT_NE(nullptr, screenManager_);
    std::vector<std::pair<ScreenId, std::shared_ptr<HdiOutput>>> screens;
    ScreenId notExistId = 99999;
    auto output = std::make_shared<HdiOutput>(static_cast<uint32_t>(notExistId));
    screens.push_back({notExistId, output});
    screenManager_->OnProcessDisconnected(screens);
}

/*
 * @tc.name: OnProcessDisconnectedTest003
 * @tc.desc: Test OnProcessDisconnected with screen in map but null RSScreen
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(RSScreenManagerTest, OnProcessDisconnectedTest003, TestSize.Level2)
{
    ASSERT_NE(nullptr, screenManager_);
    uint32_t screenId = 1050;
    {
        std::lock_guard<std::mutex> lock(screenManager_->screenMapMutex_);
        screenManager_->screens_[screenId] = nullptr;
    }
    ...
    std::vector<std::pair<ScreenId, std::shared_ptr<HdiOutput>>> screens;
    screens.push_back({static_cast<ScreenId>(screenId), output});
    screenManager_->OnProcessDisconnected(screens);
    {
        std::lock_guard<std::mutex> lock(screenManager_->screenMapMutex_);
        screenManager_->screens_.erase(screenId);
...
/*
 * @tc.name: OnProcessDisconnectedTest004
 * @tc.desc: Test OnProcessDisconnected with physical screen and null output
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(RSScreenManagerTest, OnProcessDisconnectedTest004, TestSize.Level2)
{
    ASSERT_NE(nullptr, screenManager_);
    uint32_t screenId = 1100;
    screenManager_->MockHdiScreenConnected(std::make_shared<RSScreen>(screenId));
    std::vector<std::pair<ScreenId, std::shared_ptr<HdiOutput>>> screens;
    screens.push_back({static_cast<ScreenId>(screenId), nullptr});
    screenManager_->OnProcessDisconnected(screens);
}

/*
 * @tc.name: OnProcessDisconnectedTest005
 * @tc.desc: Test OnProcessDisconnected with physical screen and valid output
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(RSScreenManagerTest, OnProcessDisconnectedTest005, TestSize.Level2)
{
    ASSERT_NE(nullptr, screenManager_);
    uint32_t screenId = 1200;
    screenManager_->MockHdiScreenConnected(std::make_shared<RSScreen>(screenId));
    auto output = std::make_shared<HdiOutput>(screenId);
    std::vector<std::pair<ScreenId, std::shared_ptr<HdiOutput>>> screens;
    screens.push_back({static_cast<ScreenId>(screenId), output});
...
/*
 * @tc.name: OnProcessDisconnectedTest006
 * @tc.desc: Test OnProcessDisconnected with virtual screen
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(RSScreenManagerTest, OnProcessDisconnectedTest006, TestSize.Level2)
{
    ASSERT_NE(nullptr, screenManager_);
    auto virtualScreenId =
        screenManager_->CreateVirtualScreen("virtual_ut", VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT, nullptr);
    ASSERT_NE(virtualScreenId, INVALID_SCREEN_ID);
    std::vector<std::pair<ScreenId, std::shared_ptr<HdiOutput>>> screens;
    screens.push_back({virtualScreenId, nullptr});
    screenManager_->OnProcessDisconnected(screens);
    usleep(SLEEP_TIME_US);
}

/*
 * @tc.name: OnProcessDisconnectedTest007
 * @tc.desc: Test OnProcessDisconnected with mixed physical and virtual screens
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(RSScreenManagerTest, OnProcessDisconnectedTest007, TestSize.Level2)
{
    ASSERT_NE(nullptr, screenManager_);
    // Setup physical screen
    uint32_t physicalScreenId = 1300;
    screenManager_->MockHdiScreenConnected(std::make_shared<RSScreen>(physicalScreenId));
    auto physicalOutput = std::make_shared<HdiOutput>(physicalScreenId);
    // Setup virtual screen
    auto virtualScreenId =
        screenManager_->CreateVirtualScreen("virtual_mixed", VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT, nullptr);
    ASSERT_NE(virtualScreenId, INVALID_SCREEN_ID);
    std::vector<std::pair<ScreenId, std::shared_ptr<HdiOutput>>> screens;
    screens.push_back({static_cast<ScreenId>(physicalScreenId), physicalOutput});
    screens.push_back({virtualScreenId, nullptr});
    screenManager_->OnProcessDisconnected(screens);
    usleep(SLEEP_TIME_US);
}
...
namespace {
constexpr uint32_t MAX_VIRTUAL_SCREEN_NUM_LIMIT = 64;
constexpr uint32_t MAX_WIDTH_LIMIT = 65536;
constexpr uint32_t MAX_HEIGHT_LIMIT = 65536;
constexpr uint32_t EXCEEDED_WIDTH = 65537;
constexpr uint32_t EXCEEDED_HEIGHT = 65537;

sptr<Surface> CreateTestSurfaceForMultiSurface()
{
    auto csurface = IConsumerSurface::Create();
    if (csurface == nullptr) {
        return nullptr;
...
ScreenId CreateTestVirtualScreenForMultiSurface(const sptr<RSScreenManager>& mgr,
    uint32_t width = VIRTUAL_SCREEN_WIDTH, uint32_t height = VIRTUAL_SCREEN_HEIGHT)
{
    return mgr->CreateVirtualScreen("testVScreen", width, height, nullptr, INVALID_SCREEN_ID, 0, {});
}
}
 ...
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, AddVirtualScreenSurface_001, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto id = CreateTestVirtualScreenForMultiSurface(screenManager_);
    ASSERT_NE(INVALID_SCREEN_ID, id);

    std::vector<SurfaceRegionConfig> emptyConfigs;
    auto result = screenManager_->AddVirtualScreenSurface(id, emptyConfigs);
    EXPECT_EQ(INVALID_ARGUMENTS, result);
 ...
 * @tc.desc: Test AddVirtualScreenSurface with screen not found
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, AddVirtualScreenSurface_002, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto psurface = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface, nullptr);

    std::vector<SurfaceRegionConfig> configs;
    configs.emplace_back(SurfaceRegionConfig{psurface, RectI{0, 0, VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT}});

    auto result = screenManager_->AddVirtualScreenSurface(INVALID_SCREEN_ID, configs);
    EXPECT_EQ(SCREEN_NOT_FOUND, result);
}

/*
 * @tc.name: AddVirtualScreenSurface_003
 * @tc.desc: Test AddVirtualScreenSurface with non-virtual screen
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, AddVirtualScreenSurface_003, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    ScreenId physId = 0xFFFF;
    auto rsScreen = std::make_shared<RSScreen>(physId);
    screenManager_->MockHdiScreenConnected(rsScreen);
    ...
    auto result = screenManager_->AddVirtualScreenSurface(physId, configs);
    EXPECT_EQ(INVALID_ARGUMENTS, result);

    screenManager_->screens_.erase(physId);
}

/*
 * @tc.name: AddVirtualScreenSurface_004
 * @tc.desc: Test AddVirtualScreenSurface with all null surfaces
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, AddVirtualScreenSurface_004, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto id = CreateTestVirtualScreenForMultiSurface(screenManager_);
    ASSERT_NE(INVALID_SCREEN_ID, id);

    std::vector<SurfaceRegionConfig> nullConfigs;
    nullConfigs.emplace_back(SurfaceRegionConfig{nullptr, RectI{}});
    nullConfigs.emplace_back(SurfaceRegionConfig{nullptr, RectI{}});

    auto result = screenManager_->AddVirtualScreenSurface(id, nullConfigs);
    EXPECT_EQ(INVALID_ARGUMENTS, result);
...
/*
 * @tc.name: AddVirtualScreenSurface_005
 * @tc.desc: Test AddVirtualScreenSurface with surface already used by another screen
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, AddVirtualScreenSurface_005, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto psurface = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface, nullptr);

    auto id1 = screenManager_->CreateVirtualScreen(
        "vScreen1", VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT, psurface, INVALID_SCREEN_ID, 0, {});
    ASSERT_NE(INVALID_SCREEN_ID, id1);

    auto id2 = CreateTestVirtualScreenForMultiSurface(screenManager_);
    ASSERT_NE(INVALID_SCREEN_ID, id2);

    std::vector<SurfaceRegionConfig> configs;
    configs.emplace_back(SurfaceRegionConfig{psurface, RectI{0, 0, VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT}});

    auto result = screenManager_->AddVirtualScreenSurface(id2, configs);
    EXPECT_EQ(SURFACE_NOT_UNIQUE, result);
...
/*
 * @tc.name: AddVirtualScreenSurface_006
 * @tc.desc: Test AddVirtualScreenSurface success
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, AddVirtualScreenSurface_006, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto id = CreateTestVirtualScreenForMultiSurface(screenManager_);
    ASSERT_NE(INVALID_SCREEN_ID, id);
    ...
    auto result = screenManager_->AddVirtualScreenSurface(id, configs);
    EXPECT_EQ(SUCCESS, result);

    auto screen = screenManager_->GetScreen(id);
    ASSERT_NE(screen, nullptr);
    auto surfaceConfigs = screen->GetMultiSurfaceConfigs();
    EXPECT_EQ(configs.size(), surfaceConfigs.size());
...
/*
 * @tc.name: AddVirtualScreenSurface_007
 * @tc.desc: Test AddVirtualScreenSurface with mixed null and valid surfaces
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, AddVirtualScreenSurface_007, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto id = CreateTestVirtualScreenForMultiSurface(screenManager_);
    ASSERT_NE(INVALID_SCREEN_ID, id);
    ...
    std::vector<SurfaceRegionConfig> mixedConfigs;
    mixedConfigs.emplace_back(SurfaceRegionConfig{nullptr, RectI{}});
    mixedConfigs.emplace_back(SurfaceRegionConfig{psurface, RectI{0, 0, VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT}});

    auto result = screenManager_->AddVirtualScreenSurface(id, mixedConfigs);
    EXPECT_EQ(SUCCESS, result);

    auto screen = screenManager_->GetScreen(id);
    ASSERT_NE(screen, nullptr);
    auto surfaceConfigs = screen->GetMultiSurfaceConfigs();
    EXPECT_EQ(static_cast<size_t>(1), surfaceConfigs.size());
...
/*
 * @tc.name: AddVirtualScreenSurface_008
 * @tc.desc: Test AddVirtualScreenSurface with surface already on the same screen returns SURFACE_NOT_UNIQUE
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, AddVirtualScreenSurface_008, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto psurface = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface, nullptr);
    ...
    auto id = screenManager_->CreateVirtualScreen(
        "sameScreenTest", VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT, psurface, INVALID_SCREEN_ID, 0, {});
    ASSERT_NE(INVALID_SCREEN_ID, id);

    auto screen = screenManager_->GetScreen(id);
    ASSERT_NE(screen, nullptr);
    auto existingConfigs = screen->GetMultiSurfaceConfigs();
    ASSERT_FALSE(existingConfigs.empty());

    std::vector<SurfaceRegionConfig> dupConfigs;
    dupConfigs.emplace_back(SurfaceRegionConfig{psurface, RectI{0, 0, VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT}});
    auto result = screenManager_->AddVirtualScreenSurface(id, dupConfigs);
    EXPECT_EQ(SURFACE_NOT_UNIQUE, result);
 ...
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, RemoveVirtualScreenSurface_001, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto id = CreateTestVirtualScreenForMultiSurface(screenManager_);
    ASSERT_NE(INVALID_SCREEN_ID, id);
    ...
    auto result = screenManager_->RemoveVirtualScreenSurface(id, emptySurfaces);
    EXPECT_EQ(INVALID_ARGUMENTS, result);
 ...
 * @tc.desc: Test RemoveVirtualScreenSurface with screen not found
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, RemoveVirtualScreenSurface_002, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto psurface = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface, nullptr);
    ...
    auto result = screenManager_->RemoveVirtualScreenSurface(INVALID_SCREEN_ID, surfaces);
    EXPECT_EQ(SCREEN_NOT_FOUND, result);
}

/*
 * @tc.name: RemoveVirtualScreenSurface_003
 * @tc.desc: Test RemoveVirtualScreenSurface with non-virtual screen
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, RemoveVirtualScreenSurface_003, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    ScreenId physId = 0xFFFE;
    auto rsScreen = std::make_shared<RSScreen>(physId);
    screenManager_->MockHdiScreenConnected(rsScreen);
    ...
    auto result = screenManager_->RemoveVirtualScreenSurface(physId, surfaces);
    EXPECT_EQ(INVALID_ARGUMENTS, result);
...
/*
 * @tc.name: RemoveVirtualScreenSurface_004
 * @tc.desc: Test RemoveVirtualScreenSurface success
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, RemoveVirtualScreenSurface_004, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto psurface = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface, nullptr);
    ...
    auto id = screenManager_->CreateVirtualScreen(
        "vScreen", VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT, psurface, INVALID_SCREEN_ID, 0, {});
    ASSERT_NE(INVALID_SCREEN_ID, id);

    auto screen = screenManager_->GetScreen(id);
    ASSERT_NE(screen, nullptr);
    EXPECT_FALSE(screen->GetMultiSurfaceConfigs().empty());

    std::vector<sptr<Surface>> surfaces = {psurface};
    auto result = screenManager_->RemoveVirtualScreenSurface(id, surfaces);
    EXPECT_EQ(SUCCESS, result);
...
/*
 * @tc.name: RemoveVirtualScreenSurface_005
 * @tc.desc: Test RemoveVirtualScreenSurface with null surfaces in vector (filtered out)
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, RemoveVirtualScreenSurface_005, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto psurface = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface, nullptr);
    ...
    std::vector<sptr<Surface>> surfaces = {nullptr, psurface};
    auto result = screenManager_->RemoveVirtualScreenSurface(id, surfaces);
    EXPECT_EQ(SUCCESS, result);
...
/*
 * @tc.name: SetVirtualScreenSurface_005
 * @tc.desc: Test SetVirtualScreenSurface with null surface
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, SetVirtualScreenSurface_005, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto result = screenManager_->SetVirtualScreenSurface(INVALID_SCREEN_ID, nullptr);
    EXPECT_EQ(INVALID_ARGUMENTS, result);
}

/*
 * @tc.name: SetVirtualScreenSurface_006
 * @tc.desc: Test SetVirtualScreenSurface with screen not found
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, SetVirtualScreenSurface_006, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto psurface = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface, nullptr);

    auto result = screenManager_->SetVirtualScreenSurface(INVALID_SCREEN_ID, psurface);
    EXPECT_EQ(SCREEN_NOT_FOUND, result);
}

/*
 * @tc.name: SetVirtualScreenSurface_007
 * @tc.desc: Test SetVirtualScreenSurface with non-virtual screen
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, SetVirtualScreenSurface_007, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    ScreenId physId = 0xFFFD;
    auto rsScreen = std::make_shared<RSScreen>(physId);
    screenManager_->MockHdiScreenConnected(rsScreen);
    ...
    auto result = screenManager_->SetVirtualScreenSurface(physId, psurface);
    EXPECT_EQ(INVALID_ARGUMENTS, result);
...
/*
 * @tc.name: SetVirtualScreenSurface_008
 * @tc.desc: Test SetVirtualScreenSurface with surface already used by another screen
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, SetVirtualScreenSurface_008, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto psurface = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface, nullptr);
    ...
    auto result = screenManager_->SetVirtualScreenSurface(id2, psurface);
    EXPECT_EQ(SURFACE_NOT_UNIQUE, result);
...
/*
 * @tc.name: SetVirtualScreenSurface_009
 * @tc.desc: Test SetVirtualScreenSurface success
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, SetVirtualScreenSurface_009, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto id = CreateTestVirtualScreenForMultiSurface(screenManager_);
    ASSERT_NE(INVALID_SCREEN_ID, id);
    ...
    auto result = screenManager_->SetVirtualScreenSurface(id, psurface);
    EXPECT_EQ(SUCCESS, result);

    auto screen = screenManager_->GetScreen(id);
    ASSERT_NE(screen, nullptr);
    auto configs = screen->GetMultiSurfaceConfigs();
    EXPECT_EQ(static_cast<size_t>(1), configs.size());
...
/*
 * @tc.name: CollectVirtualScreenSurfaceIds_001
 * @tc.desc: Test CollectVirtualScreenSurfaceIds excludes given screenId
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, CollectVirtualScreenSurfaceIds_001, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto psurface1 = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface1, nullptr);
    auto psurface2 = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface2, nullptr);

    auto id1 = screenManager_->CreateVirtualScreen(
        "vScreen1", VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT, psurface1, INVALID_SCREEN_ID, 0, {});
    ASSERT_NE(INVALID_SCREEN_ID, id1);
    auto id2 = screenManager_->CreateVirtualScreen(
        "vScreen2", VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT, psurface2, INVALID_SCREEN_ID, 0, {});
    ASSERT_NE(INVALID_SCREEN_ID, id2);

    auto ids = screenManager_->CollectVirtualScreenSurfaceIds(id1);
    EXPECT_EQ(static_cast<size_t>(1), ids.size());
    EXPECT_NE(ids.end(), ids.find(psurface2->GetUniqueId()));
    EXPECT_EQ(ids.end(), ids.find(psurface1->GetUniqueId()));
...
/*
 * @tc.name: CollectVirtualScreenSurfaceIds_002
 * @tc.desc: Test CollectVirtualScreenSurfaceIds skips null and non-virtual screens
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, CollectVirtualScreenSurfaceIds_002, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    ScreenId physId = 0xFFFC;
    auto rsScreen = std::make_shared<RSScreen>(physId);
    screenManager_->MockHdiScreenConnected(rsScreen);

    auto ids = screenManager_->CollectVirtualScreenSurfaceIds();
    EXPECT_EQ(static_cast<size_t>(0), ids.count(physId));

    screenManager_->screens_.erase(physId);
}

/*
 * @tc.name: CollectVirtualScreenSurfaceIds_003
 * @tc.desc: Test CollectVirtualScreenSurfaceIds collects from all virtual screens
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, CollectVirtualScreenSurfaceIds_003, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto psurface1 = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface1, nullptr);
    ...
    auto ids = screenManager_->CollectVirtualScreenSurfaceIds();
    EXPECT_EQ(static_cast<size_t>(2), ids.size());
    EXPECT_NE(ids.end(), ids.find(psurface1->GetUniqueId()));
    EXPECT_NE(ids.end(), ids.find(psurface2->GetUniqueId()));
...
/*
 * @tc.name: ValidateVirtualScreenLimits_001
 * @tc.desc: Test ValidateVirtualScreenLimits exceeds max count
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, ValidateVirtualScreenLimits_001, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto savedCount = screenManager_->currentVirtualScreenNum_.load();
    screenManager_->currentVirtualScreenNum_ = MAX_VIRTUAL_SCREEN_NUM_LIMIT;
    EXPECT_FALSE(screenManager_->ValidateVirtualScreenLimits(VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT));
    screenManager_->currentVirtualScreenNum_ = savedCount;
}

/*
 * @tc.name: ValidateVirtualScreenLimits_002
 * @tc.desc: Test ValidateVirtualScreenLimits exceeds max width
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, ValidateVirtualScreenLimits_002, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    EXPECT_FALSE(screenManager_->ValidateVirtualScreenLimits(EXCEEDED_WIDTH, VIRTUAL_SCREEN_HEIGHT));
}

/*
 * @tc.name: ValidateVirtualScreenLimits_003
 * @tc.desc: Test ValidateVirtualScreenLimits exceeds max height
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, ValidateVirtualScreenLimits_003, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    EXPECT_FALSE(screenManager_->ValidateVirtualScreenLimits(VIRTUAL_SCREEN_WIDTH, EXCEEDED_HEIGHT));
}

/*
 * @tc.name: ValidateVirtualScreenLimits_004
 * @tc.desc: Test ValidateVirtualScreenLimits valid limits
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, ValidateVirtualScreenLimits_004, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto savedCount = screenManager_->currentVirtualScreenNum_.load();
    screenManager_->currentVirtualScreenNum_ = 0;
    EXPECT_TRUE(screenManager_->ValidateVirtualScreenLimits(VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT));
    screenManager_->currentVirtualScreenNum_ = savedCount;
}

/*
 * @tc.name: CreateVirtualScreen_NullSurface_001
 * @tc.desc: Test CreateVirtualScreen with null surface creates empty surfaceConfigs
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, CreateVirtualScreen_NullSurface_001, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto id = screenManager_->CreateVirtualScreen(
        "vScreen", VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT, nullptr, INVALID_SCREEN_ID, 0, {});
    ASSERT_NE(INVALID_SCREEN_ID, id);

    auto screen = screenManager_->GetScreen(id);
    ASSERT_NE(screen, nullptr);
    EXPECT_TRUE(screen->GetMultiSurfaceConfigs().empty());
...
/*
 * @tc.name: CreateVirtualScreen_DuplicateSurface_001
 * @tc.desc: Test CreateVirtualScreen with duplicate surface returns INVALID_SCREEN_ID
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, CreateVirtualScreen_DuplicateSurface_001, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto psurface = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface, nullptr);
    ...
    auto id2 = screenManager_->CreateVirtualScreen(
        "vScreen2", VIRTUAL_SCREEN_WIDTH, VIRTUAL_SCREEN_HEIGHT, psurface, INVALID_SCREEN_ID, 0, {});
    EXPECT_EQ(INVALID_SCREEN_ID, id2);
...
/*
 * @tc.name: CreateVirtualScreen_ValidSurface_001
 * @tc.desc: Test CreateVirtualScreen with valid surface creates successfully
 * @tc.type: FUNC
 */
HWTEST_F(RSScreenManagerTest, CreateVirtualScreen_ValidSurface_001, TestSize.Level1)
{
    ASSERT_NE(nullptr, screenManager_);
    auto psurface = CreateTestSurfaceForMultiSurface();
    ASSERT_NE(psurface, nullptr);

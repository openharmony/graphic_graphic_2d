...
class MockRSLayer : public RSLayer {
public:
    explicit MockRSLayer(RSLayerId id, bool usingFlag, const std::string& name = "L")
        : id_(id), isNeedComposition_(usingFlag), surfaceName_(name) {}
    ~MockRSLayer() override = default;
...
{
    std::shared_ptr<RSRenderComposerContext> context = std::make_shared<RSRenderComposerContext>();
    std::vector<std::shared_ptr<RSLayer>> empty;
    context->SetRSLayersVec(std::move(empty));
    EXPECT_TRUE(context->GetRSLayersVec().empty());
}
...
HWTEST_F(RSRenderComposerContextTest, SetRSLayersVec_Override, TestSize.Level1)
{
    ...
    layersA.push_back(std::shared_ptr<RSLayer>(std::make_shared<MockRSLayer>(1, true)));
    context->SetRSLayersVec(std::move(layersA));
    ASSERT_EQ(context->GetRSLayersVec().size(), 1u);

    std::vector<std::shared_ptr<RSLayer>> layersB;
    layersB.push_back(std::shared_ptr<RSLayer>(std::make_shared<MockRSLayer>(2, true)));
    layersB.push_back(std::shared_ptr<RSLayer>(std::make_shared<MockRSLayer>(3, true)));
    context->SetRSLayersVec(std::move(layersB));
    auto vec = context->GetRSLayersVec();
...
HWTEST_F(RSRenderComposerContextTest, GetRSLayersVec_CopySemantics, TestSize.Level1)
{
    ...
    layers.push_back(std::shared_ptr<RSLayer>(std::make_shared<MockRSLayer>(1, true)));
    layers.push_back(std::shared_ptr<RSLayer>(std::make_shared<MockRSLayer>(2, true)));
    context->SetRSLayersVec(std::move(layers));

    auto vecA = context->GetRSLayersVec();
    ASSERT_EQ(vecA.size(), 2u);
    // mutate the returned copy
    vecA.pop_back();

    // internal storage should remain unchanged
    auto vecB = context->GetRSLayersVec();
...
HWTEST_F(RSRenderComposerContextTest, ClearAllRSLayers_EmptiesVector, TestSize.Level1)
{
    ...
    layers.push_back(std::shared_ptr<RSLayer>(std::make_shared<MockRSLayer>(2, true)));
    context->SetRSLayersVec(std::move(layers));
    ASSERT_EQ(context->GetRSLayersVec().size(), 2u);
...
HWTEST_F(RSRenderComposerContextTest, SetAndGet_ReturnsSameVector, TestSize.Level1)
{
    std::shared_ptr<RSRenderComposerContext> context = std::make_shared<RSRenderComposerContext>();
    std::vector<std::shared_ptr<RSLayer>> layers;
    std::shared_ptr<RSLayer> l1 = std::make_shared<MockRSLayer>(1, false, "L1");
    std::shared_ptr<RSLayer> l2 = std::make_shared<MockRSLayer>(2, true, "L2");
    layers.push_back(l1);
    layers.push_back(l2);
    context->SetRSLayersVec(std::move(layers));

...
#include "setancoforcedodirect_fuzzer.h"
...
namespace {
const uint8_t DO_SET_ANCO_FORCE_DO_DIRECT = 0;
const uint8_t TARGET_SIZE = 1;
constexpr uint32_t STR_LEN = 10;

void DoSetAncoForceDoDirect(FuzzedDataProvider& fdp)
{
    bool direct = fdp.ConsumeBool();
    g_rsInterfaces->SetAncoForceDoDirect(direct);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    switch (tarPos) {
        case OHOS::Rosen::DO_SET_ANCO_FORCE_DO_DIRECT:
            OHOS::Rosen::DoSetAncoForceDoDirect(fdp);
            break;
        default:
            return -1;
    }

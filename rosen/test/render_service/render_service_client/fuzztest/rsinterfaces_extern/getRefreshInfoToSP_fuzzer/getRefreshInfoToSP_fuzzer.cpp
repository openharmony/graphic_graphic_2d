...
#include "getRefreshInfoToSP_fuzzer.h"
...
namespace {
const uint8_t DO_GET_REFRESH_INFO_TO_SP = 0;
const uint8_t TARGET_SIZE = 1;

void DoGetRefreshInfoToSP(FuzzedDataProvider& fdp)
{
    NodeId id = fdp.ConsumeIntegral<uint64_t>();
    g_rsInterfaces->GetRefreshInfoToSP(id);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    switch (tarPos) {
        case OHOS::Rosen::DO_GET_REFRESH_INFO_TO_SP:
            OHOS::Rosen::DoGetRefreshInfoToSP(fdp);
            break;
        default:
            return -1;
    }

...
#include "setvmecachestatus_fuzzer.h"
...
namespace {
const uint8_t DO_SET_VMA_CACHE_STATUS = 0;
const uint8_t TARGET_SIZE = 1;
constexpr uint32_t STR_LEN = 10;

void DoSetVmaCacheStatus(FuzzedDataProvider& fdp)
{
    bool flag = fdp.ConsumeBool();

    g_rsInterfaces->SetVmaCacheStatus(flag);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    switch (tarPos) {
        case OHOS::Rosen::DO_SET_VMA_CACHE_STATUS:
            OHOS::Rosen::DoSetVmaCacheStatus(fdp);
            break;
        default:
            return -1;
    }

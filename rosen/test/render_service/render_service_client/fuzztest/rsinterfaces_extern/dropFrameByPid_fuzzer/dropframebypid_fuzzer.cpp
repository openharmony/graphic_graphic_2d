...
#include "dropframebypid_fuzzer.h"
...
namespace {
const uint8_t DO_DROP_FRAME_BY_PID = 0;
const uint8_t TARGET_SIZE = 1;
constexpr uint32_t STR_LEN = 10;

void DoDropFrameByPid(FuzzedDataProvider& fdp)
{
    std::vector<int32_t> pidList;
    uint8_t listSize = fdp.ConsumeIntegral<uint8_t>();
    for (uint8_t i = 0; i < listSize; i++) {
        pidList.push_back(fdp.ConsumeIntegral<int32_t>());
    }
    int32_t dropFrameLevel = fdp.ConsumeIntegral<int32_t>();

    g_rsInterfaces->DropFrameByPid(pidList, dropFrameLevel);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    switch (tarPos) {
        case OHOS::Rosen::DO_DROP_FRAME_BY_PID:
            OHOS::Rosen::DoDropFrameByPid(fdp);
            break;
        default:
            return -1;
    }

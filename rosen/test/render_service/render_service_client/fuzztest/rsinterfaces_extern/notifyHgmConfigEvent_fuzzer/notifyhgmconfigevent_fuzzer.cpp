...
#include "notifyhgmconfigevent_fuzzer.h"
...
namespace {
const uint8_t DO_NOTIFY_HGMCONFIG_EVENT = 0;
const uint8_t TARGET_SIZE = 1;
constexpr uint32_t STR_LEN = 10;

void DoNotifyHgmConfigEvent(FuzzedDataProvider& fdp)
{
    std::string eventName = fdp.ConsumeRandomLengthString(STR_LEN);
    bool state = fdp.ConsumeBool();

    g_rsInterfaces->NotifyHgmConfigEvent(eventName, state);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    switch (tarPos) {
        case OHOS::Rosen::DO_NOTIFY_HGMCONFIG_EVENT:
            OHOS::Rosen::DoNotifyHgmConfigEvent(fdp);
            break;
        default:
            return -1;
    }

...
#include "firstframecommit_fuzzer.h"
...
namespace {
const uint8_t DO_REGISTER_FIRST_FRAME_COMMIT_CALLBACK = 0;
const uint8_t TARGET_SIZE = 1;

void DoRegisterFirstFrameCommitCallback(FuzzedDataProvider& fdp)
{
    ...
    if (!shouldRegister) {
        ...
        g_rsInterfaces->RegisterFirstFrameCommitCallback(callback);
    } else {
        // Use lambda for callback
        FirstFrameCommitCallback callback = [](uint64_t nodeId, int64_t timestamp) {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    switch (tarPos) {
        case OHOS::Rosen::DO_REGISTER_FIRST_FRAME_COMMIT_CALLBACK:
            OHOS::Rosen::DoRegisterFirstFrameCommitCallback(fdp);
            break;
        default:
            return -1;
    }

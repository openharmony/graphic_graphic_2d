...
#include "getRefreshInfo_fuzzer.h"
...
namespace {
const uint8_t DO_GET_REFRESH_INFO = 0;
const uint8_t TARGET_SIZE = 1;

void DoGetRefreshInfo(FuzzedDataProvider& fdp)
{
    pid_t pid = static_cast<pid_t>(fdp.ConsumeIntegral<int32_t>());
    g_rsInterfaces->GetRefreshInfo(pid);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    switch (tarPos) {
        case OHOS::Rosen::DO_GET_REFRESH_INFO:
            OHOS::Rosen::DoGetRefreshInfo(fdp);
            break;
        default:
            return -1;
    }

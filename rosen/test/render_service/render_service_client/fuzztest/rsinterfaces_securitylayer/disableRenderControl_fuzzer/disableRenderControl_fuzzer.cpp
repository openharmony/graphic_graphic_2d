...
#include "disableRenderControl_fuzzer.h"
...
namespace {
const uint8_t DO_DISABLE_RENDER_CONTROL_SCREEN = 0;
const uint8_t TARGET_SIZE = 1;

void DoDisableRenderControlScreen(FuzzedDataProvider& fdp)
{
    ScreenId id = fdp.ConsumeIntegral<uint64_t>();
    g_rsInterfaces->DisablePowerOffRenderControl(id);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    switch (tarPos) {
        case OHOS::Rosen::DO_DISABLE_RENDER_CONTROL_SCREEN:
            OHOS::Rosen::DoDisableRenderControlScreen(fdp);
            break;
        default:
            return -1;
    }

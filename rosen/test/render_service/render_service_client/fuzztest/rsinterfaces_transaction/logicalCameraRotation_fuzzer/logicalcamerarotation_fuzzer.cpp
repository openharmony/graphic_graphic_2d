...
#include "logicalcamerarotation_fuzzer.h"
...
namespace {
const uint8_t DO_SET_LOGICAL_CAMERA_ROTATION_CORRECTION = 0;
const uint8_t TARGET_SIZE = 1;

void DoSetLogicalCameraRotationCorrection(FuzzedDataProvider& fdp)
{
    ScreenId id = fdp.ConsumeIntegral<uint64_t>();
    ScreenRotation logicalCorrection = static_cast<ScreenRotation>(fdp.ConsumeIntegral<uint32_t>());
    g_rsInterfaces->SetLogicalCameraRotationCorrection(id, logicalCorrection);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    switch (tarPos) {
        case OHOS::Rosen::DO_SET_LOGICAL_CAMERA_ROTATION_CORRECTION:
            OHOS::Rosen::DoSetLogicalCameraRotationCorrection(fdp);
            break;
        default:
            return -1;
    }

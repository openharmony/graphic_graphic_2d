...
namespace {
const uint8_t DO_SET_LAYER_TOP_FOR_HARDWARE_COMPOSER = 0;
const uint8_t TARGET_SIZE = 1;

void DoSetLayerTopForHardwareComposer(FuzzedDataProvider& fdp)
{
    NodeId nodeId = fdp.ConsumeIntegral<uint64_t>();
    bool isTop = fdp.ConsumeBool();
    uint32_t zOrder = fdp.ConsumeIntegral<uint32_t>();
    g_rsInterfaces->SetLayerTopForHWC(nodeId, isTop, zOrder);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    switch (tarPos) {
        case OHOS::Rosen::DO_SET_LAYER_TOP_FOR_HARDWARE_COMPOSER:
            OHOS::Rosen::DoSetLayerTopForHardwareComposer(fdp);
            break;
        default:
            return -1;
    }

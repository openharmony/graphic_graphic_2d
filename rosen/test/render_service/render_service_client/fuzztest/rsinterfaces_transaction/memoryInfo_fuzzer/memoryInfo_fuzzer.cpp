...
void DoGetMemoryGraphic(FuzzedDataProvider& fdp)
{
    int pid = fdp.ConsumeIntegral<int>();
    g_rsInterfaces->GetMemoryGraphic(pid);
}

void DoGetTotalAppMemSize(FuzzedDataProvider& fdp)
{
    float cpuMemSize = fdp.ConsumeFloatingPoint<float>();
    float gpuMemSize = fdp.ConsumeFloatingPoint<float>();
    g_rsInterfaces->GetTotalAppMemSize(cpuMemSize, gpuMemSize);
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    switch (tarPos) {
        case OHOS::Rosen::DO_GET_MEMORY_GRAPHIC:
            OHOS::Rosen::DoGetMemoryGraphic(fdp);
            break;
        case OHOS::Rosen::DO_GET_TOTAL_APP_MEM_SIZE:
            OHOS::Rosen::DoGetTotalAppMemSize(fdp);
            break;
        default:
            return -1;
    }

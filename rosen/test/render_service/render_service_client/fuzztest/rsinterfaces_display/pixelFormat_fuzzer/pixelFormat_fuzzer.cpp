...
namespace {
const uint8_t DO_GET_PIXEL_FORMAT = 0;
const uint8_t DO_SET_PIXEL_FORMAT = 1;
...
void DoGetPixelFormat(FuzzedDataProvider& fdp)
{
    ...
    g_rsInterfaces->GetPixelFormat(id, pixelFormat);
}

void DoSetPixelFormat(FuzzedDataProvider& fdp)
{
    ScreenId id = fdp.ConsumeIntegral<uint64_t>();
    GraphicPixelFormat format = static_cast<GraphicPixelFormat>(fdp.ConsumeIntegral<uint32_t>());
    g_rsInterfaces->SetPixelFormat(id, format);
}

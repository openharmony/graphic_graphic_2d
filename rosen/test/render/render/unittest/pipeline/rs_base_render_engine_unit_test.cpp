 ...
 */
HWTEST(RsBaseRenderEngineTest, NeedForceCPU01, TestSize.Level1)
{
 ...
 */
HWTEST(RsBaseRenderEngineTest, SetHighContrast01, TestSize.Level1)
{
 ...
 * @tc.require: issueI6QM6E
 */
HWTEST(RsBaseRenderEngineTest, NeedForceCPU02, TestSize.Level1)
{
    auto node = RSTestUtil::CreateSurfaceNodeWithBuffer();
    auto buffer = node->GetRSSurfaceHandler()->GetBuffer();
 ...
 * @tc.require: issueI6QM6E
 */
HWTEST(RsBaseRenderEngineTest, DrawDisplayNodeWithParams01, TestSize.Level1)
{
    NodeId id = 0;
 ...
 * @tc.require: issueI6QM6E
 */
HWTEST(RsBaseRenderEngineTest, CheckIsHdrSurfaceBuffer01, TestSize.Level1)
{
    auto renderEngine = std::make_shared<RSRenderEngine>();
    auto node = RSTestUtil::CreateSurfaceNodeWithBuffer();
 ...
 * @tc.require: issueI6QM6E
 */
HWTEST(RsBaseRenderEngineTest, CreateEglImageFromBuffer01, TestSize.Level1)
{
    if (!RSSystemProperties::IsUseVulkan()) {
        auto renderEngine = std::make_shared<RSRenderEngine>();
        renderEngine->Init();
...
HWTEST(RsBaseRenderEngineTest, DrawImageRect, TestSize.Level1)
{
 ...
 * @tc.require: issueI6QM6E
 */
HWTEST(RsBaseRenderEngineTest, RegisterDeleteBufferListener01, TestSize.Level1)
{
    auto renderEngine = std::make_shared<RSRenderEngine>();
 ...
 * @tc.require:
 */
HWTEST(RsBaseRenderEngineTest, ConvertDrawingColorSpaceToSpaceInfo, TestSize.Level1)
{
 ...
 * @tc.require: issueI6GJ1Z
 */
HWTEST(RsBaseRenderEngineTest, ConvertColorGamutToDrawingColorSpace, TestSize.Level1)
{
    std::shared_ptr<Drawing::ColorSpace> colorSpace;
 ...
 * @tc.require:
 */
HWTEST(RsBaseRenderEngineTest, GetCanvasColorSpace, TestSize.Level1)
{
    std::unique_ptr<Drawing::Canvas> drawingCanvas = std::make_unique<Drawing::Canvas>(10, 10);
    std::shared_ptr<RSPaintFilterCanvas> canvas = std::make_shared<RSPaintFilterCanvas>(drawingCanvas.get());
    ASSERT_NE(canvas, nullptr);

/*
 * Copyright (c) 2025 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#include "drawing_canvas_utils_fuzzer.h"

#include <cstddef>
#include <cstdint>

#include "get_object.h"

#include "drawing_bitmap.h"
#include "drawing_canvas.h"
#include "drawing_canvas_utils.h"
#include "pipeline/rs_recording_canvas.h"
#include "pixel_map.h"
#include "render/rs_pixel_map_util.h"
#include "draw/canvas.h"
#include "drawing_pixel_map.h"
#include "native_pixel_map_manager.h"
#include "image/pixelmap_native.h"
#include "pixelmap_native_impl.h"
#ifdef RS_ENABLE_GPU
#include "image_source.h"
#include "pixel_map.h"
#endif

namespace OHOS {
namespace Rosen {
namespace Drawing {
static unsigned int g_testJpgLength = 2431;
static unsigned char g_testJpgBuffer[] = {
    
};

void DrawCanvasUtilsFuzzerTest001(const uint8_t* data, size_t size)
{
    g_data = data;
    g_size = size;
    g_pos = 0;

    std::shared_ptr<Media::PixelMap> p = nullptr;
    OH_Drawing_PixelMap* pixelMap = OH_Drawing_PixelMapGetFromOhPixelMapNative(nullptr);
    if (pixelMap) {
        p = reinterpret_cast<OH_PixelmapNative*>(pixelMap)->GetInnerPixelmap();
    }
    const Drawing::Rect* center = new Rect(GetObject<float>(), GetObject<float>(),
        GetObject<float>(), GetObject<float>());
    const Drawing::Rect* dst = new Rect(GetObject<float>(), GetObject<float>(),
        GetObject<float>(), GetObject<float>());
    DrawingCanvasUtils::DrawPixelMapNine(nullptr, p, center, dst,
        GetObject<Drawing::FilterMode>());

    int32_t width = GetObject<int32_t>();
    int32_t height = GetObject<int32_t>();
    Canvas canvas = Canvas(width, height);
    DrawingCanvasUtils::DrawPixelMapNine(&canvas, p, center, dst,
        GetObject<Drawing::FilterMode>());
    DrawingCanvasUtils::DrawPixelMapNine(&canvas, p, nullptr, dst,
        GetObject<Drawing::FilterMode>());
    DrawingCanvasUtils::DrawPixelMapNine(&canvas, nullptr, center, dst,
        GetObject<Drawing::FilterMode>());
    DrawingCanvasUtils::DrawPixelMapNine(&canvas, nullptr, nullptr, dst,
        GetObject<Drawing::FilterMode>());
    Canvas canvas2 = Canvas(DrawingType::RECORDING);
    DrawingCanvasUtils::DrawPixelMapNine(&canvas2, p, center, dst,
        GetObject<Drawing::FilterMode>());
    if (center != nullptr) {
        delete center;
    }

    if (dst != nullptr) {
        delete dst;
    }
}

void DrawCanvasUtilsFuzzerTest002(const uint8_t* data, size_t size)
{
    g_data = data;
    g_size = size;
    g_pos = 0;

    std::shared_ptr<Media::PixelMap> p = nullptr;
    OH_Drawing_PixelMap* pixelMap = OH_Drawing_PixelMapGetFromOhPixelMapNative(nullptr);
    if (pixelMap) {
        p = reinterpret_cast<OH_PixelmapNative*>(pixelMap)->GetInnerPixelmap();
    }
    const Drawing::Rect* src = new Rect(GetObject<float>(), GetObject<float>(),
        GetObject<float>(), GetObject<float>());
    const Drawing::Rect* dst = new Rect(GetObject<float>(), GetObject<float>(),
        GetObject<float>(), GetObject<float>());

    DrawingCanvasUtils::DrawPixelMapRect(nullptr, p, src, dst,
        nullptr);

    int32_t width = GetObject<int32_t>();
    int32_t height = GetObject<int32_t>();
    Canvas canvas = Canvas(width, height);
    DrawingCanvasUtils::DrawPixelMapRect(&canvas, p, src, dst,
        nullptr);
    DrawingCanvasUtils::DrawPixelMapRect(&canvas, p, nullptr, dst,
        nullptr);
    DrawingCanvasUtils::DrawPixelMapRect(&canvas, nullptr, src, dst,
        nullptr);
    DrawingCanvasUtils::DrawPixelMapRect(&canvas, nullptr, nullptr, dst,
        nullptr);
    Canvas canvas2 = Canvas(DrawingType::RECORDING);
    DrawingCanvasUtils::DrawPixelMapRect(&canvas2, p, src, dst,
        nullptr);
    if (src != nullptr) {
        delete src;
    }

    if (dst != nullptr) {
        delete dst;
    }
}

void DrawCanvasUtilsFuzzerTest003(const uint8_t* data, size_t size)
{
    g_data = data;
    g_size = size;
    g_pos = 0;
    int32_t width = GetObject<int32_t>();
    int32_t height = GetObject<int32_t>();
    Drawing::Canvas canvas = Canvas(width, height);
    DrawingCanvasUtils::DrawRecordCmd(&canvas, nullptr, GetObject<bool>());

    Drawing::Canvas canvas2 = Canvas(DrawingType::RECORDING);
    DrawingCanvasUtils::DrawRecordCmd(&canvas2, nullptr, GetObject<bool>());
}

void DrawCanvasUtilsFuzzerTest004(const uint8_t* data, size_t size)
{
    g_data = data;
    g_size = size;
    g_pos = 0;

    Drawing::Bitmap bitmap;
    DrawingCanvasUtils::ExtractDrawingBitmap(nullptr, bitmap);

    size_t bufferSize = g_testJpgLength;
    uint8_t *buffer = g_testJpgBuffer;
    uint32_t errorCode = 0;
    Media::SourceOptions opts;
    opts.formatHint = "image/jpeg";
    std::unique_ptr<Media::ImageSource> imageSource = Media::ImageSource::CreateImageSource(buffer,
        bufferSize, opts, errorCode);
    Media::DecodeOptions decodeOpts;
    decodeOpts.desiredPixelFormat = GetObject<Media::PixelFormat>();
    std::shared_ptr<Media::PixelMap> pixelMap = imageSource->CreatePixelMap(decodeOpts, errorCode);

    Drawing::Bitmap bitmap1;
    DrawingCanvasUtils::ExtractDrawingBitmap(pixelMap, bitmap1);
}

} // namespace Drawing
} // namespace Rosen
} // namespace OHOS

/* Fuzzer entry point */
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    /* Run your code on data */
    OHOS::Rosen::Drawing::DrawCanvasUtilsFuzzerTest001(data, size);
    OHOS::Rosen::Drawing::DrawCanvasUtilsFuzzerTest002(data, size);
    OHOS::Rosen::Drawing::DrawCanvasUtilsFuzzerTest003(data, size);
    OHOS::Rosen::Drawing::DrawCanvasUtilsFuzzerTest004(data, size);
    return 0;
}

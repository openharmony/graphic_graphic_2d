...
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 1)
{
...
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 2)
{
...
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 3)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 4)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 5)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 6)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 7)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：不透明
// 视效：CreateLinearToSrgbGamma
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 8)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// Pixmap_Scene_0159
// Pixmap构造接口：Pixmap_ImageInfo_13
// Pixmap操作接口：GetWidth
// cilp：ClipRoundRect(G2)
// transform：Shear
// 抗锯齿：AA
// 透明度：不透明
// 视效：CreateBlendShader
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 9)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）

    // 2.Pixmap构造接口，Pixmap_ImageInfo_13
    auto colorspace = Drawing::ColorSpace::CreateSRGB();
    ImageInfo imageInfo(512, 512, Drawing::ColorType::COLORTYPE_BGRA_8888, Drawing::AlphaType::ALPHATYPE_UNKNOWN,
        colorspace); // 512 width * 512 height
    Pixmap pixmap(imageInfo, nullptr, imageInfo.GetBytesPerPixel() * imageInfo.GetWidth());
...
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 10)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 11)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
    ...
    for (auto& blurType : blurTypes) {
        ...
        bitmap1.SetPixels(const_cast<void*>(pixel));
        playbackCanvas_->AttachBrush(brush);
        if (pixmap.GetColorType() == Drawing::ColorType::COLORTYPE_BGRA_8888) {
            playbackCanvas_->DrawRect(Drawing::Rect(800 + (rectPos % VARIATION) * LEFT,
                800 + (rectPos / VARIATION) * LEFT, 1000 + (rectPos % VARIATION) * LEFT,
                1000 + (rectPos / VARIATION) * LEFT)); // rect region(800, 800, 1000, 1000)
        } else {
...
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 12)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 13)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：半透明
// 视效：CreateBlurMaskFilter
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 14)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：半透明
// 视效：CreateBlendModeColorFilter
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 15)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：不透明
// 视效：CreateBlurImageFilter
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 16)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：不透明
// 视效：CreateMatrixColorFilter
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 17)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 18)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：透明
// 视效：CreateLinearToSrgbGamma
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 19)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：半透明
// 视效：CreateComposeColorFilter
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 20)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：半透明
// 视效：null
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 21)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：不透明
// 视效：CreateColorShader
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 22)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：透明
// 视效：CreateLumaColorFilter
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 23)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）

    // 2.Pixmap构造接口，Pixmap_ImageInfo_14
    auto colorspace = Drawing::ColorSpace::CreateSRGB();
...
// 透明度：不透明
// 视效：CreateBlurImageFilter
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 24)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：透明
// 视效：CreateBlendModeColorFilter
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 25)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：半透明
// 视效：CreateBlendImageFilter
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 26)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：半透明
// 视效：CreateComposeColorFilter
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 27)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
    ...
    bitmap1.SetPixels(const_cast<void*>(pixel));
    playbackCanvas_->AttachBrush(brush);
    if (pixmap.GetColorType() == Drawing::ColorType::COLORTYPE_RGBA_F16) {
        playbackCanvas_->DrawRect(Drawing::Rect(800, 800, 1000, 1000)); // rect region(800, 800, 1000, 1000)
    } else {
        playbackCanvas_->DrawRect(Drawing::Rect(1000, 1000, 1200, 1200)); // rect region(1000, 1000, 1200, 1200)
    }
...
// 透明度：不透明
// 视效：null
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 28)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：透明
// 视效：CreateMatrixColorFilter
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 29)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：AA
    Drawing::Brush brush;
    brush.SetAntiAlias(true); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）
...
// 透明度：半透明
// 视效：CreateSrgbGammaToLinear
DEF_DTK(DTK_Pixmap_Scene6, TestLevel::L2, 30)
{
    // 1.创建笔刷，设置笔刷是否为抗锯齿效果，抗锯齿：非AA
    Drawing::Brush brush;
    brush.SetAntiAlias(false); // 设置笔刷抗锯齿，true为AA（抗锯齿），false为非AA（不抗锯齿）

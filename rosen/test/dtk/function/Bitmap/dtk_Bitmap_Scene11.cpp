...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 1)
{
    /*
    ...
    bitmap0.ClearWithColor(0xFFCDEF78); // 不透明效果，淡绿色
    Image image;
    ...
    bitmap.ClearWithColor(0xFF789645); // 不透明效果，橄榄色
    ...
    bitmap1.Build(bitmap.GetWidth(), 500, format1); // 500 height
    bitmap1.ClearWithColor(0x894567FA);             // 半透明效果，灰绿色
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 2)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
    ...
    Drawing::Bitmap bitmap1;
    Drawing::BitmapFormat format1 { Drawing::ColorType::COLORTYPE_RGBA_8888, Drawing::AlphaType::ALPHATYPE_PREMUL };
    bitmap1.Build(500, bitmap.GetHeight(), format1); // 500 width
    bitmap1.ClearWithColor(0xFF4567FA);              // 不透明效果，深蓝色
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 3)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
    ...
    bitmap1.ClearWithColor(0x004567FA); // 透明效果，深蓝色
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 4)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 5)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
    ...
    if (bitmap1.GetRowBytes() == sizeof(uint32_t) * bitmap1.GetWidth()) {
        playbackCanvas_->DrawBackground(0x00675800); // 透明效果
    }
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 6)
{
    // 绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 7)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 8)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 9)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 10)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
    ...
    bitmap1.Build(500, 500, format1); // 500 weidth & 500 height
    if (bitmap1.IsEmpty()) {
        playbackCanvas_->DrawBackground(0xFF675800); // 不透明效果，深橄榄色
    }
...
// 透明度：透明
// 视效：null
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 11)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
    ...
    bitmap1.Build(500, 500, format1); // 500 weidth & 500 height
    if (bitmap1.IsValid()) {
        playbackCanvas_->DrawBackground(0x00675800); // 透明效果，橄榄色
    }
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 12)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 13)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
...
// 透明度：透明
// 视效：CreateBlendModeColorFilter
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 14)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
    ...
    for (auto blendMode : blendModes) {
        ...
        playbackCanvas_->AttachBrush(brush);
        playbackCanvas_->DrawBitmap(bitmap, 200, 200);                    // 200，200 is bitmap position
        playbackCanvas_->DrawImage(*image1, 250, 300, SamplingOptions()); // 250，300 is image position
    }
    playbackCanvas_->DetachBrush();
...
// 透明度：半透明
// 视效：CreateLinearGradient
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 15)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
    ...
    Drawing::BitmapFormat format1 { Drawing::ColorType::COLORTYPE_RGBA_8888, Drawing::AlphaType::ALPHATYPE_PREMUL };
    bitmap1.Build(500, 500, format1); // 500 weidth & 500 height
    bitmap1.ClearWithColor(0x894567FA); // 半透明效果，深蓝色
...
// 透明度：不透明
// 视效：CreateColorShader
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 16)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
...
// 透明度：透明
// 视效：CreateBlurImageFilter
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 17)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
...
// 透明度：半透明
// 视效：CreateBlendModeColorFilter
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 18)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
    ...
    if (bitmap1.ExtractSubset(bitmap, Rect(0, 1, 2, 2))) {
        playbackCanvas_->DrawBackground(0x89675800); // 半透明效果，深橄榄色
    }
...
// 透明度：不透明
// 视效：CreateComposeColorFilter
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 19)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
    ...
    Drawing::BitmapFormat format1 { Drawing::ColorType::COLORTYPE_RGBA_8888, Drawing::AlphaType::ALPHATYPE_PREMUL };
    bitmap1.Build(500, 500, format1);   // 500 weidth & 500 height
    bitmap1.ClearWithColor(0xFF789645); // 不透明效果，橄榄色
...
// 透明度：透明
// 视效：CreateMatrixColorFilter
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 20)
{
    /*
     *绘制流程说明：默认绘制两个Bitmap，即2、3两步，透明度效果只在第3步设置
     */
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 24)
{
    ...
    // 2.创建Bitmap Bitmap_Build_L1_16
    auto colorspace =
        Drawing::ColorSpace::CreateRGB(CMSTransferFuncType::DOT2, CMSMatrixType::DCIP3); // 511 width & 844 height
    auto imageInfo = ImageInfo(511, 844, COLORTYPE_ALPHA_8, ALPHATYPE_PREMUL, colorspace);
    Drawing::Bitmap bitmap;
    bitmap.Build(imageInfo);
    bitmap.ClearWithColor(0xFF789645); // 灰绿色
    ...
    // 由最后一步填充的颜色值的前两位控制，00为透明效果，89为半透明，FF为不透明
    Drawing::Bitmap bitmap1;
    auto imageInfo1 = ImageInfo(bitmap.GetWidth(), 844, COLORTYPE_ALPHA_8, ALPHATYPE_PREMUL, colorspace);
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 25)
{
    ...
    // 由最后一步填充的颜色值的前两位控制，00为透明效果，89为半透明，FF为不透明
    Drawing::Bitmap bitmap1;
    auto imageInfo1 = ImageInfo(511, bitmap.GetHeight(), COLORTYPE_ALPHA_8, ALPHATYPE_PREMUL, colorspace);
    bitmap1.Build(imageInfo1);
    bitmap1.ClearWithColor(0x894567FA); // 半透明效果，粉蓝色
...
DEF_DTK(Bitmap_Scene11, TestLevel::L2, 26)
{
    ...
    // 由最后一步填充的颜色值的前两位控制，00为透明效果，89为半透明，FF为不透明
    Drawing::Bitmap bitmap1;
    auto imageInfo1 = ImageInfo(511, 844, bitmap.GetColorType(), ALPHATYPE_PREMUL, colorspace);
    bitmap1.Build(imageInfo1);

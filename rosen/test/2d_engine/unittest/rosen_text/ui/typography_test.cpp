...
HWTEST_F(OHDrawingUITypographyTest, OH_Drawing_UI_TypographyTest001, TestSize.Level1)
{
    ...
    EXPECT_EQ(result.empty(), true);
    result =
        typography->GetTextRectsByBoundary(0, 0, RectHeightStyle::COVER_HALF_TOP_AND_BOTTOM, RectWidthStyle::TIGHT);
    EXPECT_EQ(result.empty(), true);
    result = typography->GetTextRectsByBoundary(0, 0, RectHeightStyle::COVER_TOP, RectWidthStyle::TIGHT);
    EXPECT_EQ(result.empty(), true);
    result = typography->GetTextRectsByBoundary(0, 0, RectHeightStyle::COVER_BOTTOM, RectWidthStyle::MAX);
    EXPECT_EQ(result.empty(), true);
    result = typography->GetTextRectsByBoundary(0, 0, RectHeightStyle::FOLLOW_BY_STRUT, RectWidthStyle::MAX);
    EXPECT_EQ(result.empty(), true);

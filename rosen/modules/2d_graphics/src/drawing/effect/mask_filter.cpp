...
namespace Rosen {
namespace Drawing {
MaskFilter::MaskFilter(FilterType t, BlurType blurType, scalar sigma, bool respectCTM,
    bool disableSDF) noexcept : MaskFilter()
{
    type_ = t;
    impl_->InitWithBlur(blurType, sigma, respectCTM, disableSDF);
}
...
std::shared_ptr<MaskFilter> MaskFilter::CreateBlurMaskFilter(BlurType blurType, scalar sigma, bool respectCTM,
    bool disableSDF)
{
    return std::make_shared<MaskFilter>(MaskFilter::FilterType::BLUR, blurType, sigma, respectCTM, disableSDF);
}

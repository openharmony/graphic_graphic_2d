...
const std::vector<ResetPropertyFunc> g_propertyResetterLUT = {
    [](RSProperties* prop) {},                                               // BOUNDS,
    [](RSProperties* prop) {},                                               // FRAME,
    ...
    [](RSProperties* prop) { prop->SetForegroundColor(RSColor()); },         // FOREGROUND_COLOR,
    [](RSProperties* prop) { prop->SetBackgroundColor(RSColor()); },         // BACKGROUND_COLOR,
    [](RSProperties* prop) { prop->SetBackgroundShader(nullptr); },          // BACKGROUND_SHADER,
    [](RSProperties* prop) { prop->SetBgImage(nullptr); },                   // BG_IMAGE,
    ...
    [](RSProperties* prop) { prop->SetFilter(nullptr); },                    // FILTER,
    [](RSProperties* prop) { prop->SetBackgroundFilter(nullptr); },          // BACKGROUND_FILTER,
    [](RSProperties* prop) { prop->SetLinearGradientBlurPara(nullptr); },    // LINEAR_GRADIENT_BLUR_PARA,
    [](RSProperties* prop) { prop->SetDynamicLightUpRate(std::nullopt); },   // DYNAMIC_LIGHT_UP_RATE,
    [](RSProperties* prop) { prop->SetDynamicLightUpDegree(std::nullopt); }, // DYNAMIC_LIGHT_UP_DEGREE,
    [](RSProperties* prop) { prop->SetFrameGravity(Gravity::DEFAULT); },     // FRAME_GRAVITY,
    [](RSProperties* prop) { prop->SetClipRRect(RRect()); },                 // CLIP_RRECT,
    [](RSProperties* prop) { prop->SetClipBounds(nullptr); },                // CLIP_BOUNDS,
    ...
    [](RSProperties* prop) { prop->SetShadowPath(nullptr); },                // SHADOW_PATH,
    [](RSProperties* prop) { prop->SetShadowMask(false); },                  // SHADOW_MASK,
    [](RSProperties* prop) { prop->SetMask(nullptr); },                      // MASK,
    [](RSProperties* prop) { prop->SetSpherize(0.f); },                      // SPHERIZE,
    [](RSProperties* prop) { prop->SetLightUpEffect(1.f); },                 // LIGHT_UP_EFFECT,
    [](RSProperties* prop) { prop->SetPixelStretch(std::nullopt); },         // PIXEL_STRETCH,
    [](RSProperties* prop) { prop->SetPixelStretchPercent(std::nullopt); },  // PIXEL_STRETCH_PERCENT,
    [](RSProperties* prop) { prop->SetGrayScale(std::nullopt); },            // GRAY_SCALE,
    [](RSProperties* prop) { prop->SetBrightness(std::nullopt); },           // BRIGHTNESS,
    [](RSProperties* prop) { prop->SetContrast(std::nullopt); },             // CONTRAST,
    [](RSProperties* prop) { prop->SetSaturate(std::nullopt); },             // SATURATE,
    [](RSProperties* prop) { prop->SetSepia(std::nullopt); },                // SEPIA,
    [](RSProperties* prop) { prop->SetInvert(std::nullopt); },               // INVERT,
    [](RSProperties* prop) { prop->SetHueRotate(std::nullopt); },            // HUE_ROTATE,
    [](RSProperties* prop) { prop->SetColorBlend(std::nullopt); },           // COLOR_BLEND,
...
void RSProperties::ResetProperty(const std::unordered_set<RSModifierType>& dirtyTypes)
{
    for (const auto& type : dirtyTypes) {
        if (type >= RSModifierType::MAX_TYPE) {
            continue;
        }
        g_propertyResetterLUT[static_cast<uint8_t>(type)](this);
    }
}

    ...
    blurRadiusY_(blurRadiusY)
{
    ...
    if (!blurEffect) {
        ROSEN_LOGE("RuntimeShader error: %s\n", error.c_str());
    }
    ...
    SkCanvas& canvas, const sk_sp<SkImage>& image, const SkRect& src, const SkRect& dst) const
{
    ...
    auto blurMatrix = SkMatrix::Scale(invBlurScale, invBlurScale);
    ...
    mixBuilder.child("originalInput") = input->makeShader(SkTileMode::kClamp, SkTileMode::kClamp, linear, inputMatrix);

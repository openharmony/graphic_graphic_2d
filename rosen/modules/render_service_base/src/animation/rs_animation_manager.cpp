...
std::thread::id RSAnimationManager::mainThreadId_ {};

void RSAnimationManager::Init()
{
...
RSAnimationManager::~RSAnimationManager()
{
    if (mainThreadId_ != std::thread::id {}) {
        if (auto currentTid = std::this_thread::get_id(); currentTid != mainThreadId_) {
            std::ostringstream oss;
            oss << "mainThreadId=" << mainThreadId_ << ", currentTid=" << currentTid;
            ROSEN_LOGE(

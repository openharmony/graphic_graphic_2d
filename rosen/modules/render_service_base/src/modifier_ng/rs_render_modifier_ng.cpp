...
void RSFilterRenderModifier::Draw(RSPaintFilterCanvas& canvas, Drawing::Rect& rect)
{
    if (needDrawBehindWindow_) {
        ...
        RSPropertyDrawableUtils::DrawBackgroundEffect(
            &canvas, filter_, cacheManager_, renderClearFilteredCacheAfterDrawing_, bounds, true);
        return;
    }
    ...
    if (filter_) {
        ...
        RSPropertyDrawableUtils::DrawFilter(
            &canvas, filter_, cacheManager_, IsForeground(), renderClearFilteredCacheAfterDrawing_);

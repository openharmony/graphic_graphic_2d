...
int32_t RSRenderServiceConnectionProxy::SetRenderModeChangeCallback(sptr<RSIRenderModeChangeCallback> callback)
{
    ...
    int32_t err = Remote()->SendRequest(RSIRenderServiceConnection::SET_RENDER_MODE_CHANGE_CALLBACK, data, reply, option);
    if (err != NO_ERROR) {
...
void RSRenderServiceConnectionProxy::UpdateRenderMode(bool isUniRender)
{
    ...
    option.SetFlags(MessageOption::TF_ASYNC);
    int32_t err = Remote()->SendRequest(RSIRenderServiceConnection::UPDATE_RENDER_TYPE, data, reply, option);
    if (err != NO_ERROR) {

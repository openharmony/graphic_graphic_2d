...
{
    return std::make_unique<RSVsyncClientDarwin>();
}

RSVsyncClientDarwin::~RSVsyncClientDarwin()
{
    running_ = false;
    if (vsyncThread_) {
        vsyncThread_->join();
...
void RSVsyncClientDarwin::RequestNextVsync()
{
    if (vsyncThread_ == nullptr) {
        running_ = true;
        auto func = std::bind(&RSVsyncClientDarwin::VsyncThreadMain, this);

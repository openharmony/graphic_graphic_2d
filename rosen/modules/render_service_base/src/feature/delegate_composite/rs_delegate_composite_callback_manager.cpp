...
{
    DFX_LOGD(g_enableDfx, "RsDelegateCompositeCleanCacheDfx: OnCleanCache in, infos size=%{public}zu", infos.size());
    ...
    for (auto& info : infos) {
        ...
        if (!info.buffer->RegisterBufferDestructorCallbackFunc(
            ...
            // without the callback the release info is never processed, skip it to avoid a stale record
            ROSEN_LOGE("DelegateModeDebugTag: register buffer destructor callback fail, bufferSeqnum=%{public}u",
                info.buffer->GetSeqNum());
            continue;
        }
...
{
    RS_TRACE_NAME("AddBufferReleaseInfo in");
    if (!buffer || !releaseFence || !cSurface) {
        RS_TRACE_NAME_FMT("AddBufferReleaseInfo fail: invalid input: %d %d %d",
    ...
    const uint64_t& srcId, const uint64_t& seqNum, const pid_t& pid, const pid_t& tid)
{
    std::lock_guard<std::mutex> lock(surfaceTransactionMapMutex_);
    {
        RS_OPTIONAL_TRACE_FMT("AddSurfaceTransactionCmdInfo: srcId=%llu, seqNum=%llu, size=%u",
            srcId, seqNum, surfaceTransactionCmdMap_[srcId].size());
    ...
    {
        RS_OPTIONAL_TRACE_FMT("AddWebProxyComposerCmdInfo size=%u, pid=%u, tid=%u",
    ...
    std::map<uint64_t, std::queue<uint64_t>>& surfaceTransactionCmdInfoMap)
{
    ...
    surfaceTransactionCmdMap_.swap(surfaceTransactionCmdInfoMap);
}
    ...
    std::unique_ptr<RSTransactionData>& transactionData)
{
    ...
    RS_TRACE_NAME_FMT("PrepareDelegateCompositeCommand");
    ...
    for (auto it = processPayload.begin(); it != processPayload.end();) {
        ...
        if (cmdQueue.size() >= MAX_MAP_SIZE) {
            ROSEN_LOGE("DelegateModeDebugTag: PrepareDelegateCompositeCommand fail, cmdQueue over size");
            break;
        }
    }

    {
        ...
        if (it == allPidCmdMap_.end()) {
            if (allPidCmdMap_.size() >= MAX_MAP_SIZE) {
                ROSEN_LOGE("DelegateModeDebugTag: PrepareDelegateCompositeCommand fail, allPidCmdMap_ over size");
                return;
            }
...
void RsDelegateCompositeCallbackManager::RemoveAllListenerbyPid(pid_t pid)
{
    RS_TRACE_NAME_FMT("RemoveAllListenerbyPid pid=%u", pid);
    {
        ...
        auto iter1 = surfaceTransactionIdentityInfoMap_.find(pid);
        if (iter1 != surfaceTransactionIdentityInfoMap_.end()) {
            RS_TRACE_NAME_FMT("find pid=%u in targetsCmdSenderMap", pid);
        ...
        if (iter2 != surfaceTransactionListenerMap_.end()) {
            RS_TRACE_NAME_FMT("find pid=%u in listenerMap", pid);
...
void RsDelegateCompositeCallbackManager::UnRegisterSurfaceNodeBufferReleaseListener(pid_t pid)
{
    ...
    RS_TRACE_NAME_FMT(
        "UnRegisterSurfaceNodeBufferReleaseListener(RsDelegateCompositeCallbackManager) success(pid=%d), size=%u",
        pid, surfaceNodeBufferReleaseCallbackMap_.size());
}
    ...
    pid_t pid, sptr<RSISurfaceNodeBufferReleaseCallback> listener)
{
    ...
    RS_TRACE_NAME_FMT(
        "RegisterSurfaceNodeBufferReleaseListener Listener(RsDelegateCompositeCallbackManager), pid=%d, size=%u",
        pid, surfaceNodeBufferReleaseCallbackMap_.size());
}

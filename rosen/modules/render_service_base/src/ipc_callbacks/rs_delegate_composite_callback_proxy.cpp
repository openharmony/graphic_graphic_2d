...
GSError RSWebProxyComposerCallbackProxy::OnComplete(uint64_t timestamp, uint64_t srcId, std::queue<uint64_t>& seqNums)
{
    ...
    RS_OPTIONAL_TRACE_NAME_FMT("RSWebProxyComposerCallbackProxy ON_COMPLETE:%llu, srcId=%llu, queueSize=%u",
        timestamp, srcId, size);

    while (!seqNums.empty()) {
        RS_OPTIONAL_TRACE_NAME_FMT("seqNums=%llu", seqNums.front());
...
GSError SurfaceNodeBufferReleaseCallbackProxy::OnBufferComplete(std::queue<OnCompletedRet> &queue)
{
    ...
    if (!data.WriteInterfaceToken(RSISurfaceNodeBufferReleaseCallback::GetDescriptor())) {
        RsDelegateCompositeCallbackProxyPrintLog("OnBufferComplete::WriteInterfaceToken fail");
        return GSERROR_BINDER;
    }
    if (!data.WriteUint32(size)) {
        RsDelegateCompositeCallbackProxyPrintLog("OnBufferComplete::Write size fail");
        return GSERROR_BINDER;
    }
    while (!queue.empty()) {

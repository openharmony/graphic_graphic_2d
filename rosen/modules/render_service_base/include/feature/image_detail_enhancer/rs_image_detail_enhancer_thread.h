...
public:
    ...
    void ImageSamplingDump(uint64_t imageId, uint64_t nodeId);
#endif
    ...
    void SetScaledImage(uint64_t imageId, uint64_t nodeId, const std::shared_ptr<Drawing::Image>& image);
    std::shared_ptr<Drawing::Image> GetScaledImage(uint64_t imageId);
    void ReleaseScaledImage(uint64_t imageId);
    void ReleaseScaledImageLocked(uint64_t imageId, uint64_t nodeId);
    void PushImageList(uint64_t imageId, uint64_t nodeId, const std::shared_ptr<Drawing::Image>& image);
    void SetProcessStatus(ImageKey imageKey, bool flag);
    bool GetProcessStatus(ImageKey imageKey) const;
    bool GetEnabled();
...
private:
    ...
    struct ImageKeyHash {
        size_t operator()(const ImageKey& key) const {
            size_t h1 = std::hash<uint64_t>{}(key.nodeId);
            size_t h2 = std::hash<uint64_t>{}(key.imageId);
            return h1 ^ (h2 + 0x9e3779b97f4a7c15 + (h1 << 6) + (h1 >> 2));
        }
    };

    void MarkScaledImageDirty(uint64_t nodeId);
    void ResetStatus(int srcWidth, int srcHeight, int dstWidth, int dstHeight, ImageKey imageId);
    void SetProcessReady(ImageKey imageKey, bool flag);
    bool GetProcessReady(ImageKey imageKey) const;
    ...
    std::list<std::pair<ImageKey, std::shared_ptr<Drawing::Image>>> imageList_ = {};
    std::unordered_map<ImageKey,
        std::list<std::pair<ImageKey, std::shared_ptr<Drawing::Image>>>::iterator, ImageKeyHash> keyMap_ = {};
    std::unordered_map<ImageKey, bool, ImageKeyHash> processStatusMap_ = {};
    std::unordered_map<ImageKey, bool, ImageKeyHash> processReadyMap_ = {};
    std::unordered_map<uint64_t, std::unordered_set<uint64_t>> imageNodeMap_ = {};

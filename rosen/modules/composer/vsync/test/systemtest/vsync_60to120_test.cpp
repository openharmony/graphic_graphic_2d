...
class VSync60To120Test : public testing::Test {
public:
...
int32_t VSync60To120Test::JudgeRefreshRate(int64_t period)
{
    if (period <= 0) {
        return 0;
    }
...
void VSync60To120Test::Process1()
{
#ifdef SUPPORT_ACCESS_TOKEN
    InitNativeTokenInfo();
    ...
    while (testTimes--) {
        ...
        generatorRefreshRate = 120; // 120hz
...
void VSync60To120Test::Process2()
{
    close(pipeFd[0]);
    close(pipe1Fd[1]);
    char buf[10];
...
HWTEST_F(VSync60To120Test, ChangeRefreshRateTest, Function | MediumTest | Level2)
{
    if (pipe(pipeFd) < 0) {
        exit(1);
    }

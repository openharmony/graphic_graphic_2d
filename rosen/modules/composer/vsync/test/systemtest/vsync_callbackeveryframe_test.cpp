...
class SetVsyncCallBackForEveryFrameTest : public testing::Test {
public:
    void Process1();
    void Process2();
...
void SetVsyncCallBackForEveryFrameTest::Process2()
{
    ...
    receiver->SetVsyncCallBackForEveryFrame(fcb, true);
    usleep(100000); // 100000us
    EXPECT_GT(g_onVsyncCount, 1);
    g_onVsyncCount = 0;
    receiver->SetVsyncCallBackForEveryFrame(fcb, true);
    usleep(100000); // 100000us
    receiver->SetVsyncCallBackForEveryFrame(fcb, false);
    usleep(100000); // 100000us
    EXPECT_GT(g_onVsyncCount, 1);
    g_onVsyncCount = 0;
    usleep(100000); // 100000us
    EXPECT_EQ(g_onVsyncCount, 0);
    int64_t period;
    int64_t timeStamp;
...
/*
* Function: SetVsyncCallBackForEveryFrameTest
* Type: Function
* Rank: Important(2)
* EnvConditions: N/A
* CaseDescription: SetVsyncCallBackForEveryFrameTest
 */
HWTEST_F(SetVsyncCallBackForEveryFrameTest, SetVsyncCallBackForEveryFrame_Test, Function | MediumTest | Level2)
{
    if (pipe(pipeFd) < 0) {
        exit(1);
    }

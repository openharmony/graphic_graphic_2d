...
namespace Rosen {
    ...
    class GEShader{
        public:
            GEShader(const std::string& name, ShaderType type, const std::string& shaderStr, const Drawing::RuntimeEffectOptions* opt = nullptr);
        ...
        private:
            ...
            static GE_Error CombineShaderString(std::string& cbStr, const std::string& shaderStrFirst, const std::string& shaderStrSecond);
    ...
    class GEShaderStore {
        public:
            ...
            // std::vector<std::shared_ptr<GEShader>> GetShader(const std::vector<uint32_t>& which);
        private:
            void Initialize();
            std::unordered_map<std::string, std::shared_ptr<GEShader>> shaderMap_ = {};
            // std::unordered_map<std::string, std::vector<std::shared_ptr<GEShader>>> shaderMap_ = {};
            std::unordered_map<ShaderIndex, std::shared_ptr<GEShader>> shaderObjMap_ = {};
    };

    #define REGISTER_SHADER(key, name, type, shader, opt)                     \
    {                                                                         \

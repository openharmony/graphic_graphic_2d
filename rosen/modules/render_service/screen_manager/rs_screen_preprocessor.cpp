...
void RSScreenPreprocessor::ReconnectProcess(std::shared_ptr<HdiOutput> output)
{
    if (output == nullptr) {
        RS_LOGE("%{public}s: output is nullptr", __func__);
        return;
    ...
    ScheduleTask([this, output, screenId]() {
        auto property = screenManager_.QueryScreenProperty(screenId);
        if (property == nullptr) {
            RS_LOGW("ReconnectProcess: screen %{public}" PRIu64 " no longer exists, skip reconnect", screenId);
            return;
        }
        RS_LOGW("ReconnectProcess: screen %{public}" PRIu64 " is reconnected due to process reconnected.", screenId);

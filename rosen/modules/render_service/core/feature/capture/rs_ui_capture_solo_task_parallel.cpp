    ...
    NodeId id, const RSSurfaceCaptureConfig& captureConfig)
{
    ...
    for (auto nodeId : nodeIdVec) {
        ...
        if (!soloPixelMap) {
            RS_LOGD("RSUiCaptureSoloTaskParallel::CaptureSoloNode nodeId:[%{public}" PRIu64 "], "
    ...
    for (auto pixPair : pixelMapIdPairVector) {
        RS_LOGD("RSUiCaptureSoloTaskParallel::CaptureSoloNode FinalCheck nodeId:[%{public}" PRIu64 "], ",
            pixPair.first);
        auto pixmap = pixPair.second;
        if (pixmap) {
            RS_LOGD("RSUiCaptureSoloTaskParallel::CaptureSoloNode FinalCheck width: [%{public}d], "
                "height: [%{public}d]", pixmap->GetWidth(), pixmap->GetHeight());
        } else {
            RS_LOGD("RSUiCaptureSoloTaskParallel::CaptureSoloNode FinalCheck pixmap is nullptr");
        }
    }
    return pixelMapIdPairVector;
}
    ...
    const RSSurfaceCaptureConfig& captureConfig)
{
    ...
    std::function<void()> captureTask = [captureHandle]() -> void {
        ...
        if (!captureHandle->Run()) {
            RS_LOGD("RSUiCaptureSoloTaskParallel::CaptureSoloNodePixelMap Run failed");

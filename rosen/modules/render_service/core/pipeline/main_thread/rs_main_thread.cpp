...
void InsertToEnd(
    std::vector<std::unique_ptr<RSTransactionData>>& source, std::vector<std::unique_ptr<RSTransactionData>>& target)
{
    ...
    const RSScreenProperty& screenProperty)
{
    ...
    if (RSSingleton<RoundCornerDisplayManager>::GetInstance().GetRcdEnable()) {
        RSSingleton<RoundCornerDisplayManager>::GetInstance().RunHardwareTask(id, [id, &processor, &rcdInfo](void) {
    ...
    const std::shared_ptr<RSComposerClientManager>& composerClientManager)
{
...
#endif
        hgmRenderContext_->NotifyRpHgmFrameRate(vsyncId_, context_, rsVsyncRateReduceManager_.GetVrateMap(),
            rsVsyncRateReduceManager_.CheckNeedNotify(), pipelineParam_);
    ...
    RSSurfaceBufferCallbackManager::Instance().SetVSyncFuncs({
        .requestNextVSync = []() { RSMainThread::Instance()->RequestNextVSync(); },
        .isRequestedNextVSync = []() { return RSMainThread::Instance()->IsRequestedNextVSync(); },
    });
...
#ifdef RES_SCHED_ENABLE
    int qosRes = OHOS::ConcurrentTask::ConcurrentTaskClient::GetInstance().SetSystemQoS(
        gettid(), OHOS::ConcurrentTask::SystemQoSLevel::SYSTEM_QOS_EIGHT);
    ...
    if (RSSystemProperties::GetGpuApiType() == GpuApiType::OPENGL) {
        ...
        if (maxResourcesSize > 0) {
            gpuContext->SetResourceCacheLimits(cacheLimitsTimes * maxResources,
                cacheLimitsTimes * std::fmin(maxResourcesSize, DEFAULT_SKIA_CACHE_SIZE));
        } else {
    ...
    RSRenderNodeGC::Instance().SetImageReleaseFunc([this]() {
        PostTask([this]() { ReleaseImageMem(); }, "ReleaseImageMem in mainthread", 0,
    ...
    const sptr<IConsumerSurface>& consumer, int64_t& desiredPresentTimeStamp)
{
    ...
    if (consumer->GetFrontDesiredPresentTimeStamp(fronDesiredPresentTimeStamp, isAutoTimeStamp) !=
            GSError::GSERROR_OK ||
        fronDesiredPresentTimeStamp <= 0 || isAutoTimeStamp) {
        desiredPresentTimeStamp = 0;
        return;
    }
...
void RSMainThread::NotifyUnmarshalTask(int64_t uiTimestamp)
{
    ...
    if (isUniRender_ && rsVsyncManagerAgent_->IsUiDvsyncOn() &&
        static_cast<uint64_t>(uiTimestamp) + static_cast<uint64_t>(rsVsyncManagerAgent_->GetUiCommandDelayTime()) >=
            vsyncRsTimestamp_.load() &&
...
void RSMainThread::UpdateGpuContextCacheSize()
{
    ...
    if (gpuCacheParam != nullptr && gpuCacheParam->GetGpuCacheConfigEnable()) {
        ...
        RS_LOGI(
            "UpdateGpuContextCacheSize, gpu cache size of param config: %{public}zu Bytes", cacheLimitsResourceSize);
    } else {
        gpuContext->GetResourceCacheLimits(&maxResources, &maxResourcesSize);
        ...
        cacheLimitsResourceSize =
            baseResourceSize * actualScale * MEMUNIT_RATE * MEMUNIT_RATE; // adjust by actual Resolution
        cacheLimitsResourceSize =
            cacheLimitsResourceSize > MAX_GPU_CONTEXT_CACHE_SIZE ? MAX_GPU_CONTEXT_CACHE_SIZE : cacheLimitsResourceSize;
...
void RSMainThread::InitGPUCacheManager()
{
    ...
    RSSubThreadManager::Instance()->SetGetGPUCacheManagerFunc(
        [renderEnginePtr]() -> std::shared_ptr<GPUCacheManager> { return renderEnginePtr->GetGPUCacheManager(); });

    // Set global GPU cache cleanup callback for RSSurfaceHandler (dependency injection)
    RSSurfaceHandler::SetGPUCacheCleanupCallback([renderEnginePtr](const std::set<uint64_t>& bufferIds) {
    ...
    RSSurfaceHandler::SetConsumerDeleteBufferListenerCallback(
        [renderEnginePtr](
            const sptr<IConsumerSurface>& consumer) { renderEnginePtr->RegisterDeleteBufferListener(consumer); });
}
#endif
...
void RSMainThread::SetRSEventDetectorLoopFinishTag()
{
    if (rsCompositionTimeoutDetector_ != nullptr) {
        if (isUniRender_) {
            ...
            rsCompositionTimeoutDetector_->SetLoopFinishTag(-1, -1, defaultFocusAppInfo, defaultFocusAppInfo);
        }
    }
}
...
{
    RSLogicalDisplayRenderNode::SetScreenSwitchFinishTask(
        [conn](ScreenId id) { conn->NotifyScreenSwitchFinished(id); });
}
...
bool RSMainThread::NeedConsumeDVSyncCommand(
    uint32_t& endIndex, std::vector<std::unique_ptr<RSTransactionData>>& transactionVec)
{
    uint64_t lastTime = 0;
...
void RSMainThread::CheckAndUpdateTransactionIndex(
        ...
        for (; iter != transactionVec.end(); ++iter) {
            ...
            if (curIndex == lastIndex + 1) {
                ...
                if ((timestamp_ - transactionDataLastWaitTime_[pid]) / REFRESH_PERIOD > SKIP_COMMAND_FREQ_LIMIT) {
                    ...
                    RS_LOGE(
                        "%{public}s skip to index:%{public}" PRIu64 ", pid:%{public}d", __FUNCTION__, curIndex, pid);
                    continue;
                }
                break;
            }
...
void RSMainThread::PostTryReclaimLastBuffer(
    const std::shared_ptr<RSSurfaceRenderNode>& surfaceNode, std::shared_ptr<RSSurfaceHandler> surfaceHandler)
{
    ...
    if (!surfaceNode->IsOnTheTree() && surfaceHandler->IsNeedSwapLastBuffer()) {
        ...
        if (surfaceHandler->ReclaimLastBufferPrepare()) {
            ...
            auto task = [this, surfaceHandler]() { surfaceHandler->ReclaimLastBufferProcess(); };
            PostTask(task);
        }
    }
}
    ...
    if (UNLIKELY(consumeAndUpdateNode_ == nullptr)) {
        consumeAndUpdateNode_ = [this](const std::shared_ptr<RSSurfaceRenderNode>& surfaceNode) mutable {
            ...
            if (surfaceNode->GetName().find(CAPTURE_WINDOW_NAME) != std::string::npos ||
                ...
                RS_LOGD("ConsumeAndUpdateAllNodes set %{public}s content dirty", surfaceNode->GetName().c_str());
            ...
            auto comsumeResult = RSBaseRenderUtil::ConsumeAndUpdateBuffer(*surfaceHandler, timestamp_, dropFrameConfig,
            ...
            if (comsumeResult) {
                ...
                if (isUniRender_ && surfaceHandler->IsCurrentFrameBufferConsumed()) {
                    ...
                    if (consumer != nullptr) {
                        ...
                        RSSurfaceFpsManager::GetInstance().RecordFlushTime(
                            surfaceHandler->GetNodeId(), vsyncId_, buffer ? buffer->GetFlushedTimestamp() : 0);
                    }
                    RSGpuDirtyCollector::SetGpuDirtyEnabled(
                        buffer, RSGpuDirtyCollector::GetInstance().IsGpuDirtyEnable(surfaceNode->GetId()));
                    ...
                    if (surfaceHandler->GetBufferSizeChanged() || surfaceHandler->GetBufferTransformTypeChanged()) {
                        ...
                        RS_OPTIONAL_TRACE_NAME_FMT("hwc debug: name %s, id %" PRIu64 " disable directComposition by "
                                                   "surfaceNode buffer size changed",
                            surfaceNode->GetName().c_str(), surfaceNode->GetId());
...
void RSMainThread::CollectInfoForHardwareComposer()
{
    ...
    const auto& nodeMap = GetContext().GetNodeMap();
    nodeMap.TraverseSurfaceNodes([this, &nodeMap](const std::shared_ptr<RSSurfaceRenderNode>& surfaceNode) mutable {
        ...
        if (!surfaceNode->GetDoDirectComposition()) {
            doDirectComposition_ = false;
            RS_OPTIONAL_TRACE_NAME_FMT("hwc debug: name %s, id %" PRIu64 " disable directComposition by "
                                       "surfaceNode doDirectComposition is false",
                surfaceNode->GetName().c_str(), surfaceNode->GetId());
        ...
        if (isHardwareForcedDisabled_) {
            ...
            if (surfaceHandler->IsCurrentFrameBufferConsumed()) {
                ...
                RS_OPTIONAL_TRACE_NAME_FMT("hwc debug: name %s, id %" PRIu64
                                           " disable directComposition by lastFrame not enabled HWC "
                                           "and buffer consumed",
                    surfaceNode->GetName().c_str(), surfaceNode->GetId());
            } else {
...
bool RSMainThread::IsFoldScreenSwitching() const
{
    ...
    nodeMap.TraverseScreenNodes(
        [&res](const auto& screenNode) { res |= screenNode->GetScreenProperty().IsScreenSwitching(); });
    return res;
}
    ...
    isHardwareForcedDisabled_ =
        (!hasProtectedLayer_ &&
            (isHardwareForcedDisabled_ || doWindowAnimate_ || isFoldScreenSwitching ||
                (isMultiDisplay && (isExpandScreenOrWiredProjectionCase || !enableHwcForMirrorMode)) ||
                hasColorFilter)) ||
        CheckOverlayDisplayEnable();
    RS_OPTIONAL_TRACE_FMT(
        "hwc debug: CheckIfHardwareForcedDisabled hasProtectedLayer:%d isHardwareForcedDisabled:%d"
...
void RSMainThread::ClearMemoryCache(ClearMemoryMoment moment, bool deeply, pid_t pid)
{
    ...
    auto task = [this, moment, deeply, isDeeplyRelGpuResEnable]() {
...
void RSMainThread::AddUiCaptureTask(NodeId id, std::function<void()> task)
{
    ...
    if (!node) {
        ...
        RS_TRACE_NAME_FMT("RSMainThread::AddUiCaptureTask isDirty:%d, subDirty:%d, isOnTheTree:%d", node->IsDirty(),
            node->IsSubTreeDirty(), node->IsOnTheTree());
...
void RSMainThread::PrepareUiCaptureTasks(std::shared_ptr<RSUniRenderVisitor> uniVisitor)
{
    ...
    for (auto [id, captureTask] : pendingUiCaptureTasks_) {
        ...
        if (!cmdFlag.first && duration < TIME_OF_CAPTURE_TASK_REMAIN) {
            RS_TRACE_NAME_FMT("RSMainThread::PrepareUiCaptureTasks cmds not be processed, id: %" PRIu64
                              ", duration: %" PRIu64 "ms",
                id, duration);
            RS_LOGI("PrepareUiCaptureTasks cmds not be processed, id: %{public}" PRIu64 ", duration: %{public}" PRIu64
                    "ms",
                id, duration);
    ...
    pendingUiCaptureTasks_.insert(
        pendingUiCaptureTasks_.end(), remainUiCaptureTasks.begin(), remainUiCaptureTasks.end());
...
void RSMainThread::UniRender(std::shared_ptr<RSBaseRenderNode> rootNode)
{
    ...
    bool willGoDirectComposition =
        doDirectComposition_ && !isDirty_ && !isAccessibilityConfigChanged_ && !isCachedSurfaceUpdated_ && pointerSkip;
    ...
    if (needTraverseNodeTree) {
        ...
        if (frameRatePidFromRSS != 0) {
            nodeMapForFrameReport.TraverseSurfaceNodesBreakOnCondition(
                [this, frameRatePidFromRSS](const std::shared_ptr<RSSurfaceRenderNode>& surfaceNode) {
                    if (surfaceNode == nullptr) {
                        return false;
                    }
    ...
    if (screenPowerOnChanged_) {
        ...
        RS_LOGI("RSMainThread Power On First Frame finish, node:%{public}d", nodeNum);
    }
...
bool RSMainThread::IfStatusBarDirtyOnly()
{
    ...
    for (const auto& rootPair : activeNodesInRoot) {
        ...
        auto rootNode =
            RSBaseRenderNode::ReinterpretCast<RSSurfaceRenderNode>(context_->GetNodeMap().GetRenderNode(rootId));
...
namespace {
bool CheckReduceIntervalForAIBarNodesIfNeeded(
    const RSRenderNode::WeakPtrSet& nodeSet, const std::vector<std::shared_ptr<RSSurfaceRenderNode>>& hwcNodes)
{
    ...
    for (auto& weakPtr : nodeSet) {
        ...
        if (RSSystemProperties::GetAIBarOptEnabled()) {
            ...
            intersectHwcDamage = std::any_of(hwcNodes.begin(), hwcNodes.end(), [&filterRect](const auto& hwcNode) {
                return hwcNode && RSSurfaceRenderNodeUtils::IntersectHwcDamageWith(*hwcNode, filterRect);
            });
        }
        RS_OPTIONAL_TRACE_NAME_FMT(
            "CheckReduceIntervalForAIBarNodesIfNeeded: intersectHwcDamage[%d]", static_cast<int>(intersectHwcDamage));
...
bool RSMainThread::DoDirectComposition(std::shared_ptr<RSBaseRenderNode> rootNode)
{
    ...
    RSUniRenderThread::Instance().PostSyncTask([this, processor, screenNode]() mutable {
        ...
        rsLuminance.SetHdrStatus(
            screenId, screenNode->GetForceCloseHdr() ? HdrStatus::NO_HDR : screenNode->GetDisplayHdrStatus());
...
void RSMainThread::OnUniRenderDraw()
{
    ...
    if ((doDirectComposition_ && !directComposeHelper_.isLastFrameDirectComposition_) || isNeedResetClearMemoryTask_ ||
        !needDrawFrame_) {
...
void RSMainThread::CallbackDrawContextStatusToWMS(bool isUniRender)
{
    ...
    for (auto dynamicNodeId : curDrawStatusVec) {
        if (lastDrawStatusMap_.find(dynamicNodeId) == lastDrawStatusMap_.end()) {
            drawStatusVec.emplace_back(
                std::make_pair(dynamicNodeId, WINDOW_LAYER_INFO_TYPE::WINDOW_LAYER_DYNAMIC_STATUS));
            RS_OPTIONAL_TRACE_NAME_FMT("%s nodeId[%" PRIu64 "] status[%d]", __func__, dynamicNodeId,
    ...
    while (drawStatusIter != lastDrawStatusMap_.end()) {
        if (timestamp - drawStatusIter->second > MAX_DYNAMIC_STATUS_TIME) {
            drawStatusVec.emplace_back(
                std::make_pair(drawStatusIter->first, WINDOW_LAYER_INFO_TYPE::WINDOW_LAYER_STATIC_STATUS));
            RS_OPTIONAL_TRACE_NAME_FMT("%s nodeId[%" PRIu64 "] status[%d]", __func__, drawStatusIter->first,
...
bool RSMainThread::CheckSurfaceNeedProcess(
    OcclusionRectISet& occlusionSurfaces, std::shared_ptr<RSSurfaceRenderNode> curSurface)
{
    bool needProcess = false;
    if (curSurface->IsFocusedNode(focusNodeId_)) {
        needProcess = true;
        if (!curSurface->HasContainerWindow() && !curSurface->IsTransparent() && !curSurface->HasWindowCorner() &&
        ...
        if (insertSuccess) {
            needProcess = true;
            if (curSurface->IsTransparent() || curSurface->HasWindowCorner() ||
                curSurface->GetAnimateState() || // when node animating(i.e. 3d animation), the region cannot be trusted
                curSurface->GetName().find("hisearch") != std::string::npos) {
...
RSVisibleLevel RSMainThread::GetRegionVisibleLevel(
    const Occlusion::Region& curRegion, const Occlusion::Region& visibleRegion)
{
    ...
    std::vector<RSBaseRenderNode::SharedPtr>& curAllSurfaces, VisibleData& dstCurVisVec)
{
    ...
    auto calculator = [this, &screenNode, &occlusionSurfaces, &accumulatedRegion, &curVisVec, &hasFilterCacheOcclusion,
        ...
        curSurface->SetVisibleRegionRecursive(
            totalRegion, curVisVec, needSetVisibleRegion, visibleLevel, !systemAnimatedScenesList_.empty());
...
void RSMainThread::CalcOcclusion()
{
    ...
    RS_LOGD("CalcOcclusion animate:%{public}d isUniRender:%{public}d", doWindowAnimate_.load(), isUniRender_);
    ...
    if (!winDirty) {
        for (auto it = curAllSurfaces.rbegin(); it != curAllSurfaces.rend(); ++it) {
            ...
            if (surface->GetZorderChanged() || surface->GetDstRectChanged() || surface->IsOpaqueRegionChanged() ||
                surface->GetAlphaChanged() || (isUniRender_ && surface->IsDirtyRegionUpdated())) {
                winDirty = true;
            } else if (RSSystemParameters::GetFilterCacheOcculusionEnabled() && surface->IsTransparent() &&
                       surface->IsFilterCacheStatusChanged()) {
...
bool RSMainThread::CheckSurfaceOcclusionNeedProcess(NodeId id)
{
    ...
    if (savedAppWindowNode_.find(id) == savedAppWindowNode_.end()) {
        ...
        if (!node || !node->IsOnTheTree()) {
            RS_LOGD("SurfaceOcclusionCallback cannot find surfacenode %{public}" PRIu64 ".", id);
            return false;
        }
        auto appWindowNodeId = node->GetInstanceRootNodeId();
        if (appWindowNodeId == INVALID_NODEID) {
            RS_LOGD("SurfaceOcclusionCallback surfacenode %{public}" PRIu64 " cannot find app window node.", id);
            return false;
        }
...
void RSMainThread::RequestNextVSync(const std::string& fromWhom, int64_t lastVSyncTS, const int64_t& requestVsyncTime)
{
    ...
    VSyncReceiver::FrameCallback fcb = {
        .userData_ = this,
        .callbackWithId_ = [this](uint64_t timestamp, uint64_t frameCount,
                               void* data) { OnVsync(timestamp, frameCount, data); },
    };
    if (receiver_ != nullptr) {
        ...
        if (requestNextVsyncNum_ >= VSYNC_LOG_ENABLED_TIMES_THRESHOLD &&
            requestNextVsyncNum_ % VSYNC_LOG_ENABLED_STEP_TIMES == 0 && rsVsyncManagerAgent_ != nullptr) {
...
void RSMainThread::DumpEventHandlerInfo()
{
    ...
    if (immediateStart != std::string::npos) {
        ...
        if (immediateEnd != std::string::npos) {
            ...
            RS_LOGW(
                "RequestNextVSync PriorityEventQueue is %{public}s", SubPriorityEventQueue(priorityEventQueue).c_str());
        }
    }
...
void RSMainThread::RSJankStatsOnVsyncStart(
...
void RSMainThread::RSJankStatsOnVsyncEnd(
    int64_t onVsyncStartTime, int64_t onVsyncStartTimeSteady, float onVsyncStartTimeSteadyFloat)
{
#ifdef RS_ENABLE_GPU
...
void RSMainThread::SetVsyncInfo(uint64_t timestamp)
{
    ...
    RS_LOGD("UpdateVsyncTime = %{public}lld, curTime_ = %{public}lld, "
        ...
        static_cast<long long>(timestamp), static_cast<long long>(curTime_), static_cast<long long>(vsyncPeriod),
...
void RSMainThread::Animate(uint64_t timestamp)
{
    ...
    EraseIf(context_->animatingNodeList_,
        [this, timestamp, period, isDisplaySyncEnabled, isRateDeciderEnabled, &totalAnimationSize, &curWinAnim,
            &needRequestNextVsync, &isCalculateAnimationValue, &needPrintAnimationDFX, &minLeftDelayTime,
...
void RSMainThread::PostTask(
    RSTaskMessage::RSTask task, const std::string& name, int64_t delayTime, AppExecFwk::EventQueue::Priority priority)
{
    if (handler_) {
...
void RSMainThread::SendClientDumpNodeTreeCommands(uint32_t taskId)
{
    ...
    RS_LOGI("SendClientDumpNodeTreeCommands send task[%{public}u] count[%{public}zu]", taskId, task.count);
}
...
void RSMainThread::CollectClientNodeTreeResult(uint32_t taskId, std::string& dumpString, size_t timeout)
{
    ...
    RS_LOGI("CollectClientNodeTreeResult task[%{public}u] completionCount[%{public}zu]", taskId, completed);
}

void RSMainThread::OnCommitDumpClientNodeTree(NodeId nodeId, pid_t pid, uint32_t taskId, const std::string& result)
{
    RS_TRACE_NAME_FMT("DumpClientNodeTree collected task[%u] dataSize[%zu] pid[%d]", taskId, result.size(), pid);
    {
        std::unique_lock<std::mutex> lock(nodeTreeDumpMutex_);
    ...
    RS_LOGI("OnDumpClientNodeTree task[%{public}u] dataSize[%{public}zu] pid[%d]", taskId, result.size(), pid);
}
...
void RSMainThread::DumpMem(
    std::unordered_set<std::u16string>& argSets, std::string& dumpString, std::string& type, pid_t pid, bool isLite)
{
#if defined(RS_ENABLE_GL) || defined(RS_ENABLE_VK)
    DfxString log;
    if (pid != 0) {
        RSUniRenderThread::Instance().PostSyncTask([&log, pid] {
            ...
            MemoryManager::DumpPidMemory(
                log, pid, RSUniRenderThread::Instance().GetRenderEngine()->GetRenderContext()->GetDrGPUContext());
        });
    } else {
...
void RSMainThread::CountMem(int pid, MemoryGraphic& mem)
{
    ...
    RSUniRenderThread::Instance().PostSyncTask([&mem, pid] {
        ...
        mem = MemoryManager::CountPidMemory(
            pid, RSUniRenderThread::Instance().GetRenderEngine()->GetRenderContext()->GetDrGPUContext());
    });
#endif
}
...
void RSMainThread::CountMem(std::vector<MemoryGraphic>& mems)
{
    ...
    RSUniRenderThread::Instance().PostSyncTask([&mems, &pids] {
        MemoryManager::CountMemory(
            pids, RSUniRenderThread::Instance().GetRenderEngine()->GetRenderContext()->GetDrGPUContext(), mems);
    });
#endif
}
...
void RSMainThread::AddTransactionDataPidInfo(pid_t remotePid)
{
    ...
    if (it != effectiveTransactionDataIndexMap_.end()) {
        ...
        effectiveTransactionDataIndexMap_.emplace(
            remotePid, std::make_pair(0, std::vector<std::unique_ptr<RSTransactionData>>()));
        ;
    }
}
...
bool RSMainThread::IsFastComposeAllow(
    ...
    uint64_t unsignedNowTime, uint64_t lastVsyncTime, int64_t vsyncTimeStamp)
{
    ...
    if (!nextVsyncRequested &&
        (unsignedNowTime - lastVsyncTime) % unsignedVsyncPeriod > unsignedVsyncPeriod - FASTCOMPOSE_OFFSET) {
...
bool RSMainThread::CheckFastCompose(int64_t lastFlushedDesiredPresentTimeStamp)
{
    ...
    if (!IsFastComposeVsyncTimeSync(
            unsignedVsyncPeriod, nextVsyncRequested, unsignedNowTime, lastVsyncTime, vsyncTimeStamp)) {
        return false;
    }
    ...
    RS_TRACE_NAME_FMT("RSMainThread::CheckFastCompose now = %" PRIu64 ", lastVsyncTime = %" PRIu64 ", timestamp_ = %"
                      "" PRIu64 "earlyFastCompose = %d",
        unsignedNowTime, lastVsyncTime, timestamp_, earlyFastComposeFlag);
...
void RSMainThread::RenderFrameStart(uint64_t timestamp)
{
    ...
    int skipFirstFrame =
        (drawingRequestNextVsyncNum_.load() == SKIP_FIRST_FRAME_DRAWING_NUM) && forceUpdateUniRenderFlag_;
...
bool RSMainThread::SetSystemAnimatedScenes(SystemAnimatedScenes systemAnimatedScenes, bool isRegularAnimation)
{
    RS_OPTIONAL_TRACE_NAME_FMT("%s systemAnimatedScenes[%u] systemAnimatedScenes_[%u] threeFingerScenesListSize[%u] "
                               "systemAnimatedScenesListSize_[%u] isRegularAnimation_[%d]",
        __func__, systemAnimatedScenes, systemAnimatedScenes_, threeFingerScenesList_.size(),
        systemAnimatedScenesList_.size(), isRegularAnimation);
...
{
    auto nodePtr = RSBaseRenderNode::ReinterpretCast<RSSurfaceRenderNode>(GetContext().GetNodeMap().GetRenderNode(id));
    if (nodePtr == nullptr) {
        return false;
    }
...
{
    std::lock_guard<std::mutex> lock(watermarkMutex_);
    auto res = surfaceWatermarkHelper_.SetSurfaceWatermark(
        pid, name, watermark, nodeIdList, watermarkType, GetContext(), isSystemCalling);
    SetDirtyFlag(true);
    RequestNextVSync();
    return res;
}
...
void RSMainThread::ClearSurfaceWatermarkForNodes(
...
void RSMainThread::UpdateAnimateNodeFlag()
{
    ...
    context_->curFrameAnimatingNodeList_.insert(
        context_->animatingNodeList_.begin(), context_->animatingNodeList_.end());
...
{
    RSBackgroundThread::Instance().PostTask([this]() {
...
void RSMainThread::RegisterUIExtensionCallback(
    pid_t pid, uint64_t userId, sptr<RSIUIExtensionCallback> callback, bool unobscured)
{
...
void RSMainThread::UIExtensionNodesTraverseAndCallback()
{
    ...
    RSUniRenderUtil::UIExtensionFindAndTraverseAncestor(
        context_->GetNodeMap(), unobscureduiExtensionCallbackData_, true);
#endif
    if (CheckUIExtensionCallbackDataChanged()) {
        RS_OPTIONAL_TRACE_NAME_FMT(
            "RSMainThread::UIExtensionNodesTraverseAndCallback data size: [%lu]", uiExtensionCallbackData_.size());
        ...
        for (const auto& item : uiUnobscuredExtensionListenners_) {
            ...
            auto callback = item.second.second;
            if (callback) {
                callback->OnUIExtension(
                    std::make_shared<RSUIExtensionData>(unobscureduiExtensionCallbackData_), userId);
            }
        }
    }
...
void RSMainThread::CreateScreenNode(const sptr<RSScreenProperty>& property)
{
    ...
    RS_LOGI("%{public}s, screen id: %{public}" PRIu64 " screenWidth[%{public}d] screenHeight[%{public}d]", __func__, id,
    ...
    auto node = std::shared_ptr<RSScreenRenderNode>(
        new RSScreenRenderNode(GenerateUniqueNodeIdForRS(), id, context_->weak_from_this()),
    ...
    auto setOnTree = [id, context = context_](auto& node) {
        bool isConditionMet = node && node->GetScreenId() == id && !node->IsOnTheTree() && node->IsWaitToSetOnTree();
        if (isConditionMet) {
...
void RSMainThread::HandlePowerStatusChanged(
    ScreenId id, ScreenPropertyType type, const sptr<ScreenPropertyBase>& property)
{
    ...
    if (curStatus != ScreenPowerStatus::POWER_STATUS_ON && curStatus != ScreenPowerStatus::POWER_STATUS_ON_ADVANCED) {
        return;
    }
...
void RSMainThread::UpdateScreenProperty(ScreenId id, ScreenPropertyType type, const sptr<ScreenPropertyBase>& property)
{
    if (!property) {
        RS_LOGE("%{public}s, property is nullptr.", __func__);
        return;
...
void RSMainThread::HandleTunnelLayerId(
    const std::shared_ptr<RSSurfaceHandler>& surfaceHandler, const std::shared_ptr<RSSurfaceRenderNode>& surfaceNode)
{
    ...
    if (surfaceHandler->GetSourceType() != static_cast<uint32_t>(OHSurfaceSource::OH_SURFACE_SOURCE_LOWPOWERVIDEO)) {

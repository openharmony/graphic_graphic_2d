...
constexpr int32_t MAX_NON_SYSTEM_UI_CAPTURE_COUNT = 5;
constexpr int32_t MAX_SYSTEM_UI_CAPTURE_COUNT = 20;
...
void PerfRequest(int32_t perfRequestCode, bool onOffTag)
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "soc perf info [%{public}d %{public}d]", perfRequestCode, onOffTag);
#endif
}
    ...
    const RSScreenProperty& screenProperty)
{
    if (rcdInfo == nullptr || processor == nullptr) {
        RS_LOGD_IF(DEBUG_PIPELINE, "DoScreenRcdTask has nullptr processor or rcdInfo");
        return;
    }
    if (!RoundCornerDisplayManager::CheckRcdRenderEnable(screenProperty)) {
        RS_LOGD_IF(DEBUG_PIPELINE, "DoScreenRcdTask is not at HDI_OUPUT mode");
        return;
    }
...
void RSMainThread::MarkNodeDirty(uint64_t nodeId)
{
    RSMainThread::Instance()->PostTask([nodeId]() {
        ...
        if (node) {
            RS_LOGD_IF(DEBUG_PIPELINE, "MarkNodeDirty success: %{public}" PRIu64, nodeId);
    ...
    const std::shared_ptr<RSComposerClientManager>& composerClientManager)
{
    ...
    context_->SetRecreateNodeCallback(
        [this](NodeId nodeId, std::weak_ptr<RSSurfaceBufferInterface> surfaceBufferInterface) {
#ifndef ROSEN_CROSS_PLATFORM
            auto savedInfo = context_->GetNodeMap().GetSurfaceHandlerInfo(nodeId);
            if (savedInfo.second == nullptr) {
                return;
            }
            auto rsListener = static_cast<RSRenderServiceListener*>(savedInfo.second.GetRefPtr());
            if (rsListener != nullptr) {
                rsListener->SetRSSurfaceBufferInterface(surfaceBufferInterface);
            }
#endif
        });
...
void RSMainThread::CleanRenderNodes(pid_t remotePid) noexcept
{
    ...
    nodeMap.UnregisterSurfaceHandlerByPid(remotePid);
...
{
    RS_LOGD_IF(DEBUG_PIPELINE, "GetMaxGpuBufferSize: start query GPU buffer size limits");
...
void RSMainThread::UpdateGpuContextCacheSize()
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "systemCacheLimitResourceSize: %{public}d", systemCacheLimitResourceSize);
...
void RSMainThread::InitRSEventDetector()
{
    ...
    if (rsCompositionTimeoutDetector_ != nullptr) {
        ...
        RS_LOGD_IF(DEBUG_PIPELINE, "InitRSEventDetector finish");
    }
}
...
void RSMainThread::ProcessCommandForUniRender()
{
    ...
    nodeMap.TraverseCanvasDrawingNodes([this](const std::shared_ptr<RSCanvasDrawingRenderNode>& canvasDrawingNode) {
        ...
        bool isHybridEnabled = false;
#ifdef RS_MODIFIERS_DRAW_ENABLE
        isHybridEnabled = RSCanvasDrawingRenderNode::IsHybridEnabled();
#endif
        if (isHybridEnabled) {
#ifdef RS_MODIFIERS_DRAW_ENABLE
            auto surfaceHandler = canvasDrawingNode->GetMutableSurfaceHandler();
...
bool RSMainThread::CheckIfNeedSplitTransaction(std::unique_ptr<RSTransactionData>& rsTransactionData)
{
    if (rsTransactionData->GetRSTransactionDataScene() == RSTransactionDataScenes::Rebuild) {
        return true;
    }
    return false;
}
...
void RSMainThread::ProcessRSTransactionData(std::unique_ptr<RSTransactionData>& rsTransactionData, pid_t pid)
{
    ...
    // 检查是否为重建事务序列
    if (CheckIfNeedSplitTransaction(rsTransactionData)) {
        RS_TRACE_NAME("ProcessRSTransactionData: detected rebuild transaction sequence");
        AddSplitTransaction(std::move(rsTransactionData));
        return;
    }
...
void RSMainThread::CollectInfoForHardwareComposer()
{
    ...
    if (!pendingNonSystemUiCaptureTasks_.empty() || !pendingSystemUiCaptureTasks_.empty() ||
        !pendingSyncWindowCaptureTasks_.empty()) {
        doDirectComposition_ = false;
    ...
    nodeMap.TraverseSurfaceNodes(
        [this, &nodeMap, uiMode3D](const std::shared_ptr<RSSurfaceRenderNode>& surfaceNode) mutable {
            ...
            if (surfaceNode->GetName().find(CAPTURE_WINDOW_NAME) != std::string::npos ||
                ...
                RS_LOGD_IF(DEBUG_PIPELINE, "CollectInfoForHardwareComposer set %{public}s content dirty",
...
void RSMainThread::CheckIfHardwareForcedDisabled()
{
    ...
    bool enableHwcForWiredMirror = HWCParam::IsEnableHwcOnWiredMirror();
    // Wired mirror can keep main screen hardware composer enabled when enableHwcForWiredMirror is on
    bool disableByMultiDisplay = isMultiDisplay && isExpandScreenOrWiredProjectionCase && !enableHwcForWiredMirror;
    isHardwareForcedDisabled_ = (!hasProtectedLayer_ &&
        (isHardwareForcedDisabled_ || isFoldScreenSwitching || disableByMultiDisplay || hasColorFilter)) ||
    ...
    RS_OPTIONAL_TRACE_FMT("hwc debug: CheckIfHardwareForcedDisabled hasProtectedLayer:%d isHardwareForcedDisabled:%d"
        ...
        " enableHwcForWiredMirror:%d hasColorFilter:%d overlayDisplayEnable:%d",
        hasProtectedLayer_, isHardwareForcedDisabled_, doWindowAnimate_.load(), isFoldScreenSwitching, isMultiDisplay,
        isExpandScreenOrWiredProjectionCase, enableHwcForWiredMirror, hasColorFilter, CheckOverlayDisplayEnable());
...
void RSMainThread::ClearMemoryCache(ClearMemoryMoment moment, bool deeply, pid_t pid)
{
    ...
    auto task =
        [this, moment, deeply, isDeeplyRelGpuResEnable]() {
            ...
            RS_LOGD_IF(DEBUG_PIPELINE, "Clear memory cache %{public}d", this->GetClearMoment());
...
void RSMainThread::AddUiCaptureTask(NodeId id, bool isSystemCalling, std::function<void()> task)
{
    if (isSystemCalling) {
        pendingSystemUiCaptureTasks_.emplace_back(id, task);
    } else {
        pendingNonSystemUiCaptureTasks_.emplace_back(id, task);
    }
    ...
    if (!node) {
        RS_LOGW("RSMainThread::AddUiCaptureTask node nullptr, id: %{public}" PRIu64
            ", isSystemCalling: %{public}u", id, isSystemCalling);
    } else {
        bool isNeedSetTreeStateChangeDirty = node && (node->IsDirty() || node->IsSubTreeDirty());
        RS_TRACE_NAME_FMT("RSMainThread::AddUiCaptureTask isSystem:%d, isDirty:%d, subDirty:%d, isOnTheTree:%d",
            isSystemCalling, node->IsDirty(), node->IsSubTreeDirty(), node->IsOnTheTree());
...
{
    PrepareCaptureQueue(pendingNonSystemUiCaptureTasks_, nonSystemUiCaptureTasks_);
    PrepareCaptureQueue(pendingSystemUiCaptureTasks_, systemUiCaptureTasks_);
...
static void ProcessUiCaptureQueue(std::queue<std::tuple<NodeId, std::function<void()>>>& readyQueue,
    int32_t (&getInCaptureProgress)(), int32_t limit, const char* category)
{
    while (!readyQueue.empty()) {
        // inCaptureProgress: 已 ++ 未 -- 的在途截图数; remaining: 队列待派发数
        int32_t inCaptureProgress = getInCaptureProgress();
        if (inCaptureProgress >= limit) {
            int32_t remaining = static_cast<int32_t>(readyQueue.size());
            RS_LOGW("ProcessSyncCaptureTasks %{public}s reach limit this frame, "
                "inCaptureProgress:%{public}d, limit:%{public}d, remaining:%{public}d",
                category, inCaptureProgress, limit, remaining);
            RS_TRACE_NAME_FMT("%s reach limit: inCaptureProgress=%d, remaining=%d",
                category, inCaptureProgress, remaining);
            break;
        }
        NodeId nodeId = std::get<0>(readyQueue.front());
        auto captureTask = std::get<1>(readyQueue.front());
        readyQueue.pop();
        int32_t remaining = static_cast<int32_t>(readyQueue.size());
        RS_LOGD("ProcessSyncCaptureTasks %{public}s execute, id: %{public}" PRIu64
            ", inCaptureProgress:%{public}d, remaining:%{public}d",
            category, nodeId, inCaptureProgress, remaining);
        captureTask();
...
{
#ifdef RS_ENABLE_GPU
    // Process UI capture tasks (non-system / system, separate FIFOs and limits)
    ProcessUiCaptureQueue(nonSystemUiCaptureTasks_, RSUiCaptureTaskParallel::GetNonSystemCaptureCount,
        MAX_NON_SYSTEM_UI_CAPTURE_COUNT, "nonSystemUICapture");
    ProcessUiCaptureQueue(systemUiCaptureTasks_, RSUiCaptureTaskParallel::GetSystemCaptureCount,
        MAX_SYSTEM_UI_CAPTURE_COUNT, "systemUICapture");
    ...
    RS_TRACE_NAME_FMT("ProcessSyncCaptureTasks done: nonSystemInCaptureProgress=%d, systemInCaptureProgress=%d",
        RSUiCaptureTaskParallel::GetNonSystemCaptureCount(), RSUiCaptureTaskParallel::GetSystemCaptureCount());
#endif
}

bool RSMainThread::IsSnapshotPendingThisFrame() const
{
    return !pendingNonSystemUiCaptureTasks_.empty() || !nonSystemUiCaptureTasks_.empty() ||
...
void RSMainThread::UniRender(std::shared_ptr<RSBaseRenderNode> rootNode)
{
    ...
    if (isAccessibilityConfigChanged_) {
        RS_LOGD_IF(DEBUG_PIPELINE, "UniRender AccessibilityConfig has Changed");
    }
...
bool RSMainThread::DoDirectComposition(std::shared_ptr<RSBaseRenderNode> rootNode)
{
        ...
        for (auto& surfaceNode : hardwareEnabledNodes_) {
            ...
            if (!surfaceNode->IsHardwareForcedDisabled()) {
                ...
                if (surfaceNode->GetDeviceOfflineEnable() && processor->ProcessOfflineLayer(surfaceNode)) {
                    // hpae_offline: offline buffer and original buffer both sent to hwc,
                    // release is handled by hwc release callback.
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "DoDirectComposition end");
    return true;
}
...
void RSMainThread::CalcOcclusion()
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "CalcOcclusion animate:%{public}d isUniRender:%{public}d",
...
void RSMainThread::SurfaceOcclusionCallback()
{
    std::list<std::pair<sptr<RSISurfaceOcclusionChangeCallback>, float>> callbackList;
    {
        ...
        for (auto& listener : surfaceOcclusionListeners_) {
            ...
            if (savedLevel != level) {
                RS_LOGD_IF(DEBUG_PIPELINE, "SurfaceOcclusionCallback surfacenode: %{public}" PRIu64 ".",
                    listener.first);
                savedLevel = level;
                if (isOnTheTree) {
...
bool RSMainThread::CheckSurfaceOcclusionNeedProcess(NodeId id)
{
    ...
    if (savedAppWindowNode_.find(id) == savedAppWindowNode_.end()) {
        ...
        if (!node || !node->IsOnTheTree()) {
            RS_LOGD_IF(DEBUG_PIPELINE, "SurfaceOcclusionCallback cannot find surfacenode %{public}"
                PRIu64 ".", id);
            return false;
        }
        auto appWindowNodeId = node->GetInstanceRootNodeId();
        if (appWindowNodeId == INVALID_NODEID) {
            RS_LOGD_IF(DEBUG_PIPELINE, "SurfaceOcclusionCallback surfacenode %{public}"
        ...
        if (!surfaceNode || !appWindowNode) {
            RS_LOGD_IF(DEBUG_PIPELINE, "SurfaceOcclusionCallback ReinterpretCastTo fail.");
            return false;
        }
...
void RSMainThread::RequestNextVSync(const std::string& fromWhom, int64_t lastVSyncTS, const int64_t& requestVsyncTime)
{
    ...
    if (receiver_ != nullptr) {
        ...
        if (requestNextVsyncNum_ > REQUEST_VSYNC_NUMBER_LIMIT) {
            RS_LOGD_IF(DEBUG_PIPELINE, "RequestNextVSync too many times:%{public}d", requestNextVsyncNum_.load());
...
void RSMainThread::OnVsync(uint64_t timestamp, uint64_t frameCount, void* data)
{
    ...
    if (isUniRender_) {
        ...
        if (RSUnmarshalThread::Instance().CachedTransactionDataEmpty() ||
            RSUnmarshalThread::Instance().IsNoNeedWaitUnmarshalling()) {
...
void RSMainThread::SetVsyncInfo(uint64_t timestamp)
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "UpdateVsyncTime = %{public}lld, curTime_ = %{public}lld, "
...
void RSMainThread::Animate(uint64_t timestamp)
{
    ...
    EraseIf(context_->animatingNodeList_,
        ...
        if (node == nullptr) {
            RS_LOGD_IF(DEBUG_PIPELINE, "Animate removing expired animating node");
            return true;
        }
        ...
        if (!hasRunningAnimation) {
            ...
            RS_LOGD_IF(DEBUG_PIPELINE, "Animate removing finished animating node %{public}" PRIu64, node->GetId());
        } else {
...
void RSMainThread::ClassifyRSTransactionData(std::shared_ptr<RSTransactionData> rsTransactionData)
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "RecvRSTransactionData timestamp = %{public}" PRIu64, timestamp);
...
void RSMainThread::SendCommands()
{
    ...
    PostTask([this, transactionMapPtr]() {
        ...
        RS_LOGD_IF(DEBUG_PIPELINE, "RS send to %{public}s", dfxString.c_str());
    ...
    if (it != effectiveTransactionDataIndexMap_.end()) {
        if (forRefresh) {
            ...
            if (!it->second.second.empty()) {
                RS_LOGD_IF(DEBUG_PIPELINE, "ClearTransactionDataPidInfo process:%{public}d destroyed, skip commands",
                    remotePid);
            }
...
#if defined(RS_ENABLE_GL) || defined(RS_ENABLE_VK)
        RS_LOGD_IF(DEBUG_PIPELINE, "RSMainThread: clear cpu cache pid:%{public}d", remotePid);
...
void RSMainThread::PerfAfterAnim(bool needRequestNextVsync)
{
    ...
    if (needRequestNextVsync && timestamp_ - prePerfTimestamp_ > PERF_PERIOD) {
        RS_LOGD_IF(DEBUG_PIPELINE, "soc perf to render_service_animation");
        prePerfTimestamp_ = timestamp_;
    } else if (!needRequestNextVsync && prePerfTimestamp_) {
        RS_LOGD_IF(DEBUG_PIPELINE, "soc perf off render_service_animation");
        prePerfTimestamp_ = 0;
    }
}
...
bool RSMainThread::SetSystemAnimatedScenes(SystemAnimatedScenes systemAnimatedScenes, bool isRegularAnimation)
{
    if (systemAnimatedScenes < SystemAnimatedScenes::ENTER_MISSION_CENTER ||
        ...
        RS_LOGD_IF(DEBUG_PIPELINE, "SetSystemAnimatedScenes Out of range.");
        return false;
    }
    {
...
bool RSMainThread::IsHardwareEnabledNodesNeedSync()
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "%{public}s %{public}u", __func__, needSync);
    ...
    RSBackgroundThread::Instance().PostTask(
        [this]() {
            ...
            if (Memory::MemMgrClient::GetInstance().SubscribeAppState(*rsAppStateListener_) != -1) {
                RS_LOGD_IF(DEBUG_PIPELINE, "Subscribe MemMgr Success");
...
void RSMainThread::HandleOnTrim(Memory::SystemMemoryLevel level)
{
    if (handler_) {
        handler_->PostTask(
            [level, this]() {
                RS_LOGD_IF(DEBUG_PIPELINE, "Enter level:%{public}d, OnTrim Success", level);
...
void RSMainThread::SetCurtainScreenUsingStatus(bool isCurtainScreenOn)
{
    ...
    if (isCurtainScreenOn_ == isCurtainScreenOn) {
        RS_LOGD_IF(DEBUG_PIPELINE, "SetCurtainScreenUsingStatus: curtain screen status not change");
        return;
    }
    ...
    SetDirtyFlag();
    RequestNextVSync();
    RS_LOGD_IF(DEBUG_PIPELINE, "SetCurtainScreenUsingStatus %{public}d", isCurtainScreenOn);
#endif
}
...
bool RSMainThread::IsRebuildTransactionInProgress() const
{

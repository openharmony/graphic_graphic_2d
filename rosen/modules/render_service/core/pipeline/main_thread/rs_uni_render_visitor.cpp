...
void RSUniRenderVisitor::PartialRenderOptionInit()
{
    ...
    isCompleteRenderEnabled_ = (partialRenderType_ == PartialRenderType::FORCE_FULL_SCREEN_DIRTY_REGION);
    ...
    isOpDropped_ = isPartialRenderEnabled_ &&
        (partialRenderType_ >= PartialRenderType::CLIP_AND_DROP_OP) && !isRegionDebugEnabled_;
    ...
    auto dirtyAlignEnabled = RSSystemProperties::GetDirtyAlignEnabled();
    isDirtyAlignEnabled_ = dirtyAlignEnabled == DirtyAlignType::DEFAULT ? DirtyRegionParam::IsTileBasedAlignEnable() :
        RSSystemProperties::GetDirtyAlignEnabled() != DirtyAlignType::DISABLED;

    auto spocEnabled = RSSystemProperties::GetStencilPixelOcclusionCullingEnabled();
    isStencilPixelOcclusionCullingEnabled_ = spocEnabled == StencilPixelOcclusionCullingType::DEFAULT ?
        OcclusionCullingParam::IsStencilPixelOcclusionCullingEnable() :
        spocEnabled != StencilPixelOcclusionCullingType::DISABLED;
...
void RSUniRenderVisitor::CheckColorSpace(RSSurfaceRenderNode& node)
{
    ...
    if (!curScreenNode_) {
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckColorSpace: curScreenNode_ is nullptr");
        return;
    }
    ...
    if (gamut != GRAPHIC_COLOR_GAMUT_SRGB) {
        ...
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckColorSpace: node(%{public}s) set new colorgamut %{public}d",
...
{
    if (!node.IsOnTheTree()) {
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckColorSpaceWithSelfDrawingNode node(%{public}s) is not on the tree",
            node.GetName().c_str());
        return;
    }
    if (!node.IsHardwareForcedDisabled()) {
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckColorSpaceWithSelfDrawingNode node(%{public}s) is hardware-enabled",
            node.GetName().c_str());
        return;
    }
    if (curScreenNode_ == nullptr) {
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckColorSpaceWithSelfDrawingNode curScreenNode_ is nullptr");
        return;
    }
    ...
    if (ColorGamutParam::SkipOccludedNodeDuringColorGamutCollection() && appSurfaceNode
        ...
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckColorSpaceWithSelfDrawingNode node(%{public}s) failed to set new color "
    ...
    if (gamut != GRAPHIC_COLOR_GAMUT_SRGB) {
        curScreenNode_->UpdateColorSpace(gamut);
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckColorSpaceWithSelfDrawingNode node(%{public}s) set new colorgamut %{public}d",
            node.GetName().c_str(), static_cast<int>(gamut));
    }
}
...
void RSUniRenderVisitor::UpdateColorSpaceAfterHwcCalc(RSScreenRenderNode& node)
{
    ...
    for (const auto& selfDrawingNode : selfDrawingNodes) {
        if (!selfDrawingNode) {
            RS_LOGD_IF(DEBUG_PIPELINE, "UpdateColorSpaceAfterHwcCalc selfDrawingNode is nullptr");
            continue;
        }
        auto ancestorNode = selfDrawingNode->GetAncestorScreenNode().lock();
        if (!ancestorNode) {
            RS_LOGD_IF(DEBUG_PIPELINE, "UpdateColorSpaceAfterHwcCalc ancestorNode is nullptr");
            continue;
        }
...
{
    RSScreenType screenType = node.GetScreenProperty().GetScreenType();
...
{
    return node.GetScreenProperty().GetConnectionType() ==
...
{
    if (hasFingerprint_) {
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckPixelFormat hasFingerprint is true.");
        return;
    }
    if (!curScreenNode_) {
    ...
    if (node.GetFingerprint()) {
        ...
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckPixelFormat newPixelFormate_ is set 1010102 for fingerprint.");
        return;
    }
    if (!RSSystemProperties::GetHdrImageEnabled()) {
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckPixelFormat HdrImageEnabled false");
        return;
    }
    if (node.GetHDRImagePresent()) {
        RS_LOGD_IF(DEBUG_PIPELINE,
            "CheckPixelFormat HDRService GetHDRImagePresent true, surfaceNode: %{public}" PRIu64 "",
            node.GetId());
    ...
    if (node.GetHDRUIPresent()) {
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckPixelFormat HDRService GetHDRUIPresent true, surfaceNode: %{public}" PRIu64 "",
            node.GetId());
        curScreenNode_->SetHasUniRenderHdrSurface(true);
...
void RSUniRenderVisitor::HandlePixelFormat(RSScreenRenderNode& node)
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "HDRSwitch isHdrImageEnabled: %{public}d, isHdrVideoEnabled: %{public}d",
...
#ifdef RS_ENABLE_TV_SHUTTER_3D
    isCloseHdr |= RSTvShutter3DManager::Instance().ShouldForceCloseHdr(node);
#endif
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "HandlePixelFormat HDRService isHdrOn:%{public}d hasUniRenderHdrSurface:%{public}d "
    ...
    auto screenType = screenProperty.GetScreenType();

    RSHdrUtil::HandleVirtualScreenHDRStatus(node);

    if (screenType == VIRTUAL_TYPE_SCREEN) {
        auto pixelFormat = screenProperty.GetPixelFormat();
...
void RSUniRenderVisitor::ResetDisplayDirtyRegion()
{
    ...
    if (ret) {
        ...
        RS_LOGD_IF(DEBUG_PIPELINE, "ResetDisplayDirtyRegion on");
    }
}
...
bool RSUniRenderVisitor::CheckCurtainScreenUsingStatusChange() const
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "CheckCurtainScreenUsingStatusChange changed");
    return true;
}
...
bool RSUniRenderVisitor::CheckLuminanceStatusChange(ScreenId id)
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "CheckLuminanceStatusChange changed");
    return true;
}
...
bool RSUniRenderVisitor::IsFirstFrameOfOverdrawSwitch() const
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "IsFirstFrameOfOverdrawSwitch");
    return true;
}
...
bool RSUniRenderVisitor::IsWatermarkFlagChanged() const
{
    if (RSMainThread::Instance()->IsWatermarkFlagChanged()) {
        RS_LOGD_IF(DEBUG_PIPELINE, "FirstOrLastFrameOfWatermark");
        return true;
    } else {
        return false;
    }
    ...
    RSLogicalDisplayRenderNode& mirrorNode)
{
    ...
    auto securityExemptionList = curScreenNode_->GetScreenProperty().GetSecurityExemptionList();
    if (securityExemptionList.size() == 0) {
        RS_LOGD_IF(DEBUG_PIPELINE,
            "UpdateVirtualScreenSecurityExemption::node:%{public}" PRIu64 ", isSecurityExemption:false",
            node.GetId());
    ...
    for (const auto& exemptionLayer : securityExemptionList) {
        RS_LOGD_IF(DEBUG_PIPELINE, "UpdateVirtualScreenSecurityExemption::node:%{public}" PRIu64
    ...
    RS_LOGD_IF(DEBUG_PIPELINE,
        "UpdateVirtualScreenSecurityExemption::node:%{public}" PRIu64 ", isSecurityExemption:%{public}d",
    ...
    RSLogicalDisplayRenderNode& mirrorNode)
{
    ...
    auto rect = screenProperty.GetVisibleRect();
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "%{public}s: hasSecLayerInVisibleRect:%{public}d.", __func__, hasSecLayerInVisibleRect);
...
void RSUniRenderVisitor::QuickPrepareScreenRenderNode(RSScreenRenderNode& node, bool isParentPrepareInReverseOrder)
{
    ...
    // 0. init display info
    RS_TRACE_NAME_FMT("RSUniRender:%s %" PRIu64 " nodeId[%" PRIu64 "] dirtyStatus[%d] isPropertyDirty[%d] "
    ...
    if (dirtyFlag_) {
        RS_TRACE_NAME_FMT("need recalculate dirty region. isUniVisitorDirty:%d, zoomStateChange:%d, "
            "isActiveSurfaceRectChanged:%d, isSurfaceRectChanged:%d, isScreenRenderNodeDirty:%d",
            isDirty_, zoomStateChange_, curScreenDirtyManager_->IsActiveSurfaceRectChanged(),
            curScreenDirtyManager_->IsSurfaceRectChanged(), node.IsDirty());
        RS_LOGD_IF(DEBUG_PIPELINE, "need recalculate dirty region");
    }
...
void RSUniRenderVisitor::PrepareForSkippedCrossNode(RSSurfaceRenderNode& surfaceNode)
{
    ...
    if (!geoPtr->GetAbsMatrix().Invert(conversionMatrix)) {
        curScreenDirtyManager_->ResetDirtyAsSurfaceSize();
        RS_LOGW("Cross-Node matrix inverting for %{public}s failed, set full screen dirty.",
            nodeRef.GetName().c_str());
        return;
    }
    ...
    // 5. collect white list rect for cross node with conversion matrix
    bool isRotatingForWhiteList = displayNodeRotationChanged_ || isScreenRotationAnimating_ ||
        RSMainThread::Instance()->GetSystemAnimatedScenes() == SystemAnimatedScenes::SNAPSHOT_ROTATION;
    RSSpecialLayerUtils::CollectWhiteListRect(nodeRef, hasMirrorUsedInSpecialLayer_,
        isRotatingForWhiteList, *curScreenNode_, true);
    // 6. record all children surface nodes and their position on second display, for global dirty region conversion.
...
bool RSUniRenderVisitor::CheckIfSkipDrawInVirtualScreen(RSSurfaceRenderNode& node)
{
    ...
    if (specialLayerMgr.Find(SpecialLayerType::SKIP) ||
        !specialLayerMgr.FindScreenHasType(SpecialLayerType::IS_BLACK_LIST).empty()) {
...
void RSUniRenderVisitor::CollectHwcAndFilterNodesInSkippedSubTree(RSRenderNode& node)
{
    ...
    std::for_each((*children).begin(), (*children).end(), [this, &node](
        ...
        // Forward order: append at the end
        node.GetAllHwcNodeAndFilterNode().insert(node.GetAllHwcNodeAndFilterNode().end(),
            child->GetAllHwcNodeAndFilterNode().begin(), child->GetAllHwcNodeAndFilterNode().end());
    });
}
    ...
    bool isBlendNeedFilter)
{
    ...
    if (auto nodeParent = node.GetParent().lock()) {
        auto& parentList = nodeParent->GetAllHwcNodeAndFilterNode();
        auto& childList = node.GetAllHwcNodeAndFilterNode();
        if (isParentPrepareInReverseOrder) {
            parentList.insert(parentList.begin(), childList.begin(), childList.end());
        } else {
...
void RSUniRenderVisitor::QuickPrepareSurfaceRenderNode(RSSurfaceRenderNode& node, bool isParentPrepareInReverseOrder)
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE,
        ...
        "isPropertyDirty[%{public}d] isSubTreeAllDirty[%{public}d]",
        node.GetName().c_str(), node.GetId(), ExtractPid(node.GetId()), static_cast<int>(node.GetSurfaceNodeType()),
        node.IsSubTreeDirty(), node.IsFirstLevelCrossNode(), static_cast<int>(node.GetDirtyStatus()),
    ...
    if (isDimmingOn) {
        ...
        RS_LOGD_IF(DEBUG_PIPELINE,
            "HDRDiming IsDimmingOn: %{public}d, GetHDRPresent: %{public}d, GetHdrVideo: %{public}d",
    ...
    RS_LOGD_IF(DEBUG_PIPELINE,
        "QuickPrepare [%{public}s, %{public}" PRIu64 "] DirtyRect[%{public}d, %{public}d, %{public}d, %{public}d]",
...
bool RSUniRenderVisitor::PrepareForCloneNode(RSSurfaceRenderNode& node)
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "PrepareForCloneNode %{public}s is cloneNode", node.GetName().c_str());
...
void RSUniRenderVisitor::ResetCrossNodesVisitedStatus()
{
    ...
    for (NodeId& nodeId : hasVisitedCrossNodeIds_) {
        ...
        RS_LOGD_IF(DEBUG_PIPELINE, "%{public}s NodeId[%{public}" PRIu64 "]", __func__, nodeId);
...
CM_INLINE void RSUniRenderVisitor::PrepareForUIFirstNode(RSSurfaceRenderNode& node)
{
    ...
    if (isTargetUIFirstDfxEnabled_) {
        ...
        if (!node.uifirstState_.isTargetDfxEnabled && isTargetUIFirstDfxSurface) {
            RS_LOGD_IF(DEBUG_PIPELINE, "UIFirstDFX Name[%{public}s] ID[%{public}" PRIu64 "] OpenDebug",
                node.GetName().c_str(), node.GetId());
        }
...
void RSUniRenderVisitor::UpdateNodeVisibleRegion(RSSurfaceRenderNode& node)
{
    ...
    if (!curScreenNode_->IsFirstVisitCrossNodeDisplay() && node.IsFirstLevelCrossNode()) {
        RS_LOGD_IF(DEBUG_PIPELINE, "UpdateNodeVisibleRegion NodeName: %{public}s, NodeId: %{public}" PRIu64
...
CM_INLINE void RSUniRenderVisitor::CalculateOpaqueAndTransparentRegion(RSSurfaceRenderNode& node)
{
    ...
    if ((!curScreenNode_->IsFirstVisitCrossNodeDisplay() && node.IsFirstLevelCrossNode()) ||
        ...
        RS_LOGD_IF(DEBUG_PIPELINE, "CalculateOpaqueAndTransparentRegion Node: %{public}s, NodeId: %{public}" PRIu64
...
void RSUniRenderVisitor::CollectOcclusionInfoForWMS(RSSurfaceRenderNode& node)
{
    ...
    auto visibleLevel = RSUniDirtyComputeUtil::GetRegionVisibleLevel(selfDrawRegion, node.GetVisibleRegion());
...
void RSUniRenderVisitor::QuickPrepareEffectRenderNode(RSEffectRenderNode& node, bool isParentPrepareInReverseOrder)
{
    ...
    auto globalFilterRect = !node.FirstFrameHasEffectChildren() ?
        GetVisibleEffectDirty(node) : node.GetOldDirtyInSurface();
    auto globalHwcFilterRect = !node.FirstFrameHasEffectChildren() ?
        hwcVisitor_->GetHwcVisibleEffectDirty(node, globalFilterRect) : globalFilterRect;
    node.GetHwcRecorder().SetGlobalHwcFilterRect(globalHwcFilterRect);
    PostPrepare(node, isParentPrepareInReverseOrder, !isSubTreeNeedPrepare);
    prepareClipRect_ = prepareClipRect;
    hasAccumulatedClip_ = hasAccumulatedClip;
    dirtyFlag_ = dirtyFlag;
...
void RSUniRenderVisitor::QuickPrepareChildren(RSRenderNode& node)
{
    ...
    if (NeedPrepareChildrenInReverseOrder(node)) {
        auto& curFrameInfoDetail = node.GetCurFrameInfoDetail();
        curFrameInfoDetail.curFrameReverseChildren = true;
        RS_LOGD_IF(DEBUG_PIPELINE, "%{public}s NeedPrepareChildrenInReverseOrder nodeId: %{public}" PRIu64,
            __func__, node.GetId());
        std::for_each(
            ...
            child->QuickPrepare(shared_from_this(), true);
...
CM_INLINE bool RSUniRenderVisitor::BeforeUpdateSurfaceDirtyCalc(RSSurfaceRenderNode& node)
{
    ...
    if (autoCacheEnable_ && node.IsAppWindow()) {
        RSOpincManager::OpincSetInAppStateStart(node, unchangeMarkInApp_);
    }
...
CM_INLINE bool RSUniRenderVisitor::AfterUpdateSurfaceDirtyCalc(RSSurfaceRenderNode& node)
{
    ...
    bool isRotating = displayNodeRotationChanged_ || isScreenRotationAnimating_ ||
        RSMainThread::Instance()->GetSystemAnimatedScenes() == SystemAnimatedScenes::SNAPSHOT_ROTATION;
    RSSpecialLayerUtils::CollectWhiteListRect(node, hasMirrorUsedInSpecialLayer_, isRotating, *curScreenNode_);
    return true;
}
...
void RSUniRenderVisitor::ProcessAncoNode(const std::shared_ptr<RSSurfaceRenderNode>& hwcNodePtr, bool& ancoHasGpu)
{
    ...
    RS_LOGD_IF(DEBUG_PIPELINE, "rs debug: name:%{public}s id:%{public}" PRIu64 "src %{public}s dst %{public}s "
    ...
    if (!hwcNodePtr->GetRSSurfaceHandler() || !hwcNodePtr->GetRSSurfaceHandler()->GetBuffer()) {
        RS_LOGD_IF(DEBUG_PIPELINE, "rs debug: name:%{public}s id:%{public}" PRIu64 " handler or buffer is null, skip",
...
void RSUniRenderVisitor::UpdateHwcNodeEnableByPrevalidate(std::map<uint64_t, RequestCompositionType> &strategy,
    const RSRenderNodeMap& nodeMap)
{
    for (auto it : strategy) {
        ...
        if (it.second == RequestCompositionType::DEVICE_VCLD_OFF) {
            node->ResetVcldInfo();
            continue;
        }
    ...
    std::vector<std::shared_ptr<RSSurfaceRenderNode>>& topLayers)
{
    ...
    bool isSurfaceLockLayer = false;
    for (auto hwcNode : hwcNodes) {
        auto hwcNodePtr = hwcNode.lock();
        if (!hwcNodePtr || !hwcNodePtr->IsOnTheTree()) {
            continue;
        }
        // collect surface lock layer
        if (!isSurfaceLockLayer && hwcNodePtr->GetFixRotationByUser()) {
            isSurfaceLockLayer = true;
            RSMainThread::Instance()->SetHasSurfaceLockLayer(isSurfaceLockLayer);
        }
        auto surfaceHandler = hwcNodePtr->GetMutableRSSurfaceHandler();
        // obtain the leash or main window closest to the hwc node.
        auto instanceRootNode = hwcNodePtr->GetInstanceRootNode();
        auto appNode = RSBaseRenderNode::ReinterpretCast<RSSurfaceRenderNode>(instanceRootNode);
        if (hwcNodePtr->IsLayerTop()) {
            ...
            if (hasMirrorUsedInDirtyRegion_ && hwcNodePtr->GetRSSurfaceHandler() &&
                ...
                appNode && !(appNode->GetVisibleRegion().IsEmpty()) && curScreenDirtyManager_) {
...
void RSUniRenderVisitor::UpdateSurfaceDirtyAndGlobalDirty()
{
    ...
    UpdateHwcNodeDirtyRegionAndCreateLayer(topLayers);
    std::for_each(curMainAndLeashSurfaces.rbegin(), curMainAndLeashSurfaces.rend(),
        [this, &accumulatedDirtyRegion, &hasMainAndLeashSurfaceDirty](
    ...
    auto& hwcNodes = curScreenNode_->GetChildHwcNodes();
    UpdateIfHwcNodesHaveVisibleRegion(hwcNodes);
    ...
    const std::vector<std::weak_ptr<RSSurfaceRenderNode>>& hwcNodes)
{
    ...
    for (auto& hwcNode : hwcNodes) {
        auto hwcNodeShared = hwcNode.lock();
        if (!hwcNodeShared) {
            continue;
        }
        auto surfaceNode = RSBaseRenderNode::ReinterpretCast<RSSurfaceRenderNode>(hwcNodeShared->GetInstanceRootNode());
        if (!surfaceNode) {
            RS_LOGD_IF(DEBUG_PIPELINE, "%{public}s: surfaceNode is nullptr", __func__);
            continue;
        }
        ...
        UpdateHwcNodesIfVisibleForApp(surfaceNode, hwcNode, hasVisibleHwcNodes, needForceUpdateHwcNodes);
...
void RSUniRenderVisitor::CheckMergeDisplayDirtyByTransparent(RSSurfaceRenderNode& surfaceNode) const
{
    ...
    if (surfaceNode.IsTransparent()) {
        ...
        if (!transparentDirtyRect.IsEmpty()) {
            RS_LOGD_IF(DEBUG_PIPELINE, "CheckMergeDisplayDirtyByTransparent global merge transparent dirty "
...
void RSUniRenderVisitor::CheckMergeDisplayDirtyByZorderChanged(RSSurfaceRenderNode& surfaceNode) const
{
    ...
    if (surfaceNode.GetZorderChanged()) {
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckMergeDisplayDirtyByZorderChanged global merge GetZorderChanged "
            "%{public}s: global dirty %{public}s, add rect %{public}s", surfaceNode.GetName().c_str(),
            curScreenNode_->GetDirtyManager()->GetCurrentFrameDirtyRegion().ToString().c_str(),
...
void RSUniRenderVisitor::CheckMergeDisplayDirtyByPosChanged(RSSurfaceRenderNode& surfaceNode) const
{
    ...
    if (surfaceNode.GetAnimateState() || lastFrameSurfacePos != currentFrameSurfacePos) {
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckMergeDisplayDirtyByPosChanged global merge surface pos changed "
...
void RSUniRenderVisitor::CheckMergeDisplayDirtyByShadowChanged(RSSurfaceRenderNode& surfaceNode) const
{
    ...
    if (surfaceNode.GetRenderProperties().IsShadowValid() || isShadowDisappear) {
        ...
        if (!shadowDirtyRect.IsEmpty()) {
            RS_LOGD_IF(DEBUG_PIPELINE, "CheckMergeDisplayDirtyByShadowChanged global merge ShadowValid %{public}s: "
...
void RSUniRenderVisitor::CheckMergeDisplayDirtyBySurfaceChanged() const
{
    ...
    for (auto& surfaceChangedRect : surfaceChangedRects) {
        if (!surfaceChangedRect.IsEmpty()) {
            RS_LOGD_IF(DEBUG_PIPELINE,
                "CheckMergeDisplayDirtyBySurfaceChanged global merge Surface closed, global dirty %{public}s,"
                "add rect %{public}s",
                curScreenNode_->GetDirtyManager()->GetCurrentFrameDirtyRegion().ToString().c_str(),
...
void RSUniRenderVisitor::CheckMergeDisplayDirtyByAttractionChanged(RSSurfaceRenderNode& surfaceNode) const
{
    if (surfaceNode.GetRenderProperties().IsAttractionValid()) {
        ...
        RS_LOGD_IF(DEBUG_PIPELINE,
            "CheckMergeDisplayDirtyByAttraction global merge attraction %{public}s: global dirty %{public}s,"
...
void RSUniRenderVisitor::CheckMergeDisplayDirtyByTransparentRegions(RSSurfaceRenderNode& surfaceNode) const
{
    ...
    if (surfaceNode.HasContainerWindow()) {
        ...
        if (!containerDirtyRegion.IsEmpty()) {
            RS_LOGD_IF(DEBUG_PIPELINE, "CheckMergeDisplayDirtyByContainer global merge containerDirtyRegion "
        ...
        if (!transparentDirtyRegion.IsEmpty()) {
            RS_LOGD_IF(DEBUG_PIPELINE, "CheckMergeDisplayDirtyByTransparentRegions global merge TransparentDirtyRegion "
                "%{public}s: global dirty %{public}s, add region %{public}s", surfaceNode.GetName().c_str(),
                curScreenNode_->GetDirtyManager()->GetCurrentFrameDirtyRegion().ToString().c_str(),
    ...
    RSRenderNode& node, bool isBlendNeedFilter, const std::shared_ptr<RSRenderNode>& nodeParent)
{
    ...
    auto& instance = RSPointLightManager::Instance(node.GetLogicalDisplayNodeId());
...
void RSUniRenderVisitor::DisableOccludedHwcNodeInSkippedSubTree(const RSRenderNode& node) const
{
    ...
    const auto& allHwcAndFilterNodes = node.GetAllHwcNodeAndFilterNode();
    for (const auto& weakNodePtr : allHwcAndFilterNodes) {
        auto nodePtr = weakNodePtr.lock();
        if (!nodePtr || !nodePtr->IsHardwareEnabledType()) {
            continue;
        }
        auto hwcNodePtr = nodePtr->ReinterpretCastTo<RSSurfaceRenderNode>();
...
CM_INLINE void RSUniRenderVisitor::PostPrepare(RSRenderNode& node, bool isParentPrepareInReverseOrder,
    bool subTreeSkipped)
{
    node.UpdateLayerPartRenderDirtyRegion(curLayerPartRenderDirtyManager_);
    ...
    if (nodeParent) {
        ...
        nodeParent->GetOpincCache().UpdateSubTreeHasUnstableOpincNode(
            node.GetOpincCache(),
            RSOpincManager::IsSuggestOpincNode(node) && !RSOpincManager::IsOpincUnchangeState(node));
#ifdef SUBTREE_PARALLEL_ENABLE
...
void RSUniRenderVisitor::CollectFilterInfoAndUpdateDirty(RSRenderNode& node, RSDirtyRegionManager& dirtyManager)
{
...
void RSUniRenderVisitor::UpdateSurfaceRenderNodeScale(RSSurfaceRenderNode& node)
{
    ...
    if (RSUifirstManager::Instance().GetUiFirstType() == UiFirstCcmType::MULTI) {
        ...
        if (!getMinMaxScales) {
            RS_LOGD_IF(DEBUG_PIPELINE, "getMinMaxScales fail, node:%{public}s %{public}" PRIu64 "",
                node.GetName().c_str(), node.GetId());
...
void RSUniRenderVisitor::PrepareRootRenderNode(RSRootRenderNode& node)
{
    ...
    PostPrepare(node, false, !isSubTreeNeedPrepare);
...
void RSUniRenderVisitor::SetUniRenderThreadParam(std::unique_ptr<RSRenderThreadParams>& renderThreadParams)
{
    ...
    renderThreadParams->partialRenderType_ = partialRenderType_;
...
void RSUniRenderVisitor::CheckMergeScreenDirtyByRoundCornerDisplay() const
{
    ...
    if (!RSSingleton<RoundCornerDisplayManager>::GetInstance().GetRcdEnable()) {
        RS_LOGD_IF(DEBUG_PIPELINE, "CheckMergeScreenDirtyByRoundCornerDisplay rcd disabled!");
        return;
    }
    ...
    if (screenType == EXTERNAL_TYPE_SCREEN) {
        ...
        if (RSSingleton<RoundCornerDisplayManager>::GetInstance().HandleRoundCornerDirtyRect(
            ...
            RS_LOGD_IF(DEBUG_PIPELINE, "CheckMergeScreenDirtyByRoundCornerDisplay global merge topRcdNode dirty "
        ...
        if (RSSingleton<RoundCornerDisplayManager>::GetInstance().HandleRoundCornerDirtyRect(
            ...
            RS_LOGD_IF(DEBUG_PIPELINE, "CheckMergeScreenDirtyByRoundCornerDisplay global merge bottomRcdNode dirty "
                    "%{public}s, global dirty %{public}s, add rect %{public}s",
                std::to_string(curScreenNode_->GetScreenId()).c_str(),
                curScreenDirtyManager_->GetCurrentFrameDirtyRegion().ToString().c_str(),
...
void RSUniRenderVisitor::TryNotifyUIBufferAvailable()
{
    ...
    for (auto& id : uiBufferAvailableId_) {
        ...
        if (RSMainThread::Instance()->IsPidRebuilding(ExtractPid(id))) {
            ...
            RS_TRACE_NAME_FMT("TryNotifyUIBufferAvailable: rebuild transaction in progress id:%llu name:%s ",
                surfaceNode->GetId(), surfaceNode->GetName().c_str());
...
void RSUniRenderVisitor::DelayNotifyUIBufferAvailableIfNeed(const std::shared_ptr<RSSurfaceRenderNode>& surfaceNode)
{
    ...
    if (surfaceNode->IsPendingUIBufferNotify()) {
        ...
        RS_OPTIONAL_TRACE_NAME_FMT("DelayNotifyUIBuffer id:%llu name:%s curFrameTick:%llu", appWindowId,
            surfaceNode->GetName().c_str(), uiBufferNotifyDelayTick_);
    ...
    if (AllSiblingsUIBufferAvailable(storedLeashId)) {
        ...
        RS_TRACE_NAME_FMT("DelayNotifyUIBufferStart appWindowId:%llu name:%s curFrameTick:%llu", appWindowId,
            entry.name.c_str(), uiBufferNotifyDelayTick_);
    }
}
...
void RSUniRenderVisitor::FlushPendingAvailableUiBuffer(NodeId leashId)
{
    ...
    for (auto appWindowId : siblingIds) {
        ...
        if (surfaceNode) {
            RS_TRACE_NAME_FMT("FlushPendingAvailableUiBuffer appWindowId:%llu name:%s curFrameTick:%llu", appWindowId,
                surfaceNode->GetName().c_str(), uiBufferNotifyDelayTick_);
...
void RSUniRenderVisitor::CheckPendingUIBufferTimeout()
{
    ...
    for (NodeId leashId : timeoutLeashIds) {
        ...
        // Clean up residual entries in hasDestoryRebuildAppWindowMap_ for this leashId
        // (sibling AppWindows whose buffer never became available before timeout)
        auto residualAppWindows = nodeMap.GetPendingUIBufferAppWindowsByLeashId(leashId);
        for (auto residualId : residualAppWindows) {
            nodeMap.RemovePendingUIBufferEntry(residualId);
        }
    }
}
...
{
    auto handler = node.GetRSSurfaceHandler();
    if (handler == nullptr || !handler->HasReceivedTunnelLayerInfo()) {
        return;
    }
...
{
    if (!curScreenNode_) {
        RS_LOGD_IF(DEBUG_PIPELINE, "PrepareColorPickers: curScreenNode_ is nullptr");
        return;
    }

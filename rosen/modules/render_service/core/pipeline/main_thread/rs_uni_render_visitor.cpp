...
void RSUniRenderVisitor::ResetDisplayDirtyRegion()
{
    ...
    if (curDisplayNode_ == nullptr || screenManager_ == nullptr) {
        return;
    }
    bool ret = CheckScreenPowerChange() ||
        ...
        screenManager_->CheckVirtualScreenStatusChanged(curDisplayNode_->GetScreenId());
...
void RSUniRenderVisitor::CalculateOpaqueAndTransparentRegion(RSSurfaceRenderNode& node)
{
    ...
    node.CheckAndUpdateOpaqueRegion(
        screenRect_, curDisplayNode_->GetRotation(), node.IsContainerWindowTransparent());
...
bool RSUniRenderVisitor::InitDisplayInfo(RSDisplayRenderNode& node)
{
    ...
    curDisplayNode_->GetMultableSpecialLayerMgr().ClearSpecialLayerIds();

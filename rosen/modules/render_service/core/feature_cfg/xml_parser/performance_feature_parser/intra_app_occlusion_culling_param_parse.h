...
class IntraAppOcclusionCullingParamParse : public XMLParserBase {
public:
    IntraAppOcclusionCullingParamParse() = default;
    ~IntraAppOcclusionCullingParamParse() = default;
    int32_t ParseFeatureParam(FeatureParamMapType &featureMap, xmlNode &node) override;

private:
    int32_t ParseIntraAppOcclusionCullingInternal(FeatureParamMapType &featureMap, xmlNode &node);
};
} // namespace OHOS::Rosen

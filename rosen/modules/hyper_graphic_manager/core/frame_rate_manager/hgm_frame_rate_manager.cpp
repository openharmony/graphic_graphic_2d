...
void HgmFrameRateManager::HandleSceneEvent(pid_t pid, EventInfo eventInfo)
{
    ...
    if (STYLUS_STATUS_MAP.find(sceneName) != STYLUS_STATUS_MAP.end()) {
        if (stylusMode_ != STYLUS_STATUS_MAP.at(sceneName)) {
            HGM_LOGI("pid %{public}d, %{public}s", pid, sceneName.c_str());
        }
...
void HgmFrameRateManager::ProcessAncoRefreshRateVote(const std::string& voter, VoteInfo& curVoteInfo)
{
...
VoteInfo HgmFrameRateManager::ProcessRefreshRateVote()
{
    ...
    for (auto voterIter = voters_.begin(); voterIter != voters_.end(); voterIter++) {
        ...
        ProcessAncoRefreshRateVote(voter, curVoteInfo);
        ProcessVoteLog(curVoteInfo, false);

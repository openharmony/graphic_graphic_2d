...
static sptr<SurfaceBuffer> DmaMemAlloc(
    const int32_t& width, const int32_t& height, const std::unique_ptr<Media::PixelMap>& pixelMap)
{
...
static std::shared_ptr<Drawing::Surface> CreateSurface(
...
static std::shared_ptr<Drawing::DrawCmdList> MakePiexlMapDrawCmdList(
    std::shared_ptr<Drawing::DrawCmdList>& recording, RSDrawingContext& ctx)
{
    ...
    if (recording->GetOpItemSize() > LONGTEXT_WATER_LINE && recording->CountTextBlobNum() > LONGTEXT_WATER_LINE) {
        ...
        pixelMapDrawCmdList->AddDrawOp<Drawing::DrawPixelMapWithParmOpItem::ConstructorHandle>(
            objectHandle, sampling, paintHandle);
    }
...
std::shared_ptr<Drawing::DrawCmdList> RSExtendedModifierHelper::FinishDrawing(RSDrawingContext& ctx)
{
    ...
    if ((RSSystemProperties::GetGpuApiType() == GpuApiType::VULKAN ||
        ...
        RSSystemProperties::GetTextBlobAsPixelMap()) {

...
{
    GET_PROPERTY_FROM_MODIFIERS_NG(float, COVERAGE_NG_SHADER, LIGHT_INTENSITY, 0.f, =);
}
...
{
    GET_PROPERTY_FROM_MODIFIERS_NG(float, COVERAGE_NG_SHADER, ILLUMINATED_BORDER_WIDTH, 0.f, =);
}

int RSModifierExtractor::GetIlluminatedType() const
{
    GET_PROPERTY_FROM_MODIFIERS_NG(int, COVERAGE_NG_SHADER, ILLUMINATED_TYPE, 0, =);
}

float RSModifierExtractor::GetBloom() const
{
    GET_PROPERTY_FROM_MODIFIERS_NG(float, COVERAGE_NG_SHADER, BLOOM, 0.f, =);
}

...
void RSUIRippleMaskPara::SetProperty(const std::shared_ptr<RSUIFilterParaBase>& other)
{
    ...
    auto r = std::static_pointer_cast<RSAnimatableProperty<float>>(radius);
    auto w = std::static_pointer_cast<RSAnimatableProperty<float>>(width);
    auto c = std::static_pointer_cast<RSAnimatableProperty<Vector2f>>(center);
    if (r == nullptr || w == nullptr || c == nullptr) {

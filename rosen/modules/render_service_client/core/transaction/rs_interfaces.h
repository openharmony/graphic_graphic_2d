...
public:
    ...
    */
    ...
    /**
     * @brief Host (UEA) authorizes or revokes a guest (UEC) pid for a UIExtension surface node
     * whose NodeId is owned by the host process. Available before any UIContext or render
     * pipeline client of the host exists; must be called before the guest creates the node
     * via CreateNodeAndSurface, and after the guest established its client-to-render
     * connection (the authorization is applied only in render processes the guest connected to).
     * @param nodeId UIExtension surface node id generated by the host.
     * @param guestPid pid of the guest (UEC) process.
     * @param authorized true to authorize, false to revoke.
     * @return true if the render process accepted and applied the request (synchronous IPC).
     */

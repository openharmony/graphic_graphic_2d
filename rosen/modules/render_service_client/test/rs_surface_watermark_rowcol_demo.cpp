...
[[maybe_unused]] shared_ptr<Media::PixelMap> DecodePicture(
    const string &pathName, const Media::AllocatorType allocatorType = Media::AllocatorType::SHARE_MEM_ALLOC)
{
...
constexpr uint32_t SLEEP_FIVE_SECOND = 2;
...
int main()
{
    ...
    if (mode > 1) {
        ...
        std::cin >> nodeNum;
        std::cout << std::endl;
        std::cout << "Set surfaceNode Width:";
        std::cin >> width;
        std::cout << std::endl;
        std::cout << "Set surfaceNode Height:";
        std::cin >> height;
        std::cout << std::endl;
    }
    ...
    auto res = rsInterfaces.SetSurfaceWatermark(getpid(),
        ...
        rowCount,
        colCount);
    std::cout << "Test SetSurfaceWatermark with row=" << rowCount << ", col=" << colCount << ", result=" << res
              << std::endl;

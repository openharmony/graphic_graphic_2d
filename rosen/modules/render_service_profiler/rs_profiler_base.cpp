...
{
    return TimePauseApply2(parcel.ReadInt64());
}
...
NodeId RSProfilerBase::PatchPlainNodeId(const Parcel& parcel, NodeId id)
{
    ...
    return id | (static_cast<uint64_t>(1) << (BITS_30 + BITS_32));
}
...
std::string RSProfilerBase::ReplayImagePrintList()
{
    ...
    for (const auto& it : g_imageMap) {
        out += std::to_string(it.first >> BITS_32) + ":" + std::to_string(it.first & MASK_32_BITS) + " ";
    }

    return out;
}

uint64_t RSProfilerBase::TimePauseApply2(uint64_t time)
{
    return time;
}
...
static int32_t ImageUtilsSurfaceBufferReference(void* buffer)
{
    if (buffer == nullptr) {
        ...
        return (30 << 21) | (1 << 16) | 79/*ERR_SURFACEBUFFER_REFERENCE_FAILED*/;
    }
...
OHOS::Media::PixelMap* RSProfilerBase::PixelMapUnmarshalling(Parcel& parcel)
{
    ...
    [[maybe_unused]] const uint64_t replayUniqueId = (static_cast<uint64_t>(replayUniqueIdHi) << BITS_32) | replayUniqueIdLo;
    ...
    uint64_t replayUniqueId, Parcel& parcel, OHOS::Media::PixelMapError& error)
{
    ...
    if (pixelMap == nullptr) {
        OHOS::Media::PixelMap::ConstructPixelMapError(error, (30 << 21) | (1 << 16) | 82/*OHOS::Media::ERR_IMAGE_PIXELMAP_CREATE_FAILED*/, "pixelmap create failed");
        return nullptr;
    }
    ...
    if (!pixelMap->ReadImageInfo(parcel, imgInfo)) {
        ...
        delete pixelMap;
        OHOS::Media::PixelMap::ConstructPixelMapError(error, (30 << 21) | (1 << 16) | 150/*OHOS::Media::ERR_IMAGE_READ_PIXELMAP_FAILED*/, "read pixelmap failed");
        return nullptr;
    }
    ...
    if ((!isAstc) && bufferSize != rowDataSize * imgInfo.size.height) {
        ...
        OHOS::Media::PixelMap::ConstructPixelMapError(error, (30 << 21) | (1 << 16) | 83/*ERR_IMAGE_BUFFER_SIZE_PARCEL_ERROR*/,
    ...
    if (allocType == OHOS::Media::AllocatorType::SHARE_MEM_ALLOC) {
        ...
        if (fd < 0) {
            ...
            if (ptr != nullptr && static_cast<uint32_t>(bufferSize) == ptr->imageSize) {
                ...
                parcel.SkipBytes(HEADER_LENGTH);
            } else {
                // original
                HiviewDFX::HiLog::Error(LABEL, "fd < 0");
                delete pixelMap;
                OHOS::Media::PixelMap::ConstructPixelMapError(error, (30 << 21) | (1 << 16) | 84/*ERR_IMAGE_GET_FD_BAD*/, "fd acquisition failed");
                return nullptr;
            }
        }
        else {
            ...
            if (ptr == MAP_FAILED) {
                ...
                if (ptr == MAP_FAILED) {
                    ...
                    OHOS::Media::PixelMap::ConstructPixelMapError(error, (30 << 21) | (1 << 16) | 84/*ERR_IMAGE_GET_FD_BAD*/, "shared memory map in memalloc failed");
                    return nullptr;
                }
            }

            RSProfilerBase::ReplayImageAdd(replayUniqueId, ptr, bufferSize, HEADER_LENGTH);
        ...
        if (base == nullptr) {
            ...
            delete pixelMap;
            OHOS::Media::PixelMap::ConstructPixelMapError(error, (30 << 21) | (1 << 16) | 9/*ERR_IMAGE_GET_DATA_ABNORMAL*/, "ReadImageData failed");
            return nullptr;
        }
    }
#else
...
void RSProfilerBase::TransactionDataOnProcess(RSContext& context)
{
    ...
    for (auto item : RSProfilerBase::SpecParsePidListGet()) {
        const NodeId nodeId = (static_cast<NodeId>(item) << BITS_32) | 1 | (static_cast<NodeId>(1) << (BITS_30 + BITS_32));

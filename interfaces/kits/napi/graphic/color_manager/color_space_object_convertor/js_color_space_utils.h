...
const std::map<std::string, ApiColorSpaceType> STRING_TO_JS_MAP = {
    ...
    { "DISPLAY_BT2020_SRGB", ApiColorSpaceType::DISPLAY_BT2020_SRGB },
};
...
T* CheckParamsAndGetThis(const napi_env env, napi_callback_info info, const char* name = nullptr)
{
    ...
    if (resObject) {
        return napi_unwrap_s(env, resObject, &T::NAPI_TYPE_TAG, (void **)(&pointerValue)) == napi_ok ?
...
T* CheckSendableParamsAndGetThis(const napi_env env, napi_callback_info info, const char* name = nullptr)
{
    ...
    napi_value& resObject = propertyNameValue ? propertyNameValue : object;
    if (resObject) {
        return napi_unwrap_sendable_s(env, resObject, &T::NAPI_TYPE_TAG, (void **)(&pointerValue)) == napi_ok ?
            reinterpret_cast<T*>(pointerValue) : nullptr;
    }
    return nullptr;
}

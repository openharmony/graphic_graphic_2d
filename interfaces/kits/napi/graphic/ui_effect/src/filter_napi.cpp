...
napi_value FilterNapi::SetHeatDistortion(napi_env env, napi_callback_info info)
{
    ...
    if (argCount >= NUM_2) {
        noiseScale = GetSpecialValue(env, argValue[NUM_1]);
    }
    if (argCount >= NUM_3) {
        riseWeight = GetSpecialValue(env, argValue[NUM_2]);
    }
    if (argCount >= NUM_4) {
        progress = GetSpecialValue(env, argValue[NUM_3]);
    }
...
napi_value FilterNapi::SetBlurBubblesRise(napi_env env, napi_callback_info info)
{
    ...
    if (argCount >= NUM_5) {
        progress = GetSpecialValue(env, argValue[NUM_4]);
    }

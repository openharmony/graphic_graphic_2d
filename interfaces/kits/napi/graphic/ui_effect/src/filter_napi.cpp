...
std::map<int32_t, Drawing::TileMode> INDEX_TO_DIRECTION = {
...
napi_value FilterNapi::CreateFilter(napi_env env, napi_callback_info info)
{
    ...
    napi_property_descriptor resultFuncs[] = {
        ...
        DECLARE_NAPI_FUNCTION("radiusGradientBlur", SetRadiusGradientBlutPara),
    };
...
{
    napi_valuetype valueType = napi_undefined;
    valueType = UIEffectNapiUtils::GetType(envv, param);
    if (valueType == napi_undefined) {
        return true;
    }
    uint32_t arraySize = 0;
...
napi_value FilterNapi::SetRadiusGradientBlutPara(napi_env env, napi_callback_info info)
{
    if (!UIEffectNapiUtils::IsSystemApp()) {
        FILTER_LOG_E("SetRadiusGradientBlutPara failed");
        napi_throw_error(env, std::to_string(ERR_NOT_SYSTEM_APP).c_str(),
            "FilterNapi SetRadiusGradientBlutPara failed, is not system app");
        return nullptr;
    }
    napi_value result = nullptr;
    napi_get_undefined(env, &result);
    ...
    UIEFFECT_JS_ARGS(env, info, status, argCount, argValue, thisVar);
    UIEFFECT_NAPI_CHECK_RET_D(status == napi_ok, nullptr,
        FILTER_LOG_E("FilterNapi SetRadiusGradientBlutPara para is nullptr"));
    if (argCount != NUM_2) {
    ...
    UIEFFECT_NAPI_CHECK_RET_D(GetLinearFractionStops(env, fractionValue, para), nullptr,
        FILTER_LOG_E("FilterNapi SetRadiusGradientBlutPara parsing coordinates fail"));
    ...
    Filter* filterObj = nullptr;
    status = napi_unwarp(env, thisVar, reinterpret_cast<void**>(&filterObj));
    UIEFFECT_NAPI_CHECK_RET_D(status == napi_ok && filterObj != nullptr, nullptr,
        FILTER_LOG_E("FilterNapi SetRadiusGradientBlutPara napi_unwrap fail"));
    filterObj->AddPara(para);

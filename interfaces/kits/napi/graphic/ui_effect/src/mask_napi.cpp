...
napi_value MaskNapi::Constructor(napi_env env, napi_callback_info info)
{
    ...
    MaskNapi *maskNapi = new(std::nothrow) MaskNapi();
    UIEFFECT_NAPI_CHECK_RET_D(maskNapi != nullptr, nullptr,
        MASK_LOG_E("MaskNapi Constructor maskNapi is nullptr"));
    status = napi_wrap(env, jsThis, maskNapi, MaskNapi::Destructor, nullptr, nullptr);
    UIEFFECT_NAPI_CHECK_RET_DELETE_POINTER(status == napi_ok, nullptr, maskNapi,
...
{
    MaskNapi *maskNapi = static_cast<MaskNapi*>(nativeObject);
    UIEFFECT_NAPI_CHECK_RET_VOID_D(maskNapi != nullptr, MASK_LOG_E("MaskNapi Destructor nativeObject is nullptr"));
...
bool ParseRippleMask(napi_env env, napi_value* argv, const std::shared_ptr<RippleMaskPara>& rippleMask)
{
    ...
    if (ParseJsPoint(env, argv[NUM_0], center)) {
...
bool ParseRadialGradientMask(napi_env env, napi_value* argv, const std::shared_ptr<RadialGradientMaskPara>& mask)
{
    if (!mask) {
        return false;
    }
...
napi_value MaskNapi::Create(napi_env env, const std::shared_ptr<MaskPara>& maskPara)
{
    // Create New Object
    ...
    UIEFFECT_NAPI_CHECK_RET_D(mask != nullptr, maskObj,
    ...
    status = napi_wrap(env, maskObj, mask,
        ...
        nullptr, nullptr);
    UIEFFECT_NAPI_CHECK_RET_DELETE_POINTER(status == napi_ok && maskObj != nullptr, maskObj, mask,

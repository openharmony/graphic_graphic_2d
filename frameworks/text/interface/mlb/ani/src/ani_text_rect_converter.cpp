...
ani_status AniTextRectConverter::ParseRangeToNative(ani_env* env, ani_object obj, RectRange& rectRange)
{
    ani_int startTmp = 0;
    ani_status ret =
        env->Object_CallMethod_Int(obj, AniClassFindMethod(env, ANI_INTERFACE_RANGE, "<get>start", ":i"), &startTmp);
    if (ret != ANI_OK) {
        TEXT_LOGE("Failed to get start, ret %{public}d", ret);
        return ANI_INVALID_ARGS;
    }
    ani_int endTmp = 0;
    ret = env->Object_CallMethod_Int(obj, AniClassFindMethod(env, ANI_INTERFACE_RANGE, "<get>end", ":i"), &endTmp);
    if (ret != ANI_OK) {
    ...
    ani_env* env, const OHOS::Rosen::TextRect& textRect, ani_object& aniObj)
{
    ...
    aniObj = AniTextUtils::CreateAniObject(env, AniFindClass(env, ANI_CLASS_TEXT_BOX),
        AniClassFindMethod(env, ANI_CLASS_TEXT_BOX, "<ctor>", sign.c_str()), rectObj,
    ...
    ani_env* env, const OHOS::Rosen::Boundary& boundary, ani_object& aniObj)
{
    aniObj = AniTextUtils::CreateAniObject(env, AniFindClass(env, ANI_CLASS_RANGE),
        AniClassFindMethod(env, ANI_CLASS_RANGE, "<ctor>", "ii:"), ani_int(boundary.leftIndex),

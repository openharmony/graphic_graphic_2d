    ...
    ani_env* env, ani_object object, ani_object paragraphStyle, ani_object fontCollection)
{
    ...
    AniFontCollection* aniFontCollection = AniTextUtils::GetNativeFromObj<AniFontCollection>(
        env, fontCollection, AniGlobalMethod::fontCollectionGetNative);
...
ani_status AniParagraphBuilder::AniInit(ani_vm* vm, uint32_t* result)
{
    ...
    ret = env->Class_BindNativeMethods(AniGlobalClass::paragraphBuilder, methods.data(), methods.size());
    if (ret != ANI_OK) {
    ...
    ret = env->Class_BindStaticNativeMethods(
        AniGlobalClass::paragraphBuilder, staticMethods.data(), staticMethods.size());
    if (ret != ANI_OK) {
...
{
    AniParagraphBuilder* paragraphBuilder =
        AniTextUtils::GetNativeFromObj<AniParagraphBuilder>(env, object, AniGlobalMethod::paragraphBuilderGetNative);
...
ani_object AniParagraphBuilder::BuildLineTypeset(ani_env* env, ani_object object)
{
    ...
    ani_object lineTypographyObj =
        AniTextUtils::CreateAniObject(env, AniGlobalClass::lineTypeSet, AniGlobalMethod::lineTypesetCtor);
...
ani_object AniParagraphBuilder::NativeTransferStatic(ani_env* env, ani_class cls, ani_object input)
{
    return AniTransferUtils::TransferStatic(env, input, [](ani_env* env, void* unwrapResult) {
        ...
        ani_object staticObj =
            AniTextUtils::CreateAniObject(env, AniGlobalClass::paragraphBuilder, AniGlobalMethod::paragraphBuilderCtor);

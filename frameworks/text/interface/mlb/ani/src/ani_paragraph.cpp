...
ani_status AniParagraph::AniInit(ani_vm* vm, uint32_t* result)
{
    ...
    ret = env->Class_BindNativeMethods(AniGlobalClass::paragraph, methods.data(), methods.size());
    if (ret != ANI_OK) {
    ...
    ret = env->Class_BindStaticNativeMethods(AniGlobalClass::paragraph, staticMethods.data(), staticMethods.size());
    if (ret != ANI_OK) {
...
void AniParagraph::LayoutSync(ani_env* env, ani_object object, ani_double width)
{
    AniParagraph* aniParagraph =
        AniTextUtils::GetNativeFromObj<AniParagraph>(env, object, AniGlobalMethod::paragraphGetNative);
    ...
    ani_env* env, ani_object object, ani_object canvas, ani_object path, ani_double hOffset, ani_double vOffset)
{
    ...
    Drawing::AniPath* aniPath =
        AniTextUtils::GetNativeFromObj<Drawing::AniPath>(env, path, AniGlobalMethod::pathGetNative);

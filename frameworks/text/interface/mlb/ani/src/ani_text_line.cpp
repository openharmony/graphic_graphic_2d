...
ani_status AniTextLine::AniInit(ani_vm* vm, uint32_t* result)
{
    ...
    ret = env->Class_BindNativeMethods(AniGlobalClass::textLine, methods.data(), methods.size());
    if (ret != ANI_OK) {
    ...
    ret = env->Class_BindStaticNativeMethods(AniGlobalClass::textLine, staticMethods.data(), staticMethods.size());
    if (ret != ANI_OK) {
...
ani_int AniTextLine::GetGlyphCount(ani_env* env, ani_object object)
{
    AniTextLine* aniTextline =
        AniTextUtils::GetNativeFromObj<AniTextLine>(env, object, AniGlobalMethod::textLineGetNative);

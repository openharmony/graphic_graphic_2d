...
ani_status AniTextLine::AniInit(ani_vm* vm, uint32_t* result)
{
    ...
    ani_class cls = ANI_FIND_CLASS(env, ANI_CLASS_TEXT_LINE);
    if (cls == nullptr) {
        TEXT_LOGE("Failed to find class: %{public}s", ANI_CLASS_TEXT_LINE);
...
ani_object AniTextLine::CreateTextLine(ani_env* env, Rosen::TextLineBase* textLine)
{
    ...
    ani_object textLineObj = AniTextUtils::CreateAniObject(env, ANI_FIND_CLASS(env, ANI_CLASS_TEXT_LINE), ANI_CLASS_FIND_METHOD(env, ANI_CLASS_TEXT_LINE, "<ctor>", ":"));
...
{
    AniTextLine* aniTextline = AniTextUtils::GetNativeFromObj<AniTextLine>(env, object, ANI_CLASS_FIND_METHOD(env, ANI_CLASS_TEXT_LINE, "<get>nativeObj", ":l"));
...
ani_object AniTextLine::NativeTransferStatic(ani_env* env, ani_class cls, ani_object input)
{
    return AniTransferUtils::TransferStatic(env, input, [](ani_env* env, void* unwrapResult) {
        ...
        ani_object staticObj = AniTextUtils::CreateAniObject(env, ANI_FIND_CLASS(env, ANI_CLASS_TEXT_LINE), ANI_CLASS_FIND_METHOD(env, ANI_CLASS_TEXT_LINE, "<ctor>", ":"));

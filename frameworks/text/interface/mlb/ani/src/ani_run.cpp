...
ani_status AniRun::AniInit(ani_vm* vm, uint32_t* result)
{
    ...
    ret = env->Class_BindNativeMethods(AniGlobalClass::run, methods.data(), methods.size());
    if (ret != ANI_OK) {
    ...
    ret = env->Class_BindStaticNativeMethods(AniGlobalClass::run, staticMethods.data(), staticMethods.size());
    if (ret != ANI_OK) {
...
ani_object AniRun::CreateRun(ani_env* env, Rosen::Run* run)
{
    ...
    ani_object runObj = AniTextUtils::CreateAniObject(env, AniGlobalClass::run, AniGlobalMethod::run);
    ani_status ret =
...
{
    AniRun* aniRun = AniTextUtils::GetNativeFromObj<AniRun>(env, object, AniGlobalMethod::runGetNative);
...
ani_object AniRun::GetGlyphs(ani_env* env, ani_object object)
{
    ...
    for (const auto& glyph : glyphs) {
        ani_status ret = env->Object_CallMethod_Void(
            arrayObj, AniGlobalMethod::arraySet, index, AniTextUtils::CreateAniIntObj(env, glyph));
        if (ret != ANI_OK) {
...
ani_object AniRun::GetFont(ani_env* env, ani_object object)
{
    ...
    ani_object fontObj = AniTextUtils::CreateAniObject(env, AniGlobalClass::font, AniGlobalMethod::fontCtor);
...
ani_object AniRun::GetStringIndices(ani_env* env, ani_object object, ani_object range)
{
    ...
    for (const auto& stringIndex : stringIndices) {
        ani_status ret = env->Object_CallMethod_Void(
            arrayObj, AniGlobalMethod::arraySet, index, AniTextUtils::CreateAniIntObj(env, stringIndex));
        if (ret != ANI_OK) {
...
ani_object AniRun::NativeTransferStatic(ani_env* env, ani_class cls, ani_object input)
{
    return AniTransferUtils::TransferStatic(env, input, [](ani_env* env, void* unwrapResult) {
        ...
        ani_object staticObj = AniTextUtils::CreateAniObject(env, AniGlobalClass::run, AniGlobalMethod::run);

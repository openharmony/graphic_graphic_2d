...
ani_status AniRun::AniInit(ani_vm* vm, uint32_t* result)
{
    ...
    ani_class cls = ANIFindClass(env, ANI_CLASS_RUN);
    if (cls == nullptr) {
...
ani_object AniRun::CreateRun(ani_env* env, Rosen::Run* run)
{
    ...
    ani_object runObj = AniTextUtils::CreateAniObject(
        env, ANIFindClass(env, ANI_CLASS_RUN), ANIClassFindMethod(env, ANI_CLASS_RUN, "<ctor>", ":"));
    ani_status ret = env->Object_CallMethodByName_Void(runObj, BIND_NATIVE, "l:", reinterpret_cast<ani_long>(aniRun));
    if (ret != ANI_OK) {
...
ani_int AniRun::GetGlyphCount(ani_env* env, ani_object object)
{
    AniRun* aniRun = AniTextUtils::GetNativeFromObj<AniRun>(
        env, object, ANIClassFindMethod(env, ANI_CLASS_RUN, GET_NATIVE, ":l"));
...
ani_object AniRun::GetFont(ani_env* env, ani_object object)
{
    ...
    ani_object fontObj = AniTextUtils::CreateAniObject(
        env, ANIFindClass(env, ANI_CLASS_FONT), ANIClassFindMethod(env, ANI_CLASS_FONT, "<ctor>", ":"));
...
void AniRun::Paint(ani_env* env, ani_object object, ani_object canvas, ani_double x, ani_double y)
{
    ...
    Drawing::AniCanvas* aniCanvas = AniTextUtils::GetNativeFromObj<Drawing::AniCanvas>(
        env, canvas, ANIClassFindMethod(env, ANI_CLASS_CANVAS, GET_NATIVE, ":l"));
...
ani_object AniRun::NativeTransferStatic(ani_env* env, ani_class cls, ani_object input)
{
    return AniTransferUtils::TransferStatic(env, input, [](ani_env* env, void* unwrapResult) {
        ...
        aniRun->run_ = runPtr;
        ani_status ret =
            env->Object_CallMethodByName_Void(staticObj, BIND_NATIVE, "l:", reinterpret_cast<ani_long>(aniRun));
        if (ret != ANI_OK) {

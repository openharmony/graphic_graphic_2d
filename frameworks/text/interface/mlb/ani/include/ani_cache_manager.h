...
#define ANI_CLASS_FIND_METHOD(env, descriptor, name, signature) \
    ([&]{ \
        static ani_method __method = [&]() -> ani_method { \
            ...
            if (status != ANI_OK) { \
                TEXT_LOGE("Failed to find method: %{public}s::%{public}s::%{public}s, status %{public}d", descriptor, name, signature, status); \
                return nullptr; \
            } \
            return method; \
        }(); \
...
#define ANI_NAMESPACE_FIND_FUNCTION(env, descriptor, name, signature) \
    ([&]{ \
        static ani_function __function = [&]() -> ani_function { \
            ...
            if (status != ANI_OK) { \
                TEXT_LOGE("Failed to find function: %{public}s::%{public}s::%{public}s, status %{public}d", descriptor, name, signature, status); \
                return nullptr; \
            } \
            return function; \
        }(); \

    ...
    NapiAsyncExecute execute, NapiAsyncComplete complete)
{
    ...
    NAPI_CHECK_ARGS(contextBase, stat == napi_ok, stat, TextErrorCode::ERROR,
        return NapiTextResult::Invalid("Failed to create string,", promise),
    ...
    NAPI_CHECK_ARGS(contextBase, stat == napi_ok, stat, TextErrorCode::ERROR,
        return NapiTextResult::Invalid("Failed to create async work", promise), "Failed to create async work, stat:%d",
        static_cast<int>(stat));

    stat = napi_queue_async_work_with_qos(contextBase->env, contextBase->work, napi_qos_user_initiated);
    NAPI_CHECK_ARGS(contextBase, stat == napi_ok, stat, TextErrorCode::ERROR,
        return NapiTextResult::Invalid("Failed to queue async work", promise), "Failed to queue async work, stat:%d",
        static_cast<int>(stat));

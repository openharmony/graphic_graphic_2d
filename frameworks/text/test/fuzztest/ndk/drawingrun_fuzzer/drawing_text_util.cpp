...
OH_Drawing_TypographyStyle* UpdateTypographyStyle(OH_Drawing_TypographyStyle* typoStyle, FuzzedDataProvider& fdp)
{
    ...
    size_t familySize;
    char** fontFamily = OH_Drawing_TypographyTextlineStyleGetFontFamilies(typoStyle, &familySize);
    OH_Drawing_TypographyTextlineStyleDestroyFontFamilies(fontFamily, familySize);
...
void OHDrawingTypographyTest(const uint8_t* data, size_t size)
{
    ...
    OH_Drawing_TypographyPaintOnPath(typography, canvas, path, fdp.ConsumeIntegral<uint32_t>() % DATA_MAX_RANDOM,
        fdp.ConsumeIntegral<uint32_t>() % DATA_MAX_RANDOM);

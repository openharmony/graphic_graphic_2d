...
void RequestSync(const SyncFunc syncFunc, void *data = nullptr)
{
...
void ColorFlush()
{
    ...
    RequestSync(ColorSync, windowPtr);
}
...
class MyScreenShotCallback : public IScreenShotCallback {
public:
    virtual void OnScreenShot(const struct WMImageInfo &info) override
    {
        printf("width: %d\n", info.width);
        printf("height: %d\n", info.height);
        printf("format: %d\n", info.format);
        printf("size: %d\n", info.size);
        printf("data: %p\n", info.data);
        delete this; // cannot call twice
    }
};

class MyWindowShotCallback : public IWindowShotCallback {
public:
    virtual void OnWindowShot(const struct WMImageInfo &info) override
    {
        printf("width: %d\n", info.width);
        printf("height: %d\n", info.height);
        printf("format: %d\n", info.format);
...
void Test4()
{
    ...
    static MyWindowShotCallback mwsc;
    IWindowShotCallback *cb = &mwsc;
    WindowManager::GetInstance()->StartShotWindow(windowId, cb);
}

void Test5()
{
    static MyScreenShotCallback mssc;
    IScreenShotCallback *cb = &mssc;
    WindowManager::GetInstance()->StartShotScreen(cb);
}
...
void Test15ChildProcess(int32_t pipeFd[2], uint32_t length)
{
    ...
    write(pipeFd[1], &g_ipcConfig, sizeof(g_ipcConfig));
...
void Usage(std::string argv0, std::vector<struct WindowManagerTest>& tests)
{

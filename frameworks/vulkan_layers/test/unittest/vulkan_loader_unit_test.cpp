...
/**
 * @tc.name: test SetMetaData with fresh swapchain and static HDR metadata
 * @tc.desc: test SetMetaData function by creating fresh swapchain, setting HDR metadata, then presenting
 * @tc.type: FUNC
 * @tc.require: issueI9EYX2
 */
HWTEST_F(VulkanLoaderUnitTest, SetMetaData_Fresh_Swapchain_Static_Metadata_Test, TestSize.Level1)
{
    if (isSupportedVulkan_) {
        EXPECT_NE(fpCreateSwapchainKHR, nullptr);
        EXPECT_NE(fpDestroySwapchainKHR, nullptr);
        ...
        VkSwapchainCreateInfoKHR swapchainCI =
            GetSwapchainCreateInfo(VK_FORMAT_R8G8B8A8_UNORM, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR);

        VkSwapchainKHR testSwapChain = VK_NULL_HANDLE;
        VkResult err = fpCreateSwapchainKHR(device_, &swapchainCI, nullptr, &testSwapChain);
        EXPECT_EQ(err, VK_SUCCESS);
        EXPECT_NE(testSwapChain, VK_NULL_HANDLE);
        ...
        VkSwapchainKHR swapchains[] = { testSwapChain };
        fpSetHdrMetadataEXT(device_, 1, swapchains, &hdrMetadata);

        VkAcquireNextImageInfoKHR acquireInfo = {};
        acquireInfo.sType = VK_STRUCTURE_TYPE_ACQUIRE_NEXT_IMAGE_INFO_KHR;
        acquireInfo.swapchain = testSwapChain;
        acquireInfo.timeout = UINT64_MAX;
        acquireInfo.semaphore = semaphore_;
        acquireInfo.fence = (VkFence)nullptr;
        uint32_t imageIndex = 0;
        err = fpAcquireNextImage2KHR(device_, &acquireInfo, &imageIndex);
        EXPECT_EQ(err, VK_SUCCESS);
        ...
        presentInfo.pNext = nullptr;
        presentInfo.swapchainCount = 1;
        presentInfo.pSwapchains = &testSwapChain;
        presentInfo.pImageIndices = &imageIndex;
        presentInfo.pWaitSemaphores = &semaphore_;
        presentInfo.waitSemaphoreCount = 1;
        ...
        fpDestroySwapchainKHR(device_, testSwapChain, nullptr);
    }
}

/**
 * @tc.name: test SetMetaData with vivid dynamic metadata on fresh swapchain
 * @tc.desc: test SetMetaData function with vivid dynamic metadata on fresh swapchain
 * @tc.type: FUNC
 * @tc.require: issueI9EYX2
 */
HWTEST_F(VulkanLoaderUnitTest, SetMetaData_Fresh_Swapchain_Vivid_Metadata_Test, TestSize.Level1)
{
    if (isSupportedVulkan_) {
        EXPECT_NE(fpCreateSwapchainKHR, nullptr);
        EXPECT_NE(fpDestroySwapchainKHR, nullptr);

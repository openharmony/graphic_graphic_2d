...
修改离屏脏区裁剪逻辑时，必须检查当前帧所有可能触发屏幕级离屏的因素，区分可裁剪与不可裁剪：

- 旋转离屏：部分支持旋转的设备在旋转动画时启离屏，需要全屏显示，**不可裁剪**。
- SLR 采样离屏：扩展屏下采样/超分算法需要离屏，需要全屏显示，**不可裁剪**。
- HDR 离屏：需要 F16 buffer 承载超过 1 的亮度，**可裁剪**。
- ScRGB 离屏：PC 色域使用 ScRGB 色域的 F16 buffer 防止多窗场景的色域转换耗时，**可裁剪**。
- 交互优先、节点组缓存等窗口级离屏：不涉及屏幕级脏区裁剪，代码中应单独归类处理。

关键约束：屏幕级离屏可能由多个因素同时引起。只判断"存在 HDR 或 ScRGB"就裁剪脏区，
未考虑同时存在旋转或 SLR 离屏时脏区范围可能标记不全，会导致桌面黑块/漏刷。
裁剪条件应限制为"仅存在可裁剪的离屏因素（HDR/ScRGB）"再裁剪，存在不可裁剪的离屏因素
（旋转/SLR）时必须跳过裁剪。
...
- 离屏脏区裁剪优化是否检查了所有离屏触发因素（旋转/SLR/HDR/ScRGB），存在不可裁剪因素时是否跳过裁剪？

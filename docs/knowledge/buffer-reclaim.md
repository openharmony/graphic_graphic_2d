...
GPU 内存查杀拦截方案通过纹理 Tag 中的 Pid 统计各应用 GPU 内存占用，超过查杀水线时将该 Pid
加入查杀拦截列表，拦截该 Pid 后续的绘制（表现为黑屏/透明）。未打 Tag 的纹理（如 RS 自身申请的
GPU 内存、截屏/录屏/多任务场景）会被统计为 Pid 0。

关键缺陷：Pid 0 是"未打 Tag/未知归属"的占位特殊值，实际不存在该进程。一旦 Pid 0 的内存统计
超过水线被加入查杀列表，无法通过杀掉进程将其从列表中删除，导致 Pid 0 永久驻留查杀列表，
后续所有未打 Tag 的绘制全部被拦截，表现为截屏录屏全黑、多任务后台透明。

### 代码注意事项

- 查杀/拦截方案中，特殊值（如 Pid 0 代表"未打 Tag/未知归属"）必须与正常 Pid 走不同处理路径，
  不能加入查杀列表。Pid 0 不进行查杀（不列入查杀列表）。
- 查杀列表中添加 Pid 的代码必须验证该 Pid 是否为可杀进程，特殊占位值必须排除。添加的 Pid
  必须有对应的删除路径（进程退出时删除），否则会导致永久拦截。
- 新增基于 Pid 的拦截逻辑时，必须审查已有代码中对 Pid 0 的特殊处理，确保兼容其特殊语义。
  未打 Tag 的纹理（RS 自身申请的 GPU 内存、截屏/录屏/多任务场景）均会被统计为 Pid 0。

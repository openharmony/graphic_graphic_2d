...
插帧/算法类方案在修复画质问题或扩展生效范围时，容易引入小概率黑/花/闪问题。
泛化性扩展（如从单一产品扩展到存量产品、从少数地图扩展到 200+ 地图）会放大未暴露的小概率问题。

代码层面的注意事项：

- 插帧预测代码中必须增加异常检测逻辑：识别到异常 drawcall（如多次 bind buffer）时立即退出插帧预测，不做预测。
- 插帧方案涉及场景过多时，代码中应限制生效范围为最高频的少量场景（如 top10），避免泛化性扩展放大小概率问题。
- 大方案应支持通过配置文件快速关闭问题场景的插帧方案，便于快速收敛问题。

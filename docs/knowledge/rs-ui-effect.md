...
模糊接口获取区域为矩形区域，当模糊内容存在圆角时，会截到圆角区域外的内容（如桌面），
导致圆角发黑或闪烁。这是历史规格，修改模糊相关代码时需注意此行为。

修改模糊类效果代码时需注意：

- 矩形模糊区域与圆角窗口重叠时，会截到圆角外内容。卡片圆角、半模态窗圆角、组件圆角、
  应用启动窗圆角均受影响，圆角场景需要离屏方案避免此问题。
- 新增圆角离屏接口时，需与现有模糊接口的语义、参数、生效范围保持一致，避免应用侧误用。
- 默认优化方案性能影响较大时，应单独开接口由应用适配，而不是默认开启。

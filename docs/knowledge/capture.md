...
## 截屏录屏与 GPU 内存查杀拦截的交叉影响

截屏/录屏/多任务后台截图场景下，RS 申请的 GPU 纹理未打 Tag，会被 GPU 内存查杀拦截方案
统计为 Pid 0。当 Pid 0 内存超过查杀水线被加入拦截列表后，截屏录屏的绘制会被拦截，
表现为保存的文件全黑、多任务后台 APP 图标透明。

涉及内存查杀拦截方案改动时，需同步检查截屏/录屏/多任务等"未打 Tag"场景是否受影响，
详见 `buffer-reclaim.md` 中"查杀拦截方案的特殊值（Pid 0）防呆"。

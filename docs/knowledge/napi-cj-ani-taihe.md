...
9. 新增公开属性时，代码是否覆盖了组件 × 合成方式 × buffer 格式 × buffer 宽高不一致的所有组合分支？

## 组合场景规格

公开属性/接口在特殊组合场景下可能存在显示效果异常。典型案例：组件在 Surface 模式 + DSS 合成方式 +
黑背景 + 不透明 + buffer 宽高与 surface 不一致 + 特定 buffer 格式的组合下显示异常，
而 GPU 合成（绝大多数场景）正常。

新增公开属性（如 RenderFit 类）时，代码必须覆盖"组件类型 × 合成方式（DSS/GPU）× buffer 格式 ×
buffer 宽高与 surface 是否一致"的所有组合分支。DSS 合成路径与 GPU 合成路径行为可能不一致，
修改合成相关代码时需同时检查两条路径对同一属性的处理逻辑。

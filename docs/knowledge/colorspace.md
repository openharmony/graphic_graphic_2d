...
- ColorSpace 色彩空间定义与转换（SRGB / P3 / BT2020 / AdobeRGB 等）
- Color 颜色值表示与跨色域转换
- ColorSpaceConvertor 色域转换矩阵计算
- TransferFunc 传递函数（SRGB / Linear / Log）
...
```
ColorSpace
  ├─ colorSpaceName : ColorSpaceName (预定义 + CUSTOM)
...
关键预定义色域:
  ...
  BT2020 / LINEAR_SRGB / LINEAR_P3 / LINEAR_BT2020
  *_LIMIT 版本 (有限范围)
...
- 特殊 g 值标识：`HLG_G = -3.0f`、`PQ_G = -2.0f`、`LOG_G = -100.0f`
...
| 决策 | 代码体现 | 设计意图 |
|------|----------|----------|
| 特殊 g 值区分传递函数 | `HLG_G = -3.0f`、`PQ_G = -2.0f`、`LOG_G = -100.0f` | 统一 TransferFunc 结构，通过 g 值区分不同传递函数类型 |
...
- GamutMappingMode 两种模式的具体差异与使用场景

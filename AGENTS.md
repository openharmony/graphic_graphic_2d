...
代码、BUILD、接口/IPC、跨模块逻辑、设备行为或显示效果任务，
最终回复前按 `docs/review-gates/README.md` 判断需要执行的检视项。
纯文档、注释、知识路由、格式调整或不涉及代码行为的任务可以跳过，
并说明不适用原因。

## 提交和推送

若 Agent 负责 commit 或 push，必须确认相关代码交付门禁已完成；
门禁失败或不可用时，不继续 push，等待人工确认。
...
- 说明本次自主边界级别和累计改动文件数。
- 涉及代码交付门禁时，说明相关检视状态：已执行、失败、不可用或不适用。
- 当前为单仓环境时，说明取消编译的原因和替代静态检查；
  完整 OpenHarmony 根环境时，说明使用的产品和 target。
- 涉及提交或 push 时，说明交付门禁结果和 commit message 是否符合提交约定。

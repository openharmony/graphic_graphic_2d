...
（含稳定性及安全边界场景）。
稳定性检视采用询问制：改动涉及 C/C++ 业务代码、BUILD、接口/IPC 或跨模块逻辑时，
Agent 先询问用户是否执行稳定性检视，用户确认后再按 `stability.md` 完成检视；
用户未确认或拒绝时不执行，并在最终回复说明。
改动仅涉及测试、文档、注释、知识路由或格式调整等非业务代码时，稳定性检视不适用，无需询问，
在最终回复说明不适用原因即可。
